...
7. COMMISSION STRUCTURE (zero ambiguity): the 10% is ALREADY INCLUDED in the portal price. Agent quotes the portal price to their client; the agent's 10% comes out of that price — paid by the villa owner from the rent (on Samba-managed villas, paid by us out of the rent we collect). If the agent wants 20%, they may quote portal price + 10% to their client (the extra 10% comes from the client, not from us). Never say "commission is paid on top" or "you can earn 10% in addition to the price" — those phrasings break the deal structure.
...
export async function nodeHandler(req, res) {
  ...
  try {
    ...
    if (TEAM_NUMS.has(fromNum)) {
      ...
      try {
        const { handleExpenseMessage } = await import('../lib/expense-log.js');
        if (await handleExpenseMessage({
          db: relayDb, wa: relayWa, fromNum, fromName: fromNum === ERA_WA_NUM ? 'Era' : 'Ikiel', text, apiKey: ANTHROPIC_KEY,
          mediaType, mediaId, caption: extracted.caption || null,
          fetchImage: async (id) => { const m = await fetchWaMediaBase64(id, WA_TOKEN).catch(() => null); return m?.data ? { base64: m.data, contentType: m.mime || 'image/jpeg' } : null; },
        })) {
...
- Samba takes NO commission and NO booking fees. The only Samba cost is the flat IDR 150,000/month listing fee (free forever for founding villas).
- THE AGENT'S 10% (zero ambiguity — Ikiel, 5 Sep 2026): the agent's 10% comes OUT OF the price the owner lists. The agent quotes the listed price to the tenant; when the deal closes the OWNER pays the agent 10% of it from the rent received, and keeps 90%. Example: listed at 35jt/month → the agent quotes 35jt, the owner pays the agent 3.5jt and nets 31.5jt. So an owner should set a price that already has that 10% inside it. NEVER say the owner "gets the full amount", "keeps the full price" or that the 10% is added on top for the tenant — it is not. On MANAGED villas the rule is the same, except Samba collects the rent and Ikiel pays the agent the 10%; the owner's statement shows it.
...
Use "report" to fetch real numbers before answering a performance question (set report_slug, leave reply ""). Use "statements" to load their monthly statements before answering ANY question about payouts, expenses, bookings, fees or payment status (leave reply ""). Use "housekeeping" (managed villas) to load the cleaning log before answering ANY question about when the villa was cleaned, checked or inspected, what was flagged, or when the next clean, inspection or deep clean is (leave reply ""). Use "maintenance" (managed villas) to load their repair tickets before answering ANY question about a repair, a ticket, a claim, an approval, a cost, a "View details" link, or where the photos of a problem are (leave reply ""). Use "bookings" (managed villas) to load the booking calendar before answering who is staying, who arrives or leaves and when, how many nights, or whether the villa is free on some dates (leave reply ""); for a marketplace villa the owner runs their own calendar, so say so instead. ${booksPartner ? 'Use "books" to load the Tropicana Valley books before answering ANY question about the development\'s finances: the loan, cash, costs to pay, buyers, rent of the four units, ledger entries, categories, bank transfers (leave reply ""; set books_query to narrow the ledger). ' : ''}Use "login_link" when they ask for the portal link, cannot sign in, or lost their session (leave reply ""): a one-tap sign-in link goes to their WhatsApp and you are told whether it went; never promise a link without this action. Use "handbook" (set handbook_key, leave reply "") when they ask how something works — the portal, sign-in, pricing and billing, refunds, terms, what a statement line means, viewings, the cleaning standard, the guide — and the facts block is not enough; answer from the section that comes back. Use "import" to read an Airbnb/Booking.com page the owner linked (set import_url, leave reply ""). Use "intake" once you have enough to create or update a listing (set listing, leave reply ""). ${prospect ? 'Use "media" to send one curated image (set media_key AND a short caption in reply). Use "optout" if they clearly want to be left alone. ' : ''}Otherwise use "auto" (a normal reply) or "escalate".`;
  ...
  try {
    for (let hop = 0; hop < MAX; hop++) {
      ...
      if (parsed.action === 'housekeeping' && hop < MAX - 1) {
        // The cleaning log for their managed villas — the same records the
        // portal's Housekeeping tab shows, without staff identity.
        let block = '(housekeeping records unavailable right now)';
        try {
          if (db?.SUPABASE_URL) {
            const { ownerHousekeepingContext, ownerManagedSlugs } = await import('../lib/housekeeping-owner.js');
            const managed = await ownerManagedSlugs(db, { waNum: owner.wa_num, slugs: listingSlugs });
            block = (await ownerHousekeepingContext(db, { slugs: managed })).text;
          }
...
export async function submitOwnerIntake(owner, listing, secret) {
  ...
  try {
    ...
    // Optional extras the portal intake also understands (availability, the
