...
export default async function handler(req, res) {
  ...
  if (req.query?.relay === 'sweep') {
    try {
      ...
      // 16:00: the two-photo reminder and the "balas selesai" tip, once a week each.
      const evidenceNudge = await (async () => {
        try { const { runEvidenceNudge } = await import('../lib/housekeeping-backfill.js'); const { catalogNames } = await import('../lib/housekeeping.js'); return await runEvidenceNudge({ db: { SUPABASE_URL, sbHeaders }, wa: { phoneId: WA_PHONE_ID, token: WA_TOKEN }, catalogNames: await catalogNames({ SUPABASE_URL, sbHeaders }).catch(() => ({})) }); }
        catch (e) { return { error: e.message }; }
      })();
      // Rounds with their photos in but no "selesai": closed for her from 19:00.
      const roundsClosed = await (async () => {
        try { const { autoCloseRounds } = await import('../lib/housekeeping-intake.js'); return await autoCloseRounds({ db: { SUPABASE_URL, sbHeaders }, wa: { phoneId: WA_PHONE_ID, token: WA_TOKEN } }); }
        catch (e) { return { error: e.message }; }
      })();
      ...
      return res.status(200).json({ relay_sweep: out, sla_sweep: sla, closing_window_nudges: closing, housekeeping, hk_morning: hkMorning, hk_backfill: hkBackfill, rounds_closed: roundsClosed, evidence_nudge: evidenceNudge, channels, readiness, maintenance, era_backlog: eraBacklog, evening_chase: hkChase, era_brief: eraBrief, whats_new: whatsNew });
    } catch (e) {
