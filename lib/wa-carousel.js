...
// Pick the top-N available villas for the weekly carousel, joining the digest
// (availability) with the portal listings (cover image, rate, area). Returns
// null if fewer than N qualify — the caller falls back to the text digest.
export async function topAvailableVillas(digestProperties, n = CAROUSEL_CARD_COUNT) {
  ...
  const avail = (digestProperties || []).filter(p => p.availability?.availableToday && !p.isHidden);
  const cards = [];
  for (const p of avail) {
    const l = bySlug[p.slug];
    const cover = l?.coverPhotoId ? coverPhotoUrl(l.coverPhotoId) : null;
    if (!cover) continue;                       // no image → not carousel-worthy
    const beds = l?.unitType ? String(l.unitType).match(/\d+\s*(?:BR|bed)/i)?.[0] : '';
    const rate = p.monthly || l?.monthly || '';
    const detail = [rate && `${rate}/mo`, beds].filter(Boolean).join(', ') || 'monthly rental';
    cards.push({ name: p.name, area: p.area || l?.location || '', detail, slug: p.slug, imageUrl: cover });
...
export async function listingCarouselCards(n = CAROUSEL_CARD_COUNT) {
  ...
  for (const l of listings) {
    ...
    cards.push({ name: l.name, area: l.location || '', detail, slug: l.slug, imageUrl: coverPhotoUrl(l.coverPhotoId) });
    if (cards.length === n) break;
  }
  return cards.length === n ? cards : null;
}
...
export function buildCarouselComponents(firstName, villas) {
  return [
    ...
    { type: 'carousel', cards: villas.slice(0, CAROUSEL_CARD_COUNT).map((v, i) => {
      ...
      // the card line is just name + rate/type, never a raw link.
      const areaClean = v.area && !/^https?:|maps\./i.test(v.area) ? v.area : '';
      const line = `${v.name}${areaClean ? ` (${areaClean})` : ''}\n${v.detail}`;
      return {
        card_index: i,
        components: [
