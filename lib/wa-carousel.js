...
export async function createCarouselDigest(env, { name, sampleImageUrl }) {
  ...
  const card = {
    components: [
      ...
      { type: 'BODY', text: '{{1}}', example: { body_text: [['Villa Saturno in Padang Linjong -- 40jt/month, 3BR sleeps 6']] } },
  ...
  if (!r.ok) throw new Error(created?.error?.message || 'carousel create failed: ' + JSON.stringify(created).slice(0, 200));
