...
export function coverPhotoUrl(coverPhotoId) {
  // Served from our own domain (portal /api/media?source=img, retries + edge
  // cache) rather than lh3.googleusercontent.com directly: Meta's fetch of
  // the Drive URL failed intermittently with 131053 "media upload error".
  return coverPhotoId ? `${PORTAL_ORIGIN}/api/media?source=img&id=${encodeURIComponent(coverPhotoId)}&w=1600` : null;
}
...
// Set the WhatsApp number's profile picture (the avatar agents see on Maya's
// chats). Same resumable-upload flow as template headers: bytes → handle →
// profile_picture_handle on whatsapp_business_profile. env: { TOKEN, PHONE_ID }.
export async function setBusinessProfilePicture(env, buffer, mime) {
  const token = env.TOKEN;
  const appId = await deriveAppId(token);
  const handle = await uploadHeaderHandle(token, appId, buffer, mime);
  const res = await fetch(`${GRAPH}/${env.PHONE_ID}/whatsapp_business_profile`, {
    method: 'POST',
  ...
  if (!res.ok || data.error) {
    throw new Error('set profile picture failed: ' + JSON.stringify(data.error || data).slice(0, 200));
  }
  return { ok: true, handle, data };
}
...
export async function createCarouselDigest(env, { name, sampleImageUrl, bodyText, bodyExample }) {
  const token = env.TOKEN;
  ...
  const body = {
    ...
    category: 'MARKETING',
    components: [
      // v1 baked a full greeting + pitch around {{1}}, which senders then
      // stuffed with whole intro sentences → "Good morning Hi Wayan, …" and
      // the pitch said twice. v2-style templates pass a neutral body so the
      // send-time intro renders exactly once.
      { type: 'BODY',
        text: bodyText || 'Good morning {{1}}. Here is this week\'s Samba Rentals availability -- swipe through, tap any villa to see photos and live calendars, and share straight to a client. 10% commission on every rental closed.',
        example: { body_text: [[bodyExample || 'Wayan']] } },
      { type: 'CAROUSEL', cards }
    ]
  };
...
export async function topAvailableVillas(digestProperties, n = CAROUSEL_CARD_COUNT) {
  ...
  const pushCard = (slug, name, area, monthly, unitType, badge) => {
    ...
    // `tag` is the human area; `location` is usually a maps URL (which the
    // renderer drops), so tag is the field that actually shows on the card.
    cards.push({ name: name || l?.name, area: area || l?.tag || l?.location || '', detail, slug, imageUrl: cover, badge: l?.badge || null });
  };
  ...
  for (const l of listings) { if (cards.length >= n) break; pushCard(l.slug, l.name, l.tag || l.location, l.monthly, l.unitType); }
...
export async function listingCarouselCards(n = CAROUSEL_CARD_COUNT) {
  ...
  for (const l of listings) {
    ...
    cards.push({ name: l.name, area: l.tag || l.location || '', detail, slug: l.slug, imageUrl: coverPhotoUrl(l.coverPhotoId), badge: l.badge || null });
...
// ── Card images as Meta media ids, not links ────────────────────────
// With a `link`, Meta fetches every card image from our portal AT SEND TIME,
// once per recipient — the Monday digest is 6 cards × ~200 agents, a burst of
// ~1,200 fetches by Meta's crawlers within minutes. Any cold edge cache or
// Google hiccup in that burst fails the whole message with 131053 "media
// upload error" and the agent gets nothing (14 of 213 on 31 Aug 2026; 11 of
// 28 on 22 Aug before the proxy). Uploading each unique image ONCE per run
// to /{phone}/media and sending by `id` removes Meta's fetch entirely: the
// bytes are already at Meta before the first send. Media ids live 30 days,
// longer than any run. Never throws — a card whose upload fails keeps its
// link, so the worst case is exactly today's behaviour.
export async function uploadCardMedia(cards, { token, phoneId } = {}) {
  const out = { uploaded: 0, failed: 0, errors: [] };
  if (!token || !phoneId || !Array.isArray(cards)) return out;
  const byUrl = new Map();                    // same villa on several cards → one upload
  for (const c of cards) {
    if (!c?.imageUrl || c.imageId) continue;
    if (!byUrl.has(c.imageUrl)) byUrl.set(c.imageUrl, []);
    byUrl.get(c.imageUrl).push(c);
  }
  for (const [url, group] of byUrl) {
    try {
      let last = null, bytes = null, mime = 'image/jpeg';
      for (let attempt = 0; attempt < 3 && !bytes; attempt++) {
        try {
          const r = await fetch(url);
          if (!r.ok) { last = `image fetch ${r.status}`; continue; }
          mime = (r.headers.get('content-type') || 'image/jpeg').split(';')[0];
          bytes = Buffer.from(await r.arrayBuffer());
        } catch (e) { last = e.message; }
        if (!bytes) await new Promise(res => setTimeout(res, 300 * (attempt + 1)));
      }
      if (!bytes) throw new Error(last || 'image fetch failed');
      const form = new FormData();
      form.append('messaging_product', 'whatsapp');
      form.append('type', mime);
      form.append('file', new Blob([bytes], { type: mime }), 'card.jpg');
      const up = await fetch(`${GRAPH}/${phoneId}/media`, {
        method: 'POST', headers: { Authorization: 'Bearer ' + token }, body: form,
      });
      const d = await up.json().catch(() => ({}));
      if (!up.ok || !d?.id) throw new Error(d?.error?.message || `media upload HTTP ${up.status}`);
      for (const c of group) c.imageId = d.id;
      out.uploaded++;
    } catch (e) {
      out.failed++;
      out.errors.push(`${url.slice(-40)}: ${String(e.message || e).slice(0, 120)}`);
    }
  }
  return out;
}
...
export function buildCarouselComponents(firstName, villas, intro) {
  ...
  return [
    ...
    { type: 'carousel', cards: villas.slice(0, CAROUSEL_CARD_COUNT).map((v, i) => {
      // The card line is name (area) over rate/type. `area` can still be a
      // maps URL from older callers, so raw links are filtered here. A manual
      ...
      return {
        card_index: i,
        components: [
          { type: 'header', parameters: [{ type: 'image', image: v.imageId ? { id: v.imageId } : { link: v.imageUrl } }] },
