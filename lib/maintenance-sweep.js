...
// Rendered message per Meta message id — what the tukang, Era or the owner
// actually read — filled in by send() and written by logOut() instead of the
// bracket label (still the fallback for a template Meta has not returned).
...
async function logOut(db, { waNum, ownerId, content, mid, template, campaignId, category = 'maintenance' }) {
  ...
  await sbPost(db, 'wa_messages', {
    ...
    content: rendered || content, wa_message_id: typeof mid === 'string' ? mid : null,
...
export async function runMaintenanceSweep({
  ...
  const send = async (to, template, params, suffix, log) => {
    ...
    if (typeof mid === 'string') {
      const rendered = renderTemplateContent(templatesMap[template], params, { buttonSuffix: suffix, fallback: '' });
  ...
  // ── 1b) Heads-up: what was found, cost to follow ──────────────────
  // Same notice template, same owner gate; the title carries the caveat.
  if (has(T_NOTICE)) {
    const queue = (await sbGet(db, `maintenance_items?status=eq.new&heads_up_at=not.is.null&heads_up_sent_at=is.null&select=*,statement_groups(key,name,owner_names,notify,owner_wa_nums)&order=heads_up_at.asc&limit=30`)) || [];
    for (const item of queue) {
      const g = item.statement_groups || {};
      const nums = (g.owner_wa_nums || []).map(n => String(n).replace(/\D/g, '')).filter(Boolean);
      if (!g.notify || !nums.length) { out.skipped.push({ id: item.id, why: 'heads-up: no owner number / notify off' }); continue; }
      if (!claimed.has(item.group_key)) { out.skipped.push({ id: item.id, why: 'heads-up: owner has not claimed their portal account yet' }); continue; }
      const tok = maintenanceToken(item.group_key, item.id);
      let any = false;
      for (const to of nums) {
        const mid = await send(to, T_NOTICE, [firstName(g.owner_names), placeOf(item), `${item.title} — found by our team, being assessed; the repair cost follows for your approval`], tok,
          `[Maintenance heads-up — ${placeOf(item)}: ${item.title}]`);
        if (!mid) continue;
        any = true;
        if (!preview) {
          const ownerId = await ensureOwnerRow(db, to, g.owner_names);
          await logOut(db, { waNum: to, ownerId, mid, template: T_NOTICE, campaignId: camp?.id,
            content: `[Maintenance heads-up — ${placeOf(item)}: ${item.title}]` });
        }
      }
      if (any && !preview) await sbPatch(db, `maintenance_items?id=eq.${item.id}`, { heads_up_sent_at: nowIso(), updated_at: nowIso() });
      if (any) out.heads_up_sent = (out.heads_up_sent || 0) + 1;
    }
  }
  ...
  if (has(T_REMIND) && eraNum) {
    ...
    for (const item of queue) {
      // A tukang is booked or on site: the dispatch queues (5–8) speak for
      // this ticket; a parallel "is it finished?" is the same question twice.
      if (['offered', 'confirmed', 'arrived'].includes(item.visit_status)) {
        if (!preview) await sbPatch(db, `maintenance_items?id=eq.${item.id}`, { next_followup_at: inDays(3), updated_at: nowIso() });
        continue;
      }
