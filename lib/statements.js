...
// Receipts ("Received payment from …") become positive adjustment lines,
// so an expenses-only payout equals Era's own "Total Balance", negated.
async function syncExpenseSheet(db, group, { dryRun = false, out, newDrafts }) {
  ...
  for (const b of blocks.sort((x, y) => x.period.localeCompare(y.period))) {
    ...
    const receipts = (b.receipts || []).map(r => fullLine({ kind: 'adjustment', description: r.description, amount: Math.abs(r.amount), flags: ['receipt', 'expense_sheet'], source_row: r.source_row }));
    const ledgerLines = [...expenses, ...receipts];
