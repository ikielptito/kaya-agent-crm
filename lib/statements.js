...
// v2 opens the owner's portal on that month (deep link) instead of the /st/
// page; preferred once Meta approves it, v1 stays as the fallback.
const TEMPLATE_V2 = 'samba_owner_statement_v2';
// Co-owned, expenses-only properties get the profit-worded template: rent
// never passes through Samba there, so "total payout to you" would read as
// a negative number. Held (never sent with the wrong words) until approved.
const PROFIT_TEMPLATE = 'samba_owner_profit_v1';
...
// ── Owner-facing expense glossary ────────────────────────────────────
// Era's descriptions are shorthand ("Advance payment Sebastian"). Owners
// and Maya see a plain label and an explanation instead; the raw text stays
// on the admin side. settings.expense_glossary = { entries: [{ match:
// <regex source, case-insensitive>, label, explain }] }.
async function expenseGlossary(db) {
  const v = (await getSettingValue(db, 'expense_glossary')) || {};
  return (Array.isArray(v.entries) ? v.entries : []).map(e => {
    let re = null;
    try { re = new RegExp(e.match, 'i'); } catch { re = null; }
    return re ? { re, label: e.label || null, explain: e.explain || null } : null;
  }).filter(Boolean);
}
function glossaryFor(desc, glossary) {
  const d = String(desc || '');
  return glossary.find(g => g.re.test(d)) || null;
}
function applyGlossary(lines, glossary) {
  if (!glossary.length) return lines;
  return lines.map(l => {
    if (l.kind !== 'expense') return l;
    const g = glossaryFor(l.description, glossary);
    return g ? { ...l, description: g.label || l.description, explain: g.explain || null, raw_description: l.description } : l;
  });
}
...
export async function syncGroup(db, group, { dryRun = false, force = false, results = null } = {}) {
  ...
  for (const tab of tabs) {
    ...
    if (existing && (existing.status === 'published' || existing.status === 'paid')) {
      ...
      // Say what moved, not just that something did: the same line-up the
      // Payouts page shows, so the alert is actionable on the phone.
      const cur = (await sbGet(db, `statement_lines?statement_id=eq.${existing.id}&select=*`)) || [];
      const summary = diffLines(cur, linesFromParse(parsed));
      const bits = [];
      if (summary.added.length) bits.push(`${summary.added.length} added`);
      if (summary.removed.length) bits.push(`${summary.removed.length} removed`);
      if (summary.changed.length) bits.push(`${summary.changed.length} changed`);
      const detail = [...summary.added.map(x => `+ ${x.label} ${fmtIDR(x.amount)}`), ...summary.removed.map(x => `− ${x.label} ${fmtIDR(x.amount)}`), ...summary.changed.map(x => `~ ${x.label} ${fmtIDR(x.from)} → ${fmtIDR(x.to)}`)].slice(0, 8).join('\n');
      await sbPatch(db, `statements?id=eq.${existing.id}`, {
        source_changed: true,
        discrepancy: { detected_at: nowIso(), note: `Era edited tab "${tab.title}" after this statement was ${existing.status}`, new_era_payout: parsed.era_payout_total, published_payout: existing.payout_total, summary: bits.join(', ') || 'no line changes' },
        updated_at: nowIso(),
      });
      await postToTelegram(`⚠️ <b>Era changed a published statement's sheet</b>\n${group.name} · ${periodLabel(parsed.period)} (${existing.status})\nOwner saw ${fmtIDR(existing.payout_total)}; sheet now says ${fmtIDR(parsed.era_payout_total)} (${bits.join(', ') || 'lines unchanged'}).${detail ? '\n' + detail : ''}\nPayouts tab → "Bring the sheet in" or "Keep as published".`);
      continue;
    }
    ...
    let statementId;
    // Lines Maya added from Era's chat ("laundry HAUS 5 250rb") are not the
    // sheet's to remove: they ride through the re-import.
    const viaMaya = existing ? ((await sbGet(db, `statement_lines?statement_id=eq.${existing.id}&flags=cs.{via_maya}&select=*`)) || []) : [];
    if (existing) {
    ...
    if (statementId) {
      let pos = lines.length;
      if (viaMaya.length) {
        await sbPost(db, 'statement_lines', viaMaya.map(({ id, created_at, ...l }) => ({ ...l, position: pos++, statement_id: statementId })), 'return=minimal');
      }
      // Expenses Era told Maya before this month's draft existed.
      try {
        const { applyInbox } = await import('./expense-log.js');
        const r = await applyInbox(db, { statementId, group_key: group.key, period: parsed.period, sheetLines: lines });
        if (r.applied || r.matched) out.inbox = { ...(out.inbox || {}), [parsed.period]: r };
      } catch (e) { out.inbox_error = e.message; }
      if (viaMaya.length) await recomputeTotals(db, statementId);
    }
...
export async function amendFinalize(db, id, { note, notifyOwner = false, actor = 'admin', changes = null } = {}) {
  const st = (await sbGet(db, `statements?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!st) throw new Error('statement not found');
  const revs = Array.isArray(st.revisions) ? st.revisions : [];
  ...
  // What actually moved, line by line, so Maya can explain the revision to
  // the owner instead of pointing at a total.
  if (Array.isArray(changes) && changes.length) rev.changes = changes.slice(0, 20);
...
// ── Sheet changed after publish: show the difference, then act ──────
// The sync only knows the tab's hash moved. This re-reads the tab and lines
// up what Era changed against what the owner saw: lines added, removed,
// amounts changed, and the payout before and after. Lines Ikiel added by
// hand, auto carry lines and expense-ledger lines are not the sheet's to
// remove, so they are listed as kept.
const KEPT_FLAGS = ['manual', 'auto_carry', 'carry_forward', 'expense_sheet', 'direct_rent'];
const isKeptLine = (l) => (l.flags || []).some(f => KEPT_FLAGS.includes(f));
const lineKey = (l) => [l.kind, l.unit_name, l.guest_name, l.stay_dates, l.expense_date, l.description]
  .map(v => String(v == null ? '' : v).toLowerCase().replace(/\s+/g, ' ').trim()).join('|');
const lineLabel = (l) => l.kind === 'booking'
  ? `${l.guest_name || 'Booking'}${l.stay_dates ? ' · ' + l.stay_dates : ''}${l.unit_name ? ' · ' + l.unit_name : ''}`
  : `${l.expense_date ? l.expense_date + ' · ' : ''}${l.description || (l.kind === 'adjustment' ? 'Adjustment' : 'Expense')}`;

function diffLines(currentLines, sheetLines) {
  const cur = new Map();
  for (const l of currentLines) if (!isKeptLine(l)) cur.set(lineKey(l), l);
  const seen = new Set(); const added = [], removed = [], changed = [];
  for (const l of sheetLines) {
    const k = lineKey(l); const c = cur.get(k);
    if (!c) { added.push({ kind: l.kind, label: lineLabel(l), amount: l.kind === 'booking' ? l.nett : l.amount }); continue; }
    seen.add(k);
    const a = l.kind === 'booking' ? Number(c.nett) || 0 : Number(c.amount) || 0;
    const b = l.kind === 'booking' ? Number(l.nett) || 0 : Number(l.amount) || 0;
    if (Math.abs(a - b) > 1) changed.push({ kind: l.kind, label: lineLabel(l), from: a, to: b });
  }
  for (const [k, c] of cur) if (!seen.has(k)) removed.push({ kind: c.kind, label: lineLabel(c), amount: c.kind === 'booking' ? c.nett : c.amount });
  return { added, removed, changed };
}

async function reparseTab(db, st, group) {
  if (!group?.sheet_file_id) return null;
  const tabs = await listTabs(group.sheet_file_id);
  const wanted = tabs.find(t => t.title === st.source_tab) || null;
  const candidates = wanted ? [wanted] : tabs;
  for (const tab of candidates) {
    let parsed;
    try { parsed = parseMonthTab(await getTabValues(group.sheet_file_id, tab.title), { tabTitle: tab.title }); } catch { continue; }
    if (parsed && parsed.period === st.period) return { tab: tab.title, parsed };
  }
  return null;
}

export async function sheetDiff(db, id) {
  const st = (await sbGet(db, `statements?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!st) throw new Error('statement not found');
  const group = (await sbGet(db, `statement_groups?key=eq.${encodeURIComponent(st.group_key)}&select=*&limit=1`))?.[0];
  const rp = await reparseTab(db, st, group);
  if (!rp) return { ok: false, reason: 'no report tab for this month on the sheet', period: st.period };
  const current = (await sbGet(db, `statement_lines?statement_id=eq.${id}&select=*&order=position.asc,id.asc`)) || [];
  const kept = current.filter(isKeptLine);
  const { added, removed, changed } = diffLines(current, linesFromParse(rp.parsed));
  const keptAdj = kept.reduce((a, l) => a + (l.kind === 'adjustment' ? Number(l.amount) || 0 : l.kind === 'expense' ? -(Number(l.amount) || 0) : 0), 0);
  const t = rp.parsed.totals;
  return {
    ok: true, period: st.period, tab: rp.tab,
    published: { gross: st.gross_total, expenses: st.expenses_total, payout: st.payout_total },
    sheet: { gross: t.gross, expenses: t.expenses, payout: t.payout + keptAdj, era_payout: rp.parsed.era_payout_total },
    added, removed, changed, kept: kept.map(l => ({ kind: l.kind, label: lineLabel(l), amount: l.amount })),
    unchanged: added.length === 0 && removed.length === 0 && changed.length === 0,
  };
}

// Replace the sheet's lines with what the sheet says now, inside a proper
// amendment (owner-visible note, previous totals kept in the revision), and
// clear the flag. Hand-added, carry and ledger lines survive untouched.
export async function amendFromSheet(db, id, { notifyOwner = false, actor = 'admin' } = {}) {
  const diff = await sheetDiff(db, id);
  if (!diff.ok) throw new Error(diff.reason);
  const st = (await sbGet(db, `statements?id=eq.${id}&select=*&limit=1`))?.[0];
  const group = (await sbGet(db, `statement_groups?key=eq.${encodeURIComponent(st.group_key)}&select=*&limit=1`))?.[0];
  const rp = await reparseTab(db, st, group);
  await amendStart(db, id, actor);
  const current = (await sbGet(db, `statement_lines?statement_id=eq.${id}&select=*&order=position.asc,id.asc`)) || [];
  const kept = current.filter(isKeptLine);
  await sbDelete(db, `statement_lines?statement_id=eq.${id}`);
  const fresh = linesFromParse(rp.parsed).map((l, i) => ({ ...l, position: i, statement_id: id }));
  let pos = fresh.length;
  const restore = kept.map(l => { const { id: _id, created_at, ...rest } = l; return fullLine({ ...rest, position: pos++, statement_id: id }); });
  if (fresh.length) await sbPost(db, 'statement_lines', fresh, 'return=minimal');
  if (restore.length) await sbPost(db, 'statement_lines', restore, 'return=minimal');
  const parts = [];
  if (diff.added.length) parts.push(`${diff.added.length} line${diff.added.length > 1 ? 's' : ''} added`);
  if (diff.removed.length) parts.push(`${diff.removed.length} removed`);
  if (diff.changed.length) parts.push(`${diff.changed.length} amount${diff.changed.length > 1 ? 's' : ''} changed`);
  const note = `Updated from Era's sheet (${parts.join(', ') || 'no line changes'})`;
  await sbPatch(db, `statements?id=eq.${id}`, {
    source_hash: rp.parsed.source_hash, source_tab: rp.tab, era_payout_total: rp.parsed.era_payout_total,
    reconciliation: rp.parsed.reconciliation, source_changed: false,
    discrepancy: { ...(st.discrepancy || {}), resolved: 'amended', resolved_at: nowIso() }, updated_at: nowIso(),
  });
  const changes = [
    ...diff.added.map(x => ({ kind: x.kind, change: 'added', label: x.label, amount: x.amount })),
    ...diff.removed.map(x => ({ kind: x.kind, change: 'removed', label: x.label, amount: x.amount })),
    ...diff.changed.map(x => ({ kind: x.kind, change: 'changed', label: x.label, from: x.from, to: x.to })),
  ];
  const out = await amendFinalize(db, id, { note, notifyOwner, actor, changes });
  return { ...out, note, diff };
}

// Attach the line-by-line changes to the latest closed revision (used once
// to backfill the two amendments made before revisions carried them).
export async function setRevisionChanges(db, id, changes) {
  const st = (await sbGet(db, `statements?id=eq.${id}&select=revisions&limit=1`))?.[0];
  if (!st) throw new Error('statement not found');
  const revs = Array.isArray(st.revisions) ? st.revisions : [];
  const last = [...revs].reverse().find(r => r && !r.open);
  if (!last) throw new Error('no closed revision');
  last.changes = (changes || []).slice(0, 20);
  await sbPatch(db, `statements?id=eq.${id}`, { revisions: revs, updated_at: nowIso() });
  return { ok: true };
}

// Keep the published numbers: acknowledge the change so it stops alerting,
// and remember the sheet as it is now so only a further edit re-flags it.
export async function dismissDiscrepancy(db, id) {
  const st = (await sbGet(db, `statements?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!st) throw new Error('statement not found');
  const group = (await sbGet(db, `statement_groups?key=eq.${encodeURIComponent(st.group_key)}&select=*&limit=1`))?.[0];
  const rp = await reparseTab(db, st, group);
  await sbPatch(db, `statements?id=eq.${id}`, {
    source_changed: false, ...(rp ? { source_hash: rp.parsed.source_hash } : {}),
    discrepancy: { ...(st.discrepancy || {}), resolved: 'kept', resolved_at: nowIso() }, updated_at: nowIso(),
  });
  return { ok: true };
}
...
export async function publicStatement(db, groupKey, period) {
  ...
  const rawLines = await sbGet(db, `statement_lines?statement_id=eq.${st.id}&select=kind,unit_name,position,guest_name,stay_dates,platform,nights,amount,commission,nett,expense_date,description&order=position.asc`);
  const lines = applyGlossary(Array.isArray(rawLines) ? rawLines : [], await expenseGlossary(db));
...
export async function runOwnerStatementSweep({ SUPABASE_URL, sbHeaders, WA_TOKEN, WA_PHONE_ID, templatesMap = {}, preview = false }) {
  ...
  const sendable = queue.filter(st => st.statement_groups?.notify
    ...
    && (!st.statement_groups?.expenses_only || preview || templatesMap[PROFIT_TEMPLATE]));
  const heldForTemplate = queue.filter(st => st.statement_groups?.expenses_only && !templatesMap[PROFIT_TEMPLATE]).length;
  ...
  for (const st of sendable.slice(0, preview ? sendable.length : cap)) {
    ...
    const revised = (st.revisions || []).some(r => r && !r.open);
    const month = periodLabel(st.period) + (revised ? ' (revised)' : '');
    const payout = fmtIDR(st.payout_total);
    // Expenses-only: net profit and expenses for the month, from the portal's
    // profit endpoint (the statement token authorises it).
    let profit = null;
    if (g.expenses_only) {
      try {
        const r = await fetch(`${PORTAL_BASE}/api/statements?action=pnl&token=${encodeURIComponent(tok)}&from=${st.period}&to=${st.period}`, { signal: AbortSignal.timeout(15000) });
        const d = r.ok ? await r.json() : null;
        const m = d?.months?.[0];
        if (m) profit = { net: fmtIDR(m.net), expenses: fmtIDR(m.expenses) };
      } catch { profit = null; }
      if (!profit) profit = { net: fmtIDR(-Number(st.expenses_total || 0)), expenses: fmtIDR(st.expenses_total) };
    }
    if (preview) {
      plan.push({ statement: `${st.group_key} ${st.period}`, to: g.owner_wa_nums, owner: g.owner_names, template: g.expenses_only ? PROFIT_TEMPLATE : TEMPLATE_NAME, payout: g.expenses_only ? `net profit ${profit.net} · expenses ${profit.expenses}` : payout, url: `${PORTAL_BASE}/st/${tok}` });
      continue;
    }
    let anySent = false;
    for (const num of g.owner_wa_nums) {
      ...
      const mid = g.expenses_only
        ? await sendProfitTemplate(WA_PHONE_ID, WA_TOKEN, to, { name: firstName, month, property: g.name, net: profit.net, expenses: profit.expenses, tok })
        : await sendStatementTemplate(WA_PHONE_ID, WA_TOKEN, to, { name: firstName, month, property: g.name, payout, tok, deepLink: templatesMap[TEMPLATE_V2] ? `${st.group_key}.${st.period}` : null });
      ...
      await sbPost(db, 'wa_messages', {
        ...
        content: g.expenses_only ? `[Monthly statement sent — ${g.name} · ${month}: net profit ${profit.net}, expenses ${profit.expenses}]` : `[Monthly statement sent — ${g.name} · ${month}: payout ${payout}]`,
        wa_message_id: typeof mid === 'string' ? mid : null,
        timestamp: nowIso(), source: 'cron', category: 'owner_statement',
        campaign_id: camp?.id || null, template_name: g.expenses_only ? PROFIT_TEMPLATE : (templatesMap[TEMPLATE_V2] ? TEMPLATE_V2 : TEMPLATE_NAME), status: 'sent',
  ...
  if (!preview) {
    await noteRun(db, camp, { sent, failed, summary: { sent, failed, statements: statementsNotified, queued: sendable.length, held_for_onboarding: heldForOnboarding, held_for_template: heldForTemplate } });
  }
  return { queued: sendable.length, sent, failed, statements: statementsNotified, held_for_onboarding: heldForOnboarding, held_for_template: heldForTemplate, ...(preview ? { plan } : {}) };
}

async function sendStatementTemplate(phoneId, token, to, { name, month, property, payout, tok, deepLink = null }) {
  try {
    const r = await fetch(`${GRAPH}/${phoneId}/messages`, {
      ...
      body: JSON.stringify({
        ...
        template: {
          name: deepLink ? TEMPLATE_V2 : TEMPLATE_NAME,
          language: { code: 'en' },
          components: [
            { type: 'body', parameters: [name, month, property, payout].map(text => ({ type: 'text', text: String(text).replace(/\s*[–—]\s*/g, ' · ') })) },
            { type: 'button', sub_type: 'url', index: '0', parameters: [{ type: 'text', text: deepLink || tok }] },
          ],
        },
      }),
    });
...
async function sendProfitTemplate(phoneId, token, to, { name, month, property, net, expenses, tok }) {
  try {
    const r = await fetch(`${GRAPH}/${phoneId}/messages`, {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + token, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        messaging_product: 'whatsapp', to, type: 'template',
        template: {
          name: PROFIT_TEMPLATE, language: { code: 'en' },
          components: [
            { type: 'body', parameters: [name, month, property, net, expenses].map(text => ({ type: 'text', text: String(text).replace(/\s*[–—]\s*/g, ' · ') })) },
...
// Re-queue a published statement for Maya's next pass (an owner who was
// onboarded after a quiet publish, or a template that was not ready).
export async function renotifyStatement(db, id) {
  const st = (await sbGet(db, `statements?id=eq.${id}&select=id,status&limit=1`))?.[0];
  if (!st) throw new Error('statement not found');
  if (!['published', 'partial', 'paid'].includes(st.status)) throw new Error(`cannot re-queue a ${st.status} statement`);
  await sbPatch(db, `statements?id=eq.${id}`, { notified_at: null, updated_at: nowIso() });
  return { ok: true };
}
...
export async function ownerStatementsContext(db, { waNum, slugs = [], months = 6 } = {}) {
  const glossary = await expenseGlossary(db);
  ...
  for (const g of groups) {
    ...
    for (const s of sts) {
      const rawLines = (await sbGet(db, `statement_lines?statement_id=eq.${s.id}&select=kind,unit_name,guest_name,stay_dates,platform,nights,amount,commission,nett,expense_date,description&order=position.asc`)) || [];
      const lines = applyGlossary(rawLines, glossary);
      ...
      if (expenses.length) {
        ...
        for (const e of expenses.slice(0, 25)) out.push(`    · ${e.expense_date || ''} ${e.description || ''}: ${fmtIDR(e.amount)}${e.explain ? ` [what this is: ${e.explain}${e.raw_description ? `; Era's wording: "${e.raw_description}"` : ''}]` : ''}`);
      ...
      const revs = (s.revisions || []).filter(r => r && !r.open);
      for (const r of revs) {
        const when = r.at ? new Date(r.at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'Asia/Makassar' }) : '';
        const prevP = r.prev?.payout_total, newP = r.new?.payout_total;
        const lines = (r.changes || []).map(c => c.change === 'changed'
          ? `${c.kind} ${c.label}: ${fmtIDR(c.from)} → ${fmtIDR(c.to)}`
          : `${c.change} ${c.kind} ${c.label} ${fmtIDR(c.amount)}`);
        out.push(`  REVISED${when ? ' ' + when : ''}: payout ${prevP != null ? fmtIDR(prevP) : '?'} → ${newP != null ? fmtIDR(newP) : '?'}${r.note ? ` (${r.note})` : ''}${lines.length ? `; what changed: ${lines.join('; ')}` : ''}`);
      }
    }
  }
