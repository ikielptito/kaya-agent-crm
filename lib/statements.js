...
async function fetchHostexSnapshot(group, period) {
  try {
    const slugs = (group?.listing_slugs || []).join(',');
    if (!slugs) return null;
    const r = await fetch(`${PORTAL_BASE}/api/statements?action=month-stats&slugs=${encodeURIComponent(slugs)}&period=${period}`, {
...
export async function refreshSnapshot(db, id) {
  ...
  const group = (await sbGet(db, `statement_groups?key=eq.${st.group_key}&select=*&limit=1`))?.[0];
  const hostex = await fetchHostexSnapshot(group, st.period);
