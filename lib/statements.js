...
export async function syncGroup(db, group, { dryRun = false, force = false, results = null } = {}) {
  ...
  for (const tab of tabs) {
    ...
    if (existing && existing.source_hash === parsed.source_hash) { out.unchanged++; continue; }
    ...
    if (existing) {
      ...
      out.created++;
      await postToTelegram(`📊 <b>New draft statement</b>\n${group.name} · ${periodLabel(parsed.period)}\nPayout ${fmtIDR(totals.payout_total)}${parsed.needs_review ? ' · ⚠ needs review' : ''}\nReview &amp; publish in the admin Payouts tab.`);
    }
