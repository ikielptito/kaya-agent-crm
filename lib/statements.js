...
// ── Statement-recorded guest nights per unit slug ───────────────────
// Era's booking lines are the truth for direct rentals the Hostex calendar
// never saw. Owner stays (platform "Owner booking" / zero-amount lines)
// are NOT guest nights and are excluded. unit_name is Era's free text
// ("Unit 2 Haus Canggu"); mapped to slugs by the unit number for
// multi-unit groups, trivially for single-unit ones.
export async function statementUnitNights(db, groupKey, from, to) {
  ...
  let q = `statements?group_key=eq.${encodeURIComponent(groupKey)}&status=neq.void&select=id,period&order=period.asc`;
  if (/^\d{4}-\d{2}$/.test(from)) q += `&period=gte.${from}`;
  if (/^\d{4}-\d{2}$/.test(to)) q += `&period=lte.${to}`;
  const sts = (await sbGet(db, q)) || [];
  ...
  for (const st of sts) {
    const lines = (await sbGet(db, `statement_lines?statement_id=eq.${st.id}&kind=eq.booking&select=unit_name,platform,nights,amount,nett,flags`)) || [];
    for (const l of lines) {
      if (/owner/i.test(l.platform || '')) continue;                 // owner's own stay
      if ((l.flags || []).includes('zero_amount')) continue;         // family/free stays
...
export async function publishStatement(db, id, { actor = 'admin', notifyOwner = true } = {}) {
  ...
  const carry = computeCarryAdjustment(prev, existingLines);
  if (carry) {
