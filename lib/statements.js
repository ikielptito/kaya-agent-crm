  ...
  for (const st of sendable.slice(0, preview ? sendable.length : cap)) {
    ...
    for (const num of g.owner_wa_nums) {
      ...
      const mid = g.expenses_only
        ...
        : await sendStatementTemplate(WA_PHONE_ID, WA_TOKEN, to, { name: firstName, month, property: g.name, payout, tok });
      ...
      await sbPost(db, 'wa_messages', {
        ...
        campaign_id: camp?.id || null, template_name: g.expenses_only ? PROFIT_TEMPLATE : TEMPLATE_NAME, status: 'sent',
