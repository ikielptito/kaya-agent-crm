  ...
  try {
    ...
    if (Array.isArray(existing) && existing[0]) {
      ...
      await fetch(`${url}/rest/v1/owners`, {
        method: 'POST', headers,
        body: JSON.stringify({ wa_num: waNum, name, onboarding_status: 'in_conversation', opt_in: true, lang, promo_code: 'PRELAUNCH90', consent_note: `Said so in chat (${new Date().toISOString().slice(0, 10)}): ${String(evidenceQuote || '').slice(0, 200)}`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }),
      });
    }
