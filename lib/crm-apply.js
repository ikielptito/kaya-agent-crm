...
export async function applyCrmActions(url, headers, agent, actions, evidenceQuote) {
  for (const a of actions) {
    if (a.type === 'convert_to_owner') { await convertToOwner(url, headers, agent, a, evidenceQuote); continue; }
    if (a.type === 'refer_owner') { await referOwner(url, headers, agent, a, evidenceQuote); continue; }
    if (a.type === 'update_viewing') { await updateViewingFromChat(url, headers, agent, a, evidenceQuote); continue; }
...
ENGAGEMENT TIER — classify the agent's engagement level based on the conversation history and this message. Set via crm_updates when the tier changes:
  ...
  engagement_tier values (use exactly these words):
  - "active"  — actively looking for a client, asking specific availability/pricing, requesting brochures, scheduling visits
  - "warm"    — responsive and interested but not actively pushing (acknowledges info, asks casual questions, says "will keep in mind")
  - "dormant" — minimal engagement, one-word replies only, long gaps between messages, or explicitly disinterested without fully declining

Update "engagement_tier" whenever you see a clear signal. Example crm_update:
  { "field": "engagement_tier", "value": "active", "reason": "agent requesting specific availability for a client" }

Do NOT downgrade from "active" to "warm" on a single quiet message — only downgrade if the pattern is sustained. Do NOT set a tier on the very first message from a brand-new agent (wait for at least one substantive exchange).
...
OWNER LEAD — when the person says they are NOT an agent but OWN or MANAGE a villa / apartments / rooms and would like promotion, listing, tenants, or "bantu promosi": this is a villa-owner lead, the scarcer side of the marketplace. Emit { "type": "convert_to_owner", "name": "<their name>", "wa_num": "", "reason": "owns <what they described>", "service_type": "owner", "replace": false } in crm_actions, set notify_team {"to":"ikiel","summary":"Owner lead: <name>, <properties>","share_agent_contact":true}, and reply in their language: we list owner villas to our agent network, it's free during launch, and you can take the details right here in the chat — ask for the property name and area to start. Do NOT send them to a web form and do not pitch agent commissions.
OWNER REFERRAL — when the AGENT shares the contact (card or typed number) of a THIRD PARTY who OWNS or manages a villa and could list it on Samba (you asked "know an owner?", or they volunteer "my friend has a villa in Cemagi, here's his number"): emit { "type": "refer_owner", "name": "<owner's name>", "wa_num": "<digits only>", "reason": "referred by this agent as a villa owner" } — NOT create_agent (they are supply, not an agent) and NOT convert_to_owner (that is for when THIS person owns the villa). Thank the agent warmly in your reply and tell them their referral gets first look when the villa goes live. If it's ambiguous whether the shared contact is an agent or an owner, ask one short question before creating anything.
THE ONE EXCLUSION — the agent's CLIENT. This rule is about agents, colleagues, teams and partners we send LISTINGS to. It never covers the agent's own client, guest, or tenant. If a number belongs to someone the agent is renting TO (their client/guest/tenant), do NOT create it, do NOT store it, and never ask for it in the first place — that relationship is the agent's and their commission depends on it. Same for owners: an owner's tenant is not a contact. When in doubt about whose number it is, ask the agent who it belongs to rather than adding it.
...
// OWNER LEAD — the person on an agent row turns out to own or manage villas
// ("Saya bukan agen, tapi saya punya properti…", Made Suci, 23 Aug 2026).
// Until now an existing agent row could never reach owner mode (the webhook
// routes owner-mode only for numbers with no agent row), so Maya's only move
// Maya records a viewing outcome the agent reported in chat (completed /
// no-show / cancelled). Deliberately CANNOT set 'confirmed' — confirmation
// only ever comes from the villa contact's reply (lib/relay.js capture) or
// the console. Only this agent's own viewings are updatable.
export async function updateViewingFromChat(url, headers, agent, a, evidenceQuote) {
  const id = parseInt(a.viewing_id, 10);
  const status = String(a.status || '').toLowerCase().trim();
  if (!Number.isFinite(id) || !['completed', 'no_show', 'cancelled'].includes(status)) return;
  try {
    const rows = await fetch(`${url}/rest/v1/viewings?id=eq.${id}&agent_id=eq.${agent.id}&select=id,status`, { headers }).then(r => r.json());
    if (!Array.isArray(rows) || !rows[0]) return;
    await fetch(`${url}/rest/v1/viewings?id=eq.${id}`, {
      method: 'PATCH', headers,
      body: JSON.stringify({
        status,
        outcome_note: String(a.note || evidenceQuote || '').slice(0, 300) || null,
    ...
    await fetch(`${url}/rest/v1/maya_updates`, {
      method: 'POST', headers,
      body: JSON.stringify({
        agent_id: agent.id, field: 'viewing_outcome', new_value: `viewing #${id} → ${status}`,
        reason: String(a.note || '').slice(0, 200), evidence: String(evidenceQuote || '').slice(0, 500),
        by_maya: true, created_at: new Date().toISOString(),
      }),
    }).catch(() => {});
  } catch (e) { console.warn('updateViewingFromChat failed:', e.message); }
}

// An AGENT refers a third-party VILLA OWNER (shares the owner's contact card
// or number for listing purposes). Creates an owner PROSPECT row — never an
// agent row — with the referral recorded in consent_note so the intro
// template choice and Maya's pitch stay honest about how we got the number.
// The referrer is credited in maya_updates so the daily report surfaces it.
export async function referOwner(url, headers, agent, a, evidenceQuote) {
  const waNum = String(a.wa_num || '').replace(/\D/g, '');
  const name = String(a.name || '').trim().slice(0, 80);
  if (!name || waNum.length < 9 || waNum.length > 15) return;
  const referrer = `${agent.name || 'agent'} #${agent.id}`;
  const today = new Date().toISOString().slice(0, 10);
  const log = (msg) => fetch(`${url}/rest/v1/maya_updates`, {
    method: 'POST', headers,
    body: JSON.stringify({
      agent_id: agent.id, field: 'refer_owner', new_value: String(msg).slice(0, 400),
      reason: String(a.reason || 'owner referral').slice(0, 200), evidence: String(evidenceQuote || '').slice(0, 500),
      by_maya: true, created_at: new Date().toISOString(),
    }),
  }).catch(() => {});
  try {
    // Dedupe against BOTH sides: an existing owner or agent with this number
    // must not become a duplicate prospect.
    const [ownEx, agEx] = await Promise.all([
      fetch(`${url}/rest/v1/owners?wa_num=eq.${waNum}&select=id,onboarding_status`, { headers }).then(r => r.json()),
      fetch(`${url}/rest/v1/agents?wa_num=eq.${waNum}&select=id,name`, { headers }).then(r => r.json()),
    ]);
    if (Array.isArray(ownEx) && ownEx[0]) {
      await log(`SKIPPED (already owner #${ownEx[0].id}, ${ownEx[0].onboarding_status || 'synced'}): ${name} +${waNum}`);
      return;
    }
    if (Array.isArray(agEx) && agEx[0]) {
      await log(`SKIPPED (number belongs to agent #${agEx[0].id} ${agEx[0].name}): ${name} +${waNum}`);
      return;
    }
    const lang = /\b(saya|punya|villa nya|bisa|tolong|promosi|bantu)\b/i.test(String(evidenceQuote || '')) ? 'id' : 'en';
    await fetch(`${url}/rest/v1/owners`, {
      method: 'POST', headers,
      body: JSON.stringify({
        wa_num: waNum, name, onboarding_status: 'agreed', opt_in: false, lang,
        promo_code: 'FOUNDING25',
        consent_note: `Referred by ${referrer} (${today}): ${String(evidenceQuote || a.reason || '').slice(0, 200)}`,
        notes: `referral from agent #${agent.id}`,
        created_at: new Date().toISOString(), updated_at: new Date().toISOString(),
      }),
    });
    await log(`Owner prospect created: ${name} +${waNum} (referred by ${referrer})`);
  } catch (e) {
    console.warn('referOwner failed:', e.message);
  }
}

// was a web link. This creates/updates the owners row as an in-conversation
// prospect, marks the agent row as role 'owner' (the webhook then routes
// the thread to owner-mode Maya, who runs the in-chat listing intake), and
// takes them out of agent broadcasts. Logged to maya_updates.
export async function convertToOwner(url, headers, agent, a, evidenceQuote) {
  const waNum = String(agent?.wa_num || '').replace(/\D/g, '');
  if (!waNum) return;
  const name = String(a.name || agent.name || '').trim().slice(0, 80) || null;
  const lang = /\b(saya|punya|villa nya|bisa|tolong|promosi|bantu)\b/i.test(String(evidenceQuote || '')) ? 'id' : 'en';
  try {
    const existing = await fetch(`${url}/rest/v1/owners?wa_num=eq.${waNum}&select=id,onboarding_status`, { headers }).then(r => r.json());
    if (Array.isArray(existing) && existing[0]) {
      await fetch(`${url}/rest/v1/owners?id=eq.${existing[0].id}`, {
        method: 'PATCH', headers,
        body: JSON.stringify({ onboarding_status: existing[0].onboarding_status === 'listed' ? 'listed' : 'in_conversation', opt_in: true, consent_note: `Said so in chat (${new Date().toISOString().slice(0, 10)}): ${String(evidenceQuote || '').slice(0, 200)}`, lang, updated_at: new Date().toISOString() }),
      });
    } else {
      await fetch(`${url}/rest/v1/owners`, {
        method: 'POST', headers,
        body: JSON.stringify({ wa_num: waNum, name, onboarding_status: 'in_conversation', opt_in: true, lang, promo_code: 'FOUNDING25', consent_note: `Said so in chat (${new Date().toISOString().slice(0, 10)}): ${String(evidenceQuote || '').slice(0, 200)}`, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }),
      });
    }
    const eng = { ...(agent.campaign_engagement || {}), role: 'owner' };
    await fetch(`${url}/rest/v1/agents?id=eq.${agent.id}`, {
      method: 'PATCH', headers,
      body: JSON.stringify({ campaign_engagement: eng, samba_alerts_opt_out: true, automation_override: null }),
    });
    await fetch(`${url}/rest/v1/maya_updates`, {
      method: 'POST', headers,
      body: JSON.stringify({ agent_id: agent.id, field: 'convert_to_owner', new_value: 'owner prospect (in_conversation)', reason: a.reason || 'said they own/manage property', evidence: String(evidenceQuote || '').slice(0, 500), by_maya: true, created_at: new Date().toISOString() }),
    }).catch(() => {});
  } catch (e) { console.warn('convertToOwner failed:', e.message); }
}
