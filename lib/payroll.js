...
export async function syncPayroll(db, { dryRun = false, force = false } = {}) {
  ...
  for (const tab of tabs) {
    ...
    const existing = (await sbGet(db, `payroll_runs?period=eq.${period}&select=*&limit=1`))?.[0] || null;
    ...
    if (existing) {
      ...
      const ins = await sbPost(db, 'payroll_runs', { period, status: 'draft', ...fields });
...
export async function reimportRun(db, id) {
  ...
  return syncPayroll(db, { force: true });
}
...
export async function listRuns(db) {
  const runs = (await sbGet(db, 'payroll_runs?select=*&order=period.desc&limit=60')) || [];
...
export async function runDetail(db, id) {
  ...
  return {
    ...
    properties: props,
    statements: await statementComparison(db, run.period, props),
