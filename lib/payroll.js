...
export async function payrollSettings(db) {
  ...
  return {
    ...
    double8_roles: (v.double8_roles && typeof v.double8_roles === 'object') ? v.double8_roles : {},
  };
}
export async function patchPayrollSettings(db, fields = {}) {
  ...
  if (fields.double8_roles && typeof fields.double8_roles === 'object') next.double8_roles = { ...(cur.double8_roles || {}), ...fields.double8_roles };
...
// Supplies and repairs mention the same words ("Garden scissors, hose and
// napkin", "Maintenance pool machine"): without the word salary, a line only
// counts when it names a role, none of these, and is at least a month's pay.
const NOT_SALARY_RE = /maintenance|machine|fix|repair|bulb|lamp|electric|chemical|material|laundry|internet|water|scissor|hose|napkin|tool|equipment|pump|filter|net\b/i;
export function isLedgerSalaryLine(l) {
  const d = String(l.description || '');
  if (SALARY_RE.test(d)) return true;
  return ROLE_HINT_RE.test(d) && !NOT_SALARY_RE.test(d) && Number(l.amount) >= 1000000;
}
...
export async function syncDouble8(db, { dryRun = false } = {}) {
  ...
  // Who is paid for a role at Double 8: someone registered under Double 8
  // with that role; else an explicit name in settings.payroll.double8_roles
  // ({pool: 'Dian', gardener: 'Dian'}); else an island-wide Samba person
  // with that role (Dian's pool work spans both entities), or one whose
  // villas include a B unit. A person can serve both entities; the money
  // stays on the Double 8 side because the run does.
  const allStaff = ((await listStaff(db)) || []).filter(x => x.active !== false);
  const roleMap = ((await getSettingValue(db, SETTINGS_KEY)) || {}).double8_roles || {};
  const norm = (v) => String(v || '').toLowerCase().trim();
  const staffForRole = (role) => {
    if (!role) return null;
    const d8 = allStaff.find(x => x.entity === 'double8' && (x.roles || []).includes(role));
    if (d8) return d8;
    if (roleMap[role]) {
      const named = allStaff.find(x => norm(x.name) === norm(roleMap[role]) || String(x.id) === String(roleMap[role]));
      // A confirmed name not yet on the register (Krisna) still names the payee.
      return named || { id: null, name: String(roleMap[role]), unregistered: true };
    }
    return allStaff.find(x => (x.roles || []).includes(role) && (!(x.slugs || []).length || (x.slugs || []).some(sl => slugsAll.includes(sl)))) || null;
  };
  ...
  for (const st of stmts) {
    ...
    const lines = raw.filter(isLedgerSalaryLine).map((l, i) => {
      ...
      const person = staffForRole(role);
      return fullLine({
        ...
        flags: person && !person.unregistered ? [] : ['not_in_registry'], source_row: l.id, position: i,
      });
    });
