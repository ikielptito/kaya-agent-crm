import { isNoiseCaption } from './staff-lang.js';
...
export function looksLikeMaintenance(text, hasImage) {
  const t = String(text || '');
  // "Ini unit 1 ya buk" under a photo names the villa; it reports nothing.
  if (isNoiseCaption(t)) return false;
...
// Second chance for a message the vocabulary gate did not recognise.
//
// The gate is a word list, and a word list can only ever know the phrasings
// somebody thought of. This is the backstop: when a message from someone in
// the staff register has been claimed by nobody, one cheap model call decides
// whether it was a report before it is dropped. Costs a fraction of a cent on
// the small number of messages that reach it, and removes the whole class of
// bug where Maya silently ignores a leak because it was worded unusually.
//
// Answers false when the model is unavailable: the message still reaches a
// human either way, so the honest failure is to stay quiet rather than to
// invent a work order.
export async function couldBeMaintenance(text, hasImage) {
  const raw = String(text || '').trim();
  if (!raw && !hasImage) return false;
  if (raw.length < 6 && !hasImage) return false;
  const out = await askClaude(
`A villa housekeeper or handyman sent this message to the property manager${hasImage ? ' along with a photo' : ''}:

"${raw}"

Is she or he reporting something at a villa that is broken, damaged, dirty,
leaking, not working, or otherwise needs fixing or attention?

Ordinary conversation is NOT a report: greetings, confirming they are on the
way, saying a job is finished, asking about pay or schedule, small talk.

Reply with ONLY: {"report": true} or {"report": false}`, 60);
...
export async function extractReports(text, { matched, hasImage, lang = 'en' } = {}) {
  const raw = String(text || '').trim();
  const fallback = [{
  ...
  const out = await askClaude(
`${lang === 'id' ? 'A housekeeper' : 'A villa manager'} in Bali sent this maintenance report about ${matched?.unit_label || matched?.group?.name || 'a managed villa'}${hasImage ? ' (with photos attached)' : ''}. It may be written in Indonesian.

"${raw}"

If the text does not describe anything broken, dirty, missing, leaking or needing work — it only names the unit, greets, thanks, or says all is fine — reply {"items": []}. Never invent a job from a photo caption that names a place.
If the text says the work is ALREADY finished ("has been repaired", "replaced yesterday", "sudah diperbaiki", "selesai diganti", "this has been followed up"), it is a status update about an existing job, not a report: reply {"items": []}.

Otherwise split it into separate work orders ONLY if it clearly describes distinct jobs (for example a door repair AND replacing chairs). Otherwise return exactly one.

Translate faithfully, and keep the original words in the description so nothing is lost: "belakang pintu" is BEHIND the door (not the back door); "leking"/"bocor" is a leak only when water is involved — if the message says a mark, stain or handprint, say that. When unsure what a word means, quote it in the title rather than guess.
...
Use "urgent" only for things that stop a guest staying or risk damage (flooding, no water, no power, broken lock, aircon dead in an occupied unit).`, 900);
  ...
  // The model saying "nothing here" is an answer; the fallback is only for
  // when the model could not be asked at all.
  if (isNoiseCaption(raw)) return [];
  const items = Array.isArray(out?.items) ? out.items.filter(i => i && i.title) : null;
...
export async function extractReport(text, opts) {
  return (await extractReports(text, opts))[0] || null;
}
...
export async function parseCleaningReply(text, { villa, tasks = [] } = {}) {
  const raw = String(text || '').trim();
  if (!raw) return { intent: 'none' };

  const today = witaToday();
  const list = (tasks || []).map(t => `  [${t.id}] ${t.villa} — ${t.kind || 'clean'}, ${t.date}`).join('\n');
  const out = await askClaude(
`Today is ${today} (Bali, WITA). A housekeeper was asked to clean ${villa || 'a villa'} today.
${list ? `Her open tasks (id, villa, kind, date):\n${list}\n` : ''}
She replied (Indonesian or English): "${raw}"

Reply with ONLY a JSON object:
{"intent": "move" | "cannot" | "done" | "correction" | "note",
 "task_id": the id of the task she is talking about, or null when unclear,
...
"done" = she has already done it ("A4 sudah saya bersihkan hari ini" → done, task_id = A4's).
"correction" = she says her previous message was about a DIFFERENT villa
  ("bukan A4, tetapi B4" → correction, task_id = B4's task).
"note" = anything else, including a question or a statement about her
  weekly schedule.
Pick task_id by the villa she names (A4 = Tropicana Valley - Unit A4, "unit 3"
= the Unit 3 of the property she works at). When she names no villa, use the
task for ${villa || 'today'}.`, 300);

  if (!out?.intent) return { intent: 'note', note: raw, task_id: null };
  const intent = ['move', 'cannot', 'done', 'correction', 'note'].includes(out.intent) ? out.intent : 'note';
  ...
  return { intent, date, task_id, summary: out.summary ? String(out.summary).slice(0, 120) : null, note: raw };
}
