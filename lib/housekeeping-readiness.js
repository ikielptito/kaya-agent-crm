...
import { isDone as isDoneWord, isRestock, realText } from './staff-lang.js';
...
// Logged to wa_messages like every other reply since 7 Sep 2026, so the
// staff console shows the photo ask and the verdict, not just her photos.
let _logDb = null;
  ...
  try {
    ...
    if (_logDb) {
      await fetch(`${_logDb.SUPABASE_URL}/rest/v1/wa_messages`, {
        ...
        body: JSON.stringify({ wa_num: String(to).replace(/\D/g, ''), direction: 'outbound', content: body, wa_message_id: mid, timestamp: new Date().toISOString(), source: 'webhook', category: 'housekeeping', status: r.ok ? 'sent' : 'failed' }),
      }).catch(() => {});
    }
...
export async function openReadiness(db, { task, person, villa }) {
  ...
  const text = `Terima kasih! Sebelum tamu datang, tolong kirim foto ${villa}:\n${spots}\n\nKalau ada yang hampir habis (sabun, tisu, air galon), tulis saja di sini. Setelah semua foto terkirim, balas "selesai".`;
  // The row is null when a check for this task already existed (a second
  // "done" tap): the ask is still worth repeating, the record is not new.
  return { id: rec?.id || null, text, toString: () => text };
}

// ── Her photos and her "selesai" ────────────────────────────────────
// Claims ONLY what a check can use: a photo, the word that closes it, and
// a line about supplies. Everything else falls through to the dispatcher —
// until 8 Sep 2026 this handler took every message for 24 hours, so a
// question about the oven and a report of a leak were both filed as
// "restock" and answered "kirim fotonya juga ya".
// A check is open until the guest is in (or 20 hours), not a flat day.
export async function handleReadiness({ db, wa, fromNum, text, mediaType, mediaId, waToken, waMessageId = null, pinned = null, asRestock = false }) {
  ...
  const open = pinned || ((await sbGet(db,
    ...
    + `&select=*&order=asked_at.desc&limit=3`)) || []).find(c => !c.guest_in_date || c.guest_in_date >= today);
  ...
  if (hasImage) {
    try {
      ...
      if (media?.base64) {
        ...
        // Read-back loop: a burst of seven photos across seven lambdas.
        let photos = [];
        for (let attempt = 0; attempt < 4; attempt++) {
          const cur = (await sbGet(db, `housekeeping_readiness?id=eq.${open.id}&select=photos&limit=1`))?.[0];
          if ((cur?.photos || []).includes(path)) { photos = cur.photos; break; }
          photos = [...(cur?.photos || []), path];
          ...
          const after = (await sbGet(db, `housekeeping_readiness?id=eq.${open.id}&select=photos&limit=1`))?.[0];
          if ((after?.photos || []).includes(path)) { photos = after.photos; break; }
        }
        await photoStore.remember(db, { path, wamid: waMessageId, waNum: fromNum, staffId: person.id, caption: body, source: 'readiness', status: 'attached', readinessId: open.id, taskId: open.task_id || null });
        if (open.task_id) await hkEvent(db, open.task_id, 'photo', { actor: person.name, payload: { path, readiness_id: open.id }, wamid: waMessageId });
        if (photos.length === 1) {
    ...
    if (body && isRestock(body)) await noteRestock(db, open, body);
    return true;
  }

  // Plain text while a check is open: only what is running low is hers to
  // keep. Anything else is somebody else's message.
  if (!asRestock && !isRestock(body)) return false;
  await noteRestock(db, open, body);
  await sendText(wa, fromNum, `Dicatat, saya sampaikan ke Era 🙏 Kirim fotonya juga ya, lalu balas "selesai".`);
  return true;
}
// A remark said to the check that is not about supplies: kept on it, so Era
// sees it with the flags.
export async function noteOnCheck(db, rec, line) { return noteRestock(db, rec, line); }
...
async function closeReadiness(db, wa, { rec, person, fromNum, villa, waToken }) {
  ...
  // The model did not answer (no key, an outage, unreadable JSON). That is
  // not a pass: the record says nobody looked, and Era hears so.
  const unverified = photos.length > 0 && !verdict;
  ...
  if (unverified) flags.push('photo check unavailable — please look at the photos yourself');

  const status = unverified ? 'unverified' : flags.length ? 'flagged' : 'pass';
  ...
  if (fresh.task_id) {
    ...
    await hkEvent(db, fresh.task_id, 'readiness_closed', { actor: person?.name || null, payload: { status, photos: photos.length, flags } });
  }
  await closeAsksFor(db, 'housekeeping_readiness', fresh.id, { status });
...
export async function readinessSweep({ db, wa, now = new Date() } = {}) {
  ...
  for (const rec of stale) {
    // Runs hourly now. A check for tomorrow's arrival is closed the evening
    // before (from 18:00), so Era can still call; one for a later arrival
    // waits for the timeout; one for today's arrival closes at once.
    if (rec.asked_at > cutoff) {
      if (!rec.guest_in_date) continue;
      if (rec.guest_in_date > plusDays(today, 1)) continue;
      if (rec.guest_in_date === plusDays(today, 1) && hour < 18) continue;
    }
...
export async function housekeeperStats(db, { days = 30 } = {}) {
  ...
  for (const c of checks) {
    ...
    else if (c.status === 'unchecked' || c.status === 'unverified') b.unchecked++;
  }
