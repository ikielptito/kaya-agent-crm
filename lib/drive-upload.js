...
export async function createOwnerFolder(folderName, parentId = process.env.DRIVE_PARENT_FOLDER_ID) {
...
export async function findOwnerFolderByName(folderName, parentId = process.env.DRIVE_PARENT_FOLDER_ID) {
  const token = await getAccessToken();
...
// The images sitting in a folder, with when they arrived — the owner's root
// folder is the photo INBOX (see lib/owner-photos.js), and routing decides by
// arrival time which villa a photo belongs to.
export async function listInboxImages(folderId) {
  const token = await getAccessToken();
  ...
  do {
    const url = `https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}`
      + `&fields=nextPageToken,files(id,name,createdTime)&pageSize=1000&supportsAllDrives=true&includeItemsFromAllDrives=true`
      + (pageToken ? `&pageToken=${pageToken}` : '');
    const r = await fetch(url, { headers: { Authorization: `Bearer ${token}` } });
    const d = await r.json();
    if (!r.ok) throw new Error(`Drive list failed: ${d?.error?.message || r.status}`);
    for (const f of d.files || []) out.push({ id: f.id, name: f.name, createdTime: f.createdTime });
    pageToken = d.nextPageToken || '';
  } while (pageToken);
  return out;
}

// Move one file into another folder (Drive: add the new parent, drop the old).
export async function moveDriveFile(fileId, toFolderId) {
  const token = await getAccessToken();
  const cur = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=parents&supportsAllDrives=true`, { headers: { Authorization: `Bearer ${token}` } });
  const cd = await cur.json();
  if (!cur.ok) throw new Error(`Drive file lookup failed: ${cd?.error?.message || cur.status}`);
  const remove = (cd.parents || []).filter(p => p !== toFolderId).join(',');
  const r = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?addParents=${toFolderId}${remove ? `&removeParents=${remove}` : ''}&supportsAllDrives=true`, {
    method: 'PATCH', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: '{}',
  });
  if (!r.ok) { const d = await r.json().catch(() => ({})); throw new Error(`Drive move failed: ${d?.error?.message || r.status}`); }
  return true;
}

// Move a file or folder to Drive's trash. Deliberately NOT a hard delete:
// trashed items are restorable for 30 days, so a mistaken cleanup is undoable.
export async function trashDriveFile(fileId) {
  ...
  if (!r.ok) {
    ...
    throw new Error(`Drive trash failed: ${d?.error?.message || r.status}`);
  }
  return true;
}
