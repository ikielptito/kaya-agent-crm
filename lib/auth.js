...
//   'finance' — the books app's key (FINANCE_SECRET): the project-finance
//             tables, read and write, and nothing else.
...
export function consoleScope(req) {
  ...
  // The Tropicana Valley books app (its own Vercel project, Ikiel and Oli)
  // holds FINANCE_SECRET: the finance_* actions on /api/statements and
  // nothing else — see financeActionAllowed.
  const fin = process.env.FINANCE_SECRET;
  if (fin && safeEq(hdr, fin)) return 'finance';
  return null;
}
...
// The books app may only touch the project-finance tables.
export function financeActionAllowed(action) {
  return /^finance_[a-z_]+$/.test(String(action || ''));
}
