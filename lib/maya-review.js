...
export async function applyDecisions(env, { approve = [], reject = [], answers = {}, staff_approve = [], staff_reject = [] } = {}) {
  ...
  if ((staff_approve.length || staff_reject.length) && pending.staff) {
    try {
      ...
      staff = await applyStaffDecisions({ SUPABASE_URL: env.SUPABASE_URL, sbHeaders: env.headers }, pending.staff, { approve: staff_approve });
  ...
  for (const l of approvedLessons) {
    const entry = { id: l.id, category: l.category, rule: l.rule, evidence: l.evidence, added_at: nowIso };
