...
const MODEL = 'claude-sonnet-4-6';        // lesson consolidation (cheap, high-volume)
const CRITIC_MODEL = 'claude-opus-5';     // the weekly analysis itself — worth Opus
// Per-token USD rates (input / output / cache-read / cache-write 5m) by model.
const RATES = {
  'claude-sonnet-4-6': { in: 3, out: 15, cr: 0.30, cw: 3.75 },
  'claude-opus-5':     { in: 5, out: 25, cr: 0.50, cw: 6.25 },
};
function costOfUsage(u, model = MODEL) {
  if (!u) return 0;
  const r = RATES[model] || RATES[MODEL];
  return (u.input_tokens || 0) / 1e6 * r.in
    + (u.output_tokens || 0) / 1e6 * r.out
    + (u.cache_read_input_tokens || 0) / 1e6 * r.cr
    + (u.cache_creation_input_tokens || 0) / 1e6 * r.cw;
}
...
// routing_lessons are about the Haiku/Sonnet ROUTER, not Maya's reply voice, so
// they live in their own bucket and are NEVER rendered into her reply prompt —
// they're guidance for tightening the router's threshold.
const EMPTY_PLAYBOOK = { version: 0, updated_at: null, lessons: [], facts: [], routing_lessons: [] };
export async function getPlaybook(env) {
  ...
  return { version: pb.version || 0, updated_at: pb.updated_at || null,
    ...
    routing_lessons: Array.isArray(pb.routing_lessons) ? pb.routing_lessons : [] };
}
...
// Manual (human) sends were first tagged source='manual' on 2026-07-06. Before
// that, Ikiel's manual takeover replies defaulted to source='api' — identical to
// Maya's, so the critic can't tell them apart. Never grade a window that reaches
// back past this: pre-cutoff, an "IKIEL" line is indistinguishable from a "MAYA"
// line and would produce false-attribution lessons.
const MANUAL_TAGGING_CUTOFF = Date.parse('2026-07-06T00:00:00Z');
...
async function gatherWeek(env, days = 7) {
  const since = new Date(Math.max(Date.now() - days * 86400000, MANUAL_TAGGING_CUTOFF)).toISOString();
  ...
  const [agents, msgs] = await Promise.all([
    ...
    q(`wa_messages?timestamp=gte.${since}&select=agent_id,direction,content,source,category,template_name,model,timestamp&order=timestamp.asc&limit=4000`),
  ]);
  ...
  for (const m of M) {
    ...
    // Only Maya's OWN outbound counts as a reply to grade. Ikiel's manual
    // takeover sends (source='manual') are context, not Maya's work.
    if (m.direction === 'outbound' && m.source !== 'manual') t.mayaReplies++;
  }
  ...
  const threads = [...byAgent.entries()]
    ...
    .map(([id, t]) => {
      ...
      const lines = tail.map(m => {
        const who = m.direction === 'inbound' ? 'AGENT' : (m.source === 'manual' ? 'IKIEL' : 'MAYA');
        // Tag each Maya reply with the model that wrote it so the critic can
        // audit the Haiku/Sonnet routing (which cheap replies missed the mark).
        const tag = (who === 'MAYA' && m.model) ? `[${m.model.includes('haiku') ? 'haiku' : 'sonnet'}]` : '';
        return `${who}${tag}: ${clean(m.content)}`;
      });
...
async function critique(env, { threads, kbContext, playbook }) {
  ...
  const existingRouting = (playbook.routing_lessons || []).map((l, i) => `${i + 1}. ${l.rule}`).join('\n') || '(none yet)';
...
You are given (1) Maya's operating rules (persona), (2) the live knowledge base she is supposed to quote from, (3) her current LEARNED PLAYBOOK of already-known lessons, and (4) transcripts of the week's conversations. Transcript speakers: AGENT = the property agent (inbound), MAYA = Maya's automated reply, IKIEL = Ikiel himself, sent manually when he took over the thread. Each MAYA line is tagged with the model that wrote it: MAYA[haiku] = a cheaper, smaller model the router picked for a message it judged trivial, MAYA[sonnet] = the stronger default. Untagged MAYA lines predate routing — treat them as sonnet.

CRITICAL — grade ONLY the lines labelled MAYA. Lines labelled IKIEL are Ikiel's own manual messages: they are CONTEXT so the conversation reads correctly, but they are NOT Maya's work. Never grade an IKIEL line, never attribute its content, price, commission or promise to Maya, and never derive a lesson or a scoreboard issue from one. If a commission was negotiated or a deal was struck on an IKIEL line, that was Ikiel's decision, not an escalation failure by Maya.
...
- ESCALATION: Maya answered something she was required to escalate (negotiation, discounts, holds/reservations, complaints, legal, "speak to Ikiel", visit confirmation), OR escalated something trivial she should have handled. ALSO — and this is the single most damaging failure in the whole rubric — any reply where Maya went AROUND the agent: asked for their client's name or contact details, offered to have Era/Ikiel/"the team" follow up with their client directly, or otherwise treated the agent's client as ours to deal with. Flag every single instance as high severity, even one, and never let it be excused by "she needed to check a rate".
...
- RAPPORT: Maya pushed against a cooling or disengaging agent instead of easing off — kept pitching, stacked more questions or brochure buttons after a low-engagement signal ("we already have that", one-word replies, "not now"), or failed to offer a graceful exit. The clearest tell is an agent who goes from neutral to annoyed or asks to be left alone: treat any reply that plausibly CAUSED that turn (a pushy question, a re-pitch, a hard sell on someone winding down) as high severity. Reading the temperature and giving room is part of the job.
- ROUTING: a MAYA[haiku] reply that fell short BECAUSE it needed more capability — a fumbled nuance, a missed escalation, a shallow or subtly-wrong answer that MAYA[sonnet] would likely have handled. This is the signal the Haiku threshold is too loose. A routing lesson must name the MESSAGE PATTERN that should NOT have gone to Haiku (e.g. "route any message mentioning a specific move-in date to Sonnet"), so the router can tighten. Do NOT flag a MAYA[haiku] reply that was perfectly fine — clean Haiku replies prove the routing is working and need no lesson.
...
Respond with ONLY a JSON object (no markdown):
{
  "scoreboard": {
    "grade": "A" | "B" | "C" | "D" | "F",
    "issues_found": <int>,
    "by_category": { "accuracy": <int>, "escalation": <int>, "voice": <int>, "leak": <int>, "lane": <int>, "etiquette": <int>, "unanswered": <int>, "rapport": <int>, "routing": <int> },
    "headline": "<one sentence: the single most important takeaway this week>"
  },
  "lessons": [
    { "category": "accuracy|escalation|voice|leak|lane|etiquette|unanswered|rapport|routing",
...
ROUTING RULES ALREADY APPLIED (messages matching these already go to Sonnet — do not re-propose a "routing" lesson for a pattern already covered here):
${existingRouting}
...
Produce the review JSON now.`;
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    // Opus 5 with adaptive thinking. 'medium' effort keeps this weekly call fast
    // enough for the cron's 60s budget while still giving strong analysis; the
    // token cap has to cover thinking + the JSON, hence the larger max_tokens.
    body: JSON.stringify({
      model: CRITIC_MODEL,
      max_tokens: 16000,
  ...
  await chargeSpend(env, costOfUsage(d.usage, CRITIC_MODEL));

  let parsed;
  try {
    // With thinking on, content[0] is a thinking block — grab the text block.
    const textBlock = (d.content || []).find(b => b.type === 'text');
    const txt = (textBlock?.text || '').trim().replace(/^```(json)?/i, '').replace(/```$/, '').trim();
...
export async function runReview(env, { days = 7, kbContext = '', preview = false } = {}) {
  ...
  // The staff lane (staff-review.js): the funnel from what the housekeepers
  // said to what got recorded, and proposals over the misses. Its failure
  // never blocks the sales review.
  let staff = null;
  try { const { stageStaffReview } = await import('./staff-review.js'); staff = await stageStaffReview(env, { days, preview }); }
  catch (e) { staff = { error: e.message, proposals: [], metrics: null }; }
  if (gathered.thread_count === 0) {
    const empty = { week_of: witaDate(0), generated_at: new Date().toISOString(),
      ...
      lessons: [], questions: [], staff, decided: false };
...
// edits: { id: text } — Ikiel's rewording of a lesson's rule (or of a staff
// proposal's word / example / tip) before it is applied. The edited text is
// what gets taught; the original stays in the pending review as evidence.
export async function applyDecisions(env, { approve = [], reject = [], answers = {}, staff_approve = [], staff_reject = [], edits = {} } = {}) {
  const pending = await getSetting(env, 'maya_review_pending', null);
  if (!pending) return { error: 'no pending review to apply' };
  // The staff lane's approvals become settings the dispatcher reads.
  let staff = null;
  if ((staff_approve.length || staff_reject.length) && pending.staff) {
    try {
      const { applyStaffDecisions } = await import('./staff-review.js');
      staff = await applyStaffDecisions({ SUPABASE_URL: env.SUPABASE_URL, sbHeaders: env.headers }, pending.staff, { approve: staff_approve, edits });
      pending.staff.approved = [...(pending.staff.approved || []), ...staff_approve];
      pending.staff.rejected = [...(pending.staff.rejected || []), ...staff_reject];
  ...
  for (const l of approvedLessons) {
    const edited = edits && typeof edits[l.id] === 'string' && edits[l.id].trim() ? clean(edits[l.id]) : null;
    if (edited) l.edited_rule = edited;
    const entry = { id: l.id, category: l.category, rule: edited || l.rule, evidence: l.evidence, added_at: nowIso, ...(edited ? { original_rule: l.rule } : {}) };
    // Routing lessons steer the router, not Maya's replies — keep them out of
    // the reply playbook so they never reach her live prompt.
    if (l.category === 'routing') playbook.routing_lessons.push(entry);
    else playbook.lessons.push(entry);
  }
  ...
  log.unshift({
    ...
    staff_approved: staff_approve.length,
  });
  ...
  return {
    ...
    playbook_facts: playbook.facts.length, consolidated, staff,
  };
}
...
// ── Live KB context for the critic ───────────────────────────────────────────
// The ground truth the critic cross-checks Maya's replies against. Mirrors what
// Maya herself is fed (whatsapp-webhook buildRentalsContext) so correct prices
// are never flagged as hallucinations. Moved here from cron-followups + fixed:
// the rentals select previously used non-existent columns (monthly/yearly/
// bedrooms/available) so EVERY rental price rendered blank and the critic marked
// Maya's correct figures as invented. Real columns: monthly_rate_idr /
// yearly_rate_idr / beds.
export async function buildReviewKbContext(url, headers) {
  ...
  const [projects, rentals] = await Promise.all([
    ...
    q('rentals?select=name,area,beds,monthly_rate_idr,yearly_rate_idr,maya_notes&active=eq.true&order=display_order.asc'),
  ]);
  ...
  const idrM = (n) => (n ? `IDR ${(n / 1e6).toFixed(0)}M` : null);
  const R = Array.isArray(rentals) ? rentals : [];
  const samba = R.map(r => {
    const beds = r.beds ? `${r.beds}BR` : null;
    const price = [r.monthly_rate_idr ? `${idrM(r.monthly_rate_idr)}/mo` : null,
                   r.yearly_rate_idr ? `${idrM(r.yearly_rate_idr)}/yr` : null].filter(Boolean).join(', ') || 'rate TBC';
    return `- ${r.name}${r.area ? ' (' + r.area + ')' : ''}${beds ? ' · ' + beds : ''}: ${price}${r.maya_notes ? ' — ' + String(r.maya_notes).slice(0, 160) : ''}`;
  }).join('\n');

  return `KAYA SALES PROJECTS (unit-level source of truth — prices/availability Maya must match):
${kaya || '(none loaded)'}

SAMBA REALTY RENTALS (monthly/yearly IDR; 10% agent commission already included in the quoted rate):
${samba || '(none loaded)'}`;
}
