...
// A short text with one button that opens a URL (interactive cta_url —
// no template, no approval). Falls back to the URL in the text.
export async function sendCtaUrl(wa, to, body, { text = 'Open', url }) {
  const t = String(body || '').trim();
  if (!url || !t || t.length > LIMITS.body) return sendText(wa, to, `${t}\n${url}`);
  const mid = await post(wa, { to, type: 'interactive', interactive: { type: 'cta_url', body: { text: t }, action: { name: 'cta_url', parameters: { display_text: clip(text, LIMITS.buttonTitle), url } } } });
  return mid || sendText(wa, to, `${t}\n${url}`);
}

// An image by URL with a caption. Returns the message id, or null.
export async function sendImage(wa, to, url, caption = '') {
  if (!url) return null;
  return post(wa, { to, type: 'image', image: { link: url, ...(caption ? { caption: clip(caption, 1024) } : {}) } });
}
