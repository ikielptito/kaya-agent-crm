...
export async function pullAgentAnalytics(env) {
  ...
  await fetch(`${env.SUPABASE_URL}/rest/v1/settings`, {
    ...
    body: JSON.stringify({ key: 'agent_portal_stats', value: {
      ...
      // Per-handle share attribution for the current month — mapped to CRM
      // agents via campaign_engagement.portal_account for the monthly shoutout.
      attribution_month: data.attribution_month || { views: {}, wa: {} },
    } })
  });
...
export async function syncOwners(env) {
  ...
  for (const row of rows) {
    ...
    // An owners row is keyed by PHONE, so name it after whoever answers that
    // phone — the contact name on the listing — before the Google-account name
    // of whoever owns it. Era is the contact on Ikiel's villas, so preferring
    // the account name labelled her row "Ikiel Ptito" and the weekly report
    // greeted her as Ikiel. Report rows carry reportContactName here too.
    if (!o.name && (row.waContactName || row.ownerName)) o.name = row.waContactName || row.ownerName;
