...
export async function buildRecords(db, { from, to, slugs = null, limit = 1500, audience = 'era' } = {}) {
  ...
  for (const jobsAll of visits.values()) {
    ...
    // A rule-skipped job is not a job (the owner never sees it); if every
    // job of the day was rule-skipped, there was no visit.
    const real = jobs.filter(j => !(j.vs === 'skipped' && RULE_SKIP.test(j.task.notes || '')));
    if (!real.length) { if (owner) continue; }
    const shown = real.length ? real : jobs;
    ...
    const rec = {
      ...
      done_at: x.done_at || null, photo_count: photoSet.size, evidence: evidenceOf({ ...x, photos: [...(x.photos || []), ...(check?.photos || [])] }),
      guest_in_date: x.guest_in_date || null, guest_out_date: x.guest_out_date || null, same_day: !!x.same_day,
      moved: !!x.moved_by,
      jobs: shown.map(j => ({ task_id: j.task.id, kind: j.task.kind, status: j.vs, evidence: evidenceOf(j.task), photo_count: (j.task.photos || []).length, round_id: roundByTask.get(j.task.id)?.id || null, check_id: checkByTask.get(j.task.id)?.id || null })),
    };
    if (!owner) {
