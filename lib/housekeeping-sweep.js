...
export async function runHousekeepingSweep({
  ...
  const due = (await sbGet(db,
    `housekeeping_tasks?task_date=eq.${today}&notified_at=is.null&status=eq.planned`
    + `&select=*,staff:assigned_staff_id(id,name,wa_num,active)&order=same_day.desc,slug.asc&limit=60`)) || [];
