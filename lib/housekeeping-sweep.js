...
export async function runHousekeepingSweep({
  ...
  for (const task of due) {
    ...
    if (!to || !task.staff?.active) {
      ...
      await markUncovered(task, 'no housekeeper assigned');
      continue;
    }
    if (modeOf(task.staff) === 'dead') {
      out.skipped.push({ id: task.id, slug: task.slug, why: `${task.staff.name}'s phone is not receiving` });
      await markUncovered(task, `${task.staff.name}'s phone has not received messages since ${String(chan[task.staff.id]?.undelivered_since || '').slice(0, 10)}`);
    ...
    if (!preview) {
      ...
      await noteAsk(task, to, mid, 'task');
    }
    out.tasks_sent++;
  }
  ...
  if (has(tInspect)) {
    for (const task of due.filter(x => x.kind === 'inspection')) {
      const to = String(task.staff?.wa_num || '').replace(/\D/g, '');
      if (!to || !task.staff?.active) { out.skipped.push({ id: task.id, slug: task.slug, why: 'nobody covers this villa' }); await markUncovered(task, 'no housekeeper assigned'); continue; }
      const mode = modeOf(task.staff);
      if (mode === 'dead') { out.skipped.push({ id: task.id, slug: task.slug, why: `${task.staff.name}'s phone is not receiving` }); await markUncovered(task, `${task.staff.name}'s phone is not receiving`); deadTold.add(task.staff.id); continue; }
      // A reader who ignores asks is not sent a photo round by template;
      // Era arranges it by phone, and the brief says so.
      if (mode === 'quiet') { out.skipped.push({ id: task.id, slug: task.slug, why: `${task.staff.name} ignores asks — arrange the round by phone` }); await markUncovered(task, `${task.staff.name} has not been answering; Era to arrange by phone`); continue; }
      const mid = await send(to, tInspect, [name(task.slug)],
        `[Inspection round — ${name(task.slug)}]`);
      if (!mid) continue;
      if (!preview) {
        await logOut(db, { waNum: to, mid, template: tInspect, campaignId: camp?.id,
          content: `[Inspection round — ${name(task.slug)}]` });
        await sbPatch(db, `housekeeping_tasks?id=eq.${task.id}`, {
          notified_at: nowIso(), status: 'notified', updated_at: nowIso(),
        });
        await noteAsk(task, to, mid, 'inspection');
      }
