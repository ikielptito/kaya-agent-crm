...
{"intent": "done" | "move" | "cannot" | "off" | "finding" | "ticket_update" | "restock" | "schedule" | "question" | "photo_correction" | "work_note" | "other",
...
- photo_correction: photos she ALREADY sent belong to a different villa ("yang saya kirim tadi itu foto B6", "foto tadi salah, itu B3"). Fill slug with the villa the photos belong to.
...
export async function dispatchStaffMessage(ctx) {
  ...
  if (ref?.kind === 'which' && ref.ask && !hasImage && body) {
    ...
    // "b3 dan b5 sudah selesai" names both: each is closed (10 Sep 2026).
    if (hit.length) {
      let any = false;
      for (const h of hit) any = (await intake.handleCleaningReply({ ...ctx, text: body, pinned: h, preParsed: { intent: ref.ask.payload?.verb === 'done' ? 'done' : 'note', task_id: h.id } })) || any;
      if (any) { await answerAsk(db, ref.ask.id, { text: body }, { wamid: waMessageId }); return 'housekeeping'; }
    }
  }
  ...
  if (!hasImage) {
    ...
    // "Yang saya kirim ke dua itu foto di B6": photos already sent belong
    // to another villa. Moved, not noted (9 Sep 2026).
    {
      const { parsePhotoCorrection, handlePhotoCorrection } = await import('./photo-correction.js');
      const pc = parsePhotoCorrection(body, person.slugs || [], names);
      if (pc) {
        trace(ctx, { layer: 'rule', intent: 'photo_correction', detail: { ...(ctx.trace?.detail || {}), slug: pc.slug, which: pc.which } });
        if (await handlePhotoCorrection({ db, person, body, names, today: work.today, slug: pc.slug, which: pc.which, say: (t) => say(db, wa, fromNum, t), tellEra: (l) => tellEra(db, wa, l) })) return 'housekeeping';
      }
    }
  }
  if (hasImage) {
  ...
  // "Tugas bersihkan kamar B3 dan B5 sudah selesai": a done that names her
  // open visits closes them, before the completion guard can read it
  // against a ticket (Gede's B3/B5 landed on the B5 countertop ticket,
  // 10 Sep 2026).
  {
    const { namedDone } = await import('./named-done.js');
    const named = namedDone(body, work.cleans.filter(t => t.task_date <= work.today), person.slugs || [], names);
    if (named.length) {
      trace(ctx, { layer: 'rule', intent: 'done', detail: { ...(ctx.trace?.detail || {}), named: named.map(t => t.slug) } });
      for (const t of named) await intake.handleCleaningReply({ ...ctx, text: body, pinned: t, preParsed: { intent: 'done', task_id: t.id } });
      for (const a of (work.asks || []).filter(a => a.kind === 'which')) await answerAsk(db, a.id, { text: body.slice(0, 200) }, { wamid: waMessageId }).catch(() => {});
      return 'housekeeping';
    }
  }
  ...
  if (cls && cls.confidence === 'high') {
    ...
    if (cls.intent === 'photo_correction' && cls.slug && (person.slugs || []).includes(cls.slug)) {
      const { handlePhotoCorrection } = await import('./photo-correction.js');
      if (await handlePhotoCorrection({ db, person, body, names, today: work.today, slug: cls.slug, which: 'last', say: (t) => say(db, wa, fromNum, t), tellEra: (l) => tellEra(db, wa, l) })) return 'housekeeping';
    }
