...
import { isAck, isDone, isAllFine, isGreeting, isQuestion, isAvail, isRestock, SCHEDULE_WORD_RE, realText } from './staff-lang.js';
...
export async function openWork(db, person, { today = witaToday() } = {}) {
  ...
  return { tasks, cleans, round, check, recent, asks, today };
}
...
export async function classifyStaffText({ apiKey = process.env.ANTHROPIC_API_KEY, body, person, work, names = {} }) {
...
{"intent": "done" | "move" | "cannot" | "off" | "finding" | "restock" | "schedule" | "question" | "work_note" | "other",
...
- finding: something at the villa is broken, dirty, missing, leaking, mouldy, needs repair or replacement — a report, not a question.
...
Pick task_id by the villa she names; when she names none and only one visit is open today, use it. When she names a villa not on her list, still fill slug with your best reading. Use "low" confidence whenever the villa or the intent is not clear from her words.`;
  try {
    ...
    return { intent: String(out.intent), task_id, slug: out.slug ? String(out.slug) : null, date, summary: out.summary ? String(out.summary).slice(0, 120) : null, confidence: out.confidence === 'low' ? 'low' : 'high' };
  } catch { return null; }
}
...
export async function routePhoto(ctx, { ref, work, body }) {
  ...
  // Nothing of hers is open: a report with the villa to be named.
  if (await handleStaffMaintenance({ ...ctx, text: body, staffSlugs: staffSlugs || person.slugs || [], force: !!body })) return 'maintenance_staff';
  return null;
}
...
export async function dispatchStaffMessage(ctx) {
  ...
  if (!hasImage) {
    ...
    try { const { handleSelfIntro } = await import('./staff-onboarding.js'); if (await handleSelfIntro({ db, wa, fromNum, text: body, person })) return 'staff_onboard'; } catch { /* optional */ }
    if (isGreeting(body)) { if (await schedule.handleGreeting({ db, wa, fromNum, text: body })) return 'housekeeping'; }
    if (isAck(body)) return 'staff_ack';
  }
  if (hasImage) {
  ...
  if (SCHEDULE_WORD_RE.test(body) || (schedule.looksLikeSchedule(body) && _resolve && _resolve.resolveUnits(body, person.slugs || []).length)) {
    if (await schedule.handleScheduleStatement({ db, wa, fromNum, text: body, waMessageId, person })) return 'housekeeping_schedule';
  }

  // ── Layer 3: the classifier ───────────────────────────────────────
  const cls = await classifyStaffText({ apiKey, body, person, work, names }).catch(() => null);
  const { handleStaffMaintenance } = await import('./maintenance-staff.js');
