...
async function runTool(name, input, ctx) {
  ...
  if (name === 'create_agent') {
    ...
    // Mirror the webhook's proven-working new-agent insert: conversation_summary
    // + conversation_history + unread_count are NOT-NULL on the base table, so
    // an insert that omits them fails the constraint. Enroll in Samba too, so
    // the new contact starts receiving Maya's regular updates.
    const dayStr = new Date().toISOString().slice(0, 10);
    const r = await fetch(`${url}/rest/v1/agents`, {
      method: 'POST',
      headers: { ...headers, 'Prefer': 'return=representation' },
      body: JSON.stringify({
        name: input.name, wa_num: wa,
        wa_url: `https://wa.me/${wa}`,
        agency: input.agency || null,
        unread_count: 0,
        notes: input.notes || 'Added via Maya assistant console',
        conversation_summary: `[${dayStr}] Added via Maya assistant console${input.reason ? ' — ' + input.reason : ''}.`,
        conversation_history: { first_contact: dayStr, last_contact: dayStr, total_messages: 0 },
        campaign_engagement: { samba: { status: 'enrolled', source: 'assistant_console' } },
      })
    });
