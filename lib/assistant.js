...
const TOOLS = [
  ...
  },
  {
    ...
    description: 'Full-text search across all WhatsApp messages. Returns matching messages plus the distinct agents involved. Use to answer "who did we already tell about X" (direction=outbound) or "who asked about Y" (direction=inbound), then diff against list_agents for the "not yet" set.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        limit: { type: 'number', description: 'Max matches returned (default 80)' }
      },
      required: ['text']
    }
  },
  ...
  },
  {
    ...
    description: 'Prepare (NOT send) a broadcast message to multiple agents. Returns the 24h-window split. The user must confirm in the app before anything is sent — after calling this, tell them the draft is ready and awaiting their confirmation. Calling it again replaces the previous draft.',
    input_schema: {
      type: 'object',
      properties: {
        message: { type: 'string', description: 'Exact text every recipient gets (no merge fields)' },
      ...
      required: ['message', 'agent_ids', 'label']
    }
  }
];
...
- Point agents at the portal for photos/listings: https://sambarentals.vercel.app
- Broadcast copy is sent verbatim to everyone — no {{name}} merge fields, so write it to work without a name.
...
- WhatsApp only delivers free-form text within 24 hours of an agent's last inbound message. draft_broadcast reports the in/out-of-window split — out-of-window agents will be SKIPPED at send time. Always tell Ikiel this split and who misses out.
...
- "Who haven't we told about X yet" → search_messages (text=X, direction=outbound) for who HAS heard, list_agents (enrolled_samba_only) for the audience, then list the difference by name.
...
async function runTool(name, input, ctx) {
  ...
  if (name === 'search_messages') {
    ...
    let q = `${url}/rest/v1/wa_messages?content=ilike.${pat}&timestamp=gte.${since}&select=agent_id,direction,content,timestamp&order=timestamp.desc&limit=${limit}`;
    if (input.direction) q += `&direction=eq.${input.direction}`;
    const rows = await (await fetch(q, { headers })).json();
    if (!Array.isArray(rows)) return { error: 'search failed' };
    const ids = [...new Set(rows.map(m => m.agent_id).filter(x => x != null))];
    let names = {};
    if (ids.length) {
    ...
    ctx.activity.push(`Searched messages for "${input.text}"`);
    return {
      total_matches: rows.length,
      agents_matched: ids.map(id => ({ id, name: names[id] || ('#' + id) })),
      matches: rows.slice(0, 40).map(m => ({
        agent_id: m.agent_id, agent: names[m.agent_id] || null,
        direction: m.direction, at: m.timestamp, text: (m.content || '').slice(0, 200)
      }))
    };
  }
  ...
  if (name === 'get_rentals') {
    ...
    return {
      rentals: rows.map(r => ({
        id: r.id, name: r.name, area: r.area, type: r.property_type, beds: r.beds,
  ...
  if (name === 'draft_broadcast') {
    ...
    if (!message || !ids.length) return { error: 'message and agent_ids required' };
    ...
    ctx.pendingBroadcast = {
      draft_id: draft.id, label: draft.label, message: draft.message,
      recipients, excluded
    };
    const inW = recipients.filter(r => r.in_window).length;
    ctx.activity.push(`Drafted broadcast "${draft.label}" (${inW} deliverable)`);
    return {
      draft_id: draft.id,
      status: 'DRAFT ONLY — nothing sent. The user sees a confirmation card and must tap Send.',
      recipients_total: recipients.length,
      in_window_now: inW,
      out_of_window_skipped_at_send: recipients.length - inW,
      excluded
    };
  }
...
export async function handleExecuteBroadcast(req, res, { SUPABASE_URL, headers }) {
  ...
  for (const rec of draft.recipients || []) {
    ...
    const r = await sendWaText(env, { id: rec.id, wa_num: rec.wa_num }, draft.message, 'broadcast');
