...
const TOOLS = [
  ...
  {
    name: 'comms_metrics',
    description: 'The communications scoreboard: outbound volume by category, delivery/read rates (from status tracking), reply rate, opt-out count, engagement-tier split, and per-template quality ratings. Use for "how are our messages doing", "read rate", "is anyone opting out", health checks.',
    input_schema: {
      type: 'object',
...
async function runTool(name, input, ctx) {
  ...
  if (name === 'comms_metrics') {
    ...
    const [msgRows, agRows, tpls] = await Promise.all([
      (await fetch(`${url}/rest/v1/wa_messages?timestamp=gte.${since}&direction=eq.outbound&select=category,status,source&limit=10000`, { headers })).json(),
    ...
    (Array.isArray(msgRows) ? msgRows : []).forEach(m => {
      ...
      if (m.status) { withStatus++; if (m.status === 'read') read++; else if (m.status === 'delivered') delivered++; else if (m.status === 'failed') failed++; }
    });
