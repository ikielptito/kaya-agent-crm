...
import { heroImageForSlug } from './wa-carousel.js';
import { baseAgentFields, createAgentRow } from './agents.js';
import { pickWelcomeTemplate, isWithinWitaHours } from './kb.js';
import { listPendingFacts, resolveFact } from './relay.js';
import { tierSplit } from './tiers.js';
...
const TOOLS = [
  ...
  {
    name: 'update_agent',
    description: 'Update fields on an agent record. Allowed fields: notes, engagement_tier (champion/active/new/warm/dormant), contact_frequency (normal/weekly/monthly/paused), samba_alerts_opt_out (boolean), automation_override (null/off/draft/hybrid/autopilot/paused), name, agency. Logged to the audit trail.',
    input_schema: {
      type: 'object',
      properties: {
        agent_id: { type: 'number' },
  ...
  {
    name: 'create_agent',
    description: 'Add a new agent to the CRM (enrolled in Samba). Deduplicates by WhatsApp number.',
    input_schema: {
      type: 'object',
      properties: {
        name: { type: 'string' },
        ...
        agency: { type: 'string' },
        notes: { type: 'string' },
        service_type: { type: 'string', enum: ['rental', 'leasehold', 'both'], description: "What this contact handles: 'rental' (monthly rentals), 'leasehold' (property sales only, no rental alerts), or 'both'." },
        reason: { type: 'string', description: 'Why this contact is being added (for the audit log).' }
      },
  ...
  {
    name: 'comms_metrics',
    description: 'The communications scoreboard: outbound volume by category, delivery/read rates (from status tracking), read rate BY FORMAT (carousel vs alert vs price-drop, keyed by template), reply rate, opt-out count, engagement-tier split, and per-template quality ratings. Use for "how are our messages doing", "read rate", "which format performs best", "is anyone opting out", health checks.',
    input_schema: {
      type: 'object',
  ...
  {
    name: 'list_pending_facts',
    description: "Villa facts Maya learned by relaying an agent's question to a listing's contact (e.g. \"Unit 1 has a bathtub in the master ensuite\"), waiting for Ikiel's approval before they go into the listing's data and get quoted to the whole agent network. The agent who asked already got their answer — this queue is only about what Maya keeps. Shows the original question, the contact's answer, and the fact proposed from it.",
    input_schema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'resolve_fact',
    description: "Approve or reject a pending villa fact from list_pending_facts. Approving appends it to that listing's extended details, so Maya can answer the same question herself next time without asking the villa again. Rejecting simply drops it. Always show Ikiel the fact and get an explicit yes before calling this.",
    input_schema: {
      type: 'object',
      properties: {
        relay_id: { type: 'number', description: 'The id from list_pending_facts' },
        approve: { type: 'boolean', description: 'true to write it into the listing, false to discard' }
      },
      required: ['relay_id', 'approve']
    }
  },
  {
    name: 'handbook',
    description: 'Read one section of the Samba Handbook by key: the owner and housekeeper guides, the pitch and legal pages, the cockpit how-to essays, the statement glossary, the unit catalog, the SOP, and how the portal and the CRM are wired (system.portal, system.crm). Use it for any "how does X work / what does it cost / where is Y" question instead of guessing. Call with no key to list the keys.',
    input_schema: { type: 'object', properties: { key: { type: 'string', description: 'A section key, e.g. system.crm, owner.guide, legal.refund' } } }
  },
  {
    name: 'system_map',
    description: 'How the system is wired, generated from the code at the last handbook build: routes (every API file and its actions), crons (with Bali times), settings (keys the CRM reads), env (variable names per repo), links (signed-link helpers). Use for "which cron/route/setting does X" questions.',
...
async function fetchApprovedTemplates(env) {
  ...
  const r = await fetch(`${GRAPH}/${wabaId}/message_templates?fields=name,status,category,language,components,quality_score&limit=100`, {
    headers: { 'Authorization': 'Bearer ' + env.TOKEN }
  });
  const d = await r.json();
...
async function sendWaTemplate(env, rec, tpl) {
  ...
  const components = [];
  // Image-header templates (v3 strategic) lead with the villa's hero photo.
  if (tpl.imageHeader) components.push({ type: 'header', parameters: [{ type: 'image', image: { link: tpl.imageHeader } }] });
  ...
  if (waMessageId) {
    await fetch(env.SUPABASE_URL + '/rest/v1/wa_messages', {
      ...
      body: JSON.stringify({
        ...
        source: 'api', category: 'broadcast', status: 'sent', template_name: tpl.name
      })
...
4. Clear the villa-fact approval queue (list_pending_facts / resolve_fact). When an agent asks something you don't know about a villa, you relay it to that listing's contact and pass their answer back — and anything permanent you learn waits here for Ikiel's yes before it goes into the listing and gets quoted to the whole network. Mention the queue when he asks what needs him, keep each one to a line (listing, what was asked, what came back), and never approve without an explicit yes.
...
- TWO DELIVERY MODES. Free-text ("message") only reaches agents inside the 24h window since their last inbound; everyone else is skipped. Template mode ("template_name" + "template_params") uses a Meta-approved template and reaches EVERYONE. Check the in-window split from draft_broadcast: if a meaningful share of the audience is out of window (or the audience is cold), use a template — check list_templates first. Strategic templates (each: {{1}}=recipient first name via the {name} token, {{2}}=villa, {{3}}=area, {{4}}=one detail line):
  - samba_new_listing_v3 — announce a villa just added to the portfolio
  - samba_price_update_v3 — price drop / rate change ({{4}} like "now 38jt/month, down from 40jt/month")
  - samba_villa_spotlight_v3 — promote/push a specific villa
  The v3 templates lead with a LARGE hero photo of the villa (premium look) plus a "View listing" button — always pass listing_slug (it drives both the photo and the button). Prefer v3; the older v2 versions are text-only fallbacks. If a v3 isn't approved yet (check list_templates), use the v2 equivalent and mention the photo version is pending.
...
ANSWER PATTERNS:
- Listings can carry a manual badge from the portal admin ("Price drop", "New"). When a villa has one, lead with it — it is the marketing hook Ikiel chose. "Which villas have badges" → get_rentals and filter.
...
// Repair unpaired UTF-16 surrogates (broken emoji halves, common in WhatsApp text).
// Anthropic's strict JSON parser rejects lone surrogates with
// "no low surrogate in string", failing the whole request.
...
async function callClaude(key, body) {
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify(deepWellFormed(body))
  });
  const data = await r.json();
...
async function runTool(name, input, ctx) {
  ...
  if (name === 'system_map') {
    const sec = handbookSection(`system.${input?.topic}`, 'system');
    return sec || { error: 'topic must be routes, crons, settings, env or links' };
  }
  if (name === 'handbook') {
    if (!input?.key) return { sections: handbookIndex('system') };
    const sec = handbookSection(input.key, 'system');
    return sec || { error: `no section "${input.key}"`, sections: handbookIndex('system').map(x => x.key) };
  }
  ...
  if (name === 'agent_activity') {
    ...
    const [msgRows, agRows] = await Promise.all([
      sbRows(url, headers, `wa_messages?timestamp=gte.${since}&select=agent_id,direction,timestamp&order=id.asc`),
  ...
  if (name === 'get_rentals') {
    ...
    // The portal (sambarentals.com) is the source of truth agents actually see;
    // the rentals table is a cache that can drift. Overlay the portal's LIVE
    // monthly/yearly rate so Maya never quotes a price different from what the
    // agent lands on. Matched by slug (portal uses hyphens, DB uses underscores).
    let portalBySlug = {};
    try {
      let listings = await (await fetch(`${PORTAL_ORIGIN}/api/listings`)).json();
      if (!Array.isArray(listings)) listings = listings.listings || [];
      listings.forEach(l => { if (l.slug) portalBySlug[l.slug] = l; });
    } catch (_) { /* fall back to DB rates */ }
    return {
      rate_source: Object.keys(portalBySlug).length ? 'live portal (sambarentals.com)' : 'database cache (portal unreachable)',
      rentals: rows.map(r => {
        ...
        const slug = slugFromPortalUrl(r.portal_url) || r.slug;
        const p = portalBySlug[slug];
        return {
          id: r.id, name: r.name, slug,
          listing_url: listingUrl(slug),
          area: r.area, type: r.property_type, beds: r.beds,
          baths: r.baths, max_guests: r.max_guests,
          // Portal rate wins (live); DB is fallback. `monthly` is the portal's
          // display string (e.g. "35jt"); quote it verbatim.
          monthly: p?.monthly || (r.monthly_rate_idr ? `${Math.round(r.monthly_rate_idr / 1e6)}jt` : null),
          yearly: p?.yearly || (r.yearly_rate_idr ? `${Math.round(r.yearly_rate_idr / 1e6)}jt` : null),
          // Manual marketing badge set in the portal admin ("Price drop", "New").
          badge: p?.badge || r.badge || null,
  ...
  if (name === 'create_agent') {
    ...
    // Reliable, self-healing insert (shared with the webhook). Enrolls in Samba
    // so the new contact starts receiving Maya's updates immediately.
    const fields = baseAgentFields({
      name: input.name, waNum: wa, agency: input.agency || null, notes: input.notes || null,
      source: 'assistant_console', reason: input.reason || 'added via assistant console',
      serviceType: input.service_type || null,
    });
    // Outside 9am-9pm WITA, defer the welcome to the 9am cron (same quiet-hours
    // rule as the CRM quick-add): flag it on the row, don't send now.
    ...
    const created = await createAgentRow(url, headers, fields);
    if (!created.ok) return { error: 'insert failed: ' + created.error };
    ...
    if (!deferWelcome && row?.id && ctx.env?.TOKEN && ctx.env?.PHONE_ID) {
      try {
        const templates = await fetchApprovedTemplates(ctx.env);
        // fetchApprovedTemplates already filters to APPROVED (and drops status),
        // so don't re-require it here.
        const welcomeTpl = pickWelcomeTemplate(templates, { requireApproved: false });
        if (welcomeTpl) {
          const wRes = await sendWaTemplate(ctx.env, { id: row.id, wa_num: wa, name: input.name }, {
            name: welcomeTpl.name, language: welcomeTpl.language || 'en',
            body: welcomeTpl.body, params: ['{name}'],
          });
          welcome_sent = !!wRes.ok;
        }
      } catch (e) { console.warn('welcome template send failed:', e.message); }
    }
    return { success: true, agent_id: row?.id || null, welcome_sent, welcome_deferred: deferWelcome };
  }
  ...
  if (name === 'comms_metrics') {
    const days = Math.min(Math.max(input.days || 30, 1), 90);
    const since = new Date(Date.now() - days * 86400000).toISOString();
    // Outbound messages (with status), agents (tiers/opt-out), templates (quality).
    const [msgRows, agRows, tpls] = await Promise.all([
      sbRows(url, headers, `wa_messages?timestamp=gte.${since}&direction=eq.outbound&select=category,template_name,status,source&order=id.asc`),
      (await fetch(`${url}/rest/v1/agents?select=engagement_tier,samba_alerts_opt_out,contact_frequency,campaign_engagement,is_test`, { headers })).json(),
      fetchApprovedTemplates(ctx.env).catch(() => null)
    ]);
    const inbound = await sbRows(url, headers, `wa_messages?timestamp=gte.${since}&direction=eq.inbound&select=agent_id&order=id.asc`);
    const byCat = {}; let withStatus = 0, delivered = 0, read = 0, failed = 0;
    // Per-format read rate: bucket by template_name (falls back to category) so
    // we can compare carousel vs alert vs price-drop performance.
    const fmt = {};
    (Array.isArray(msgRows) ? msgRows : []).forEach(m => {
      const c = m.category || 'other'; byCat[c] = (byCat[c] || 0) + 1;
      const key = m.template_name || m.category || 'free-text';
      const f = fmt[key] || (fmt[key] = { sent: 0, with_status: 0, read: 0 });
      f.sent++;
      if (m.status) {
        withStatus++; f.with_status++;
        if (m.status === 'read') { read++; f.read++; }
        else if (m.status === 'delivered') delivered++;
        else if (m.status === 'failed') failed++;
      }
    });
    const by_format = Object.entries(fmt)
      .sort((a, b) => b[1].sent - a[1].sent)
      .map(([k, v]) => ({ format: k, sent: v.sent, tracked: v.with_status, read_rate: v.with_status ? `${Math.round(v.read / v.with_status * 100)}%` : 'n/a' }));
    const enrolled = (Array.isArray(agRows) ? agRows : []).filter(a => !a.is_test && a.campaign_engagement && a.campaign_engagement.samba);
    let optedOut = 0;
    const split = tierSplit(enrolled);
    enrolled.forEach(a => { if (a.samba_alerts_opt_out) optedOut++; });
    const repliedAgents = new Set((Array.isArray(inbound) ? inbound : []).map(m => m.agent_id).filter(Boolean));
    const totalOut = Array.isArray(msgRows) ? msgRows.length : 0;
    ctx.activity.push(`Pulled the comms scoreboard (${days}d)`);
    return {
      window_days: days,
      outbound_total: totalOut,
      outbound_per_enrolled_agent: enrolled.length ? +(totalOut / enrolled.length).toFixed(1) : null,
      by_category: byCat,
      delivery_tracking: {
        note: withStatus < totalOut * 0.5 ? 'Status tracking was fixed 10 Jul — historical sends before that lack status; rates below are only over messages that carry a status.' : 'ok',
        messages_with_status: withStatus,
        read_rate: withStatus ? `${Math.round(read / withStatus * 100)}%` : 'n/a',
        delivered_or_read_rate: withStatus ? `${Math.round((delivered + read) / withStatus * 100)}%` : 'n/a',
        failed
      },
      read_rate_by_format: by_format,
      reply_rate: enrolled.length ? `${Math.round(repliedAgents.size / enrolled.length * 100)}% of enrolled replied` : 'n/a',
      opt_out: { count: optedOut, of_enrolled: enrolled.length, rate: enrolled.length ? `${(optedOut / enrolled.length * 100).toFixed(1)}%` : 'n/a' },
      engagement_tiers: { ...split.tiers, introduced_no_reply: split.introduced, introduced_stalled: split.stalled, line: split.line },
      template_quality: Array.isArray(tpls) ? tpls.filter(t => t.category === 'MARKETING').map(t => ({ name: t.name, quality: t.quality || 'unknown' })) : 'unavailable'
    };
  }
  ...
  if (name === 'draft_broadcast') {
    ...
    if (input.template_name) {
      ...
      // Image-header templates lead with the villa's portal cover photo — resolve
      // it from the slug so every recipient sees the hero image.
      let imageHeader = null;
      if (t.image_header) {
        if (!buttonSlug) return { error: `template "${t.name}" shows a villa photo — pass listing_slug so I can attach the right hero image.` };
        imageHeader = await heroImageForSlug(buttonSlug);
        if (!imageHeader) return { error: `no portal cover photo found for "${buttonSlug}" — check the slug against get_rentals.` };
      }
      template = { name: t.name, language: t.language, body: t.body, params, buttonSlug, imageHeader,
  ...
  if (name === 'list_pending_facts') {
    const rows = await listPendingFacts({ SUPABASE_URL: url, sbHeaders: headers }, Math.min(input.limit || 25, 50));
    ctx.activity.push(`Checked the villa-fact approval queue (${rows.length} pending)`);
    return {
      pending: rows.map(r => ({
        relay_id: r.id,
        listing: r.property_name || r.rental_slug,
        slug: r.rental_slug,
        agent_asked: r.question,
        contact_answered: r.answer,
        proposed_fact: r.kb_fact,
        confirmed_by: r.contact_name,
        answered_at: r.answered_at,
      })),
      note: rows.length
        ? 'Approving writes the fact into that listing\'s extended details, so Maya answers it herself next time instead of asking the villa again.'
        : 'Nothing waiting — every fact Maya learned has been reviewed.',
    };
  }

  if (name === 'resolve_fact') {
    const out = await resolveFact({ SUPABASE_URL: url, sbHeaders: headers }, input.relay_id, !!input.approve);
    if (!out.ok) return { error: out.error };
    ctx.activity.push(input.approve ? `Approved a villa fact for ${out.slug}` : 'Rejected a villa fact');
    return input.approve
      ? { applied: true, slug: out.slug, added: out.fact }
      : { applied: false, result: 'fact discarded; the listing is unchanged' };
  }
...
export async function handleAssistant(req, res, { SUPABASE_URL, headers }) {
  ...
  const system = buildSystemPrompt() + `\n\n${handbookDigest('system')}\n\nFor how-does-it-work questions about Samba or the CRM, read the handbook (tool "handbook") before answering; never guess a route, a price or a policy.`;
  let llmCalls = 0;
  let finalText = '';
