...
async function runTool(name, input, ctx) {
  ...
  if (name === 'comms_metrics') {
    ...
    const [msgRows, agRows, tpls] = await Promise.all([
      (await fetch(`${url}/rest/v1/wa_messages?timestamp=gte.${since}&direction=eq.outbound&select=category,template_name,status,source&limit=10000`, { headers })).json(),
    ...
    const inbound = await (await fetch(`${url}/rest/v1/wa_messages?timestamp=gte.${since}&direction=eq.inbound&select=agent_id&limit=10000`, { headers })).json();
