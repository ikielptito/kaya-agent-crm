...
import { isDone as isDoneWord, isAck, isQuestion, isAvail, isAllFine, isNoiseCaption, realText as realTextShared } from './staff-lang.js';
...
async function closeTask(db, wa, { task, person, fromNum, villa, wamid = null }) {
  ...
  // "Sudah selesai" minutes after the morning message is an acknowledgement
  // pressed on the wrong button. Recorded as done all the same (she said
  // so), but she hears once how the button is meant.
  if (task.notified_at && Date.now() - Date.parse(task.notified_at) < 15 * 60e3 && task.kind !== 'inspection') {
    try { const { coach } = await import('./coaching.js'); await coach(db, wa, { person, fromNum, key: 'early_done' }); } catch { /* optional */ }
  }
...
export async function handleInspection({ db, wa, fromNum, text, mediaType, mediaId, waToken, waMessageId = null, replyTo = null, buttonPayload = null, pinned = null, mode = null, date: wantedDate = null }) {
  ...
  let raised = [];
  // "Ini unit 1 ya buk" under a photo is a label, not a finding: it goes
  // on neither the owner's report nor a ticket.
  const said = body && !isNoiseCaption(body) ? body : '';
  const firstText = !!said && !insp.findings;
  if (said) {
    await sbPatch(db, `housekeeping_inspections?id=eq.${insp.id}`, {
      findings: [insp.findings, said].filter(Boolean).join(' · ').slice(0, 1000),
    });
  }

  // Inside a round the photo already has its home; only the WORDS can
  // raise a ticket, by vocabulary, never the photo's mere presence.
  if (said && looksLikeMaintenance(said, false)) {
    ...
    if (matched?.group_key) {
      ...
      // A fault seen again on this round that already has a ticket is a
      // note on that ticket, not a second one (the guard's overlap rule).
      let openHere = [];
      try { const { openTicketsFor } = await import('./ticket-guard.js'); openHere = await openTicketsFor(db, { slug: open.slug }); } catch { openHere = []; }
      const { similarOpen } = await import('./ticket-guard.js');
      for (const rep of reports.slice(0, 3)) {
        const dup = openHere.length ? similarOpen(openHere, { title: rep.title, text: rep.description || '', slug: open.slug }) : null;
        if (dup) {
          await appendThread(db, dup.item.id, { who: person.name, text: `Seen again during the inspection of ${today}: ${String(rep.title).slice(0, 120)}` });
          try { const { maintEvent } = await import('./events.js'); await maintEvent(db, dup.item.id, 'reported_again', { actor: person.name, payload: { source: 'inspection', inspection_id: insp.id, title: rep.title }, wamid: waMessageId }); } catch { /* optional */ }
          if (!raised.includes(dup.item.id)) raised.push(dup.item.id);
          continue;
        }
