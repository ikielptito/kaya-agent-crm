...
export async function handleCleaningReply({ db, wa, fromNum, text }) {
  ...
  if (/^\s*(sudah|selesai|beres|done|udah|kelar|siap)\b[\s.!👍✅]*$/i.test(body)) {
  ...
  const parsed = await parseCleaningReply(body, { villa });
  ...
  if (parsed.intent === 'move' && parsed.date && parsed.date !== open.task_date) {
    ...
    await notifyEra(db, wa, `${person.name} moved the ${open.kind.replace('_', ' ')} at ${villa} to ${parsed.date}: "${body}"`);
    return true;
  }
  ...
  if (parsed.intent === 'cannot') {
    ...
    await sbPatch(db, `housekeeping_tasks?id=eq.${open.id}`, {
      notes: [open.notes, body].filter(Boolean).join(' · ').slice(0, 500), updated_at: nowIso(),
    });
    await sendText(wa, fromNum, `Baik, saya kabari Era ya. Terima kasih sudah memberi tahu.`);
    await notifyEra(db, wa, `${person.name} cannot do the ${open.kind.replace('_', ' ')} at ${villa}: "${body}"`);
    return true;
  }
