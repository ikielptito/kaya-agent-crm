...
async function closeTask(db, wa, { task, person, fromNum, villa }) {
  await sbPatch(db, `housekeeping_tasks?id=eq.${task.id}`, {
  ...
  try {
    ...
    if (ask) { await sendText(wa, fromNum, ask); return; }
  } catch { /* the thank-you below still goes out */ }
  await sendText(wa, fromNum, `Terima kasih, ${villa} sudah dicatat selesai.`);
}

export async function handleCleaningReply({ db, wa, fromNum, text, buttonPayload = null, replyTo = null, tapOnly = false }) {
  ...
  const tasks = await openTasksFor(db, person, { today });
  if (!tasks.length) return false;
  const names = await (await import('./housekeeping.js')).catalogNames(db).catch(() => ({}));
  const nameOf = (slug) => names[slug] || slug;

  // Which task: the one named by the message she tapped, else the oldest.
  let open = tasks[0];
  if (tapped && replyTo) {
  ...
  if (tapped === 'done' || /^\s*(sudah|selesai|beres|done|udah|kelar|siap)\b[\s.!👍✅]*$/i.test(body)) {
    await closeTask(db, wa, { task: open, person, fromNum, villa: nameOf(open.slug) });
    return true;
  }
  ...
  let parsed;
  if (tapped === 'tomorrow') parsed = { intent: 'move', date: plusDays(today, 1), task_id: open.id };
...
// A reply about her availability rather than the villa.
const AVAIL_RE = /\b(libur|cuti|sakit|izin|tidak bisa|gak bisa|ga bisa|nggak bisa|besok|lusa|minggu depan|hari senin|hari selasa|hari rabu|hari kamis|hari jumat|hari sabtu|hari minggu|day off|off today|can'?t today|cannot today|not today)\b/i;

export async function handleInspection({ db, wa, fromNum, text, mediaType, mediaId, waToken, waMessageId = null, replyTo = null, buttonPayload = null }) {
  ...
  if (!hasImage && AVAIL_RE.test(body) && body.length < 120) {
    const villa = await villaName(db, open.slug);
    let date = null;
    try { const { parseCleaningReply } = await import('./maintenance-intake.js'); date = (await parseCleaningReply(body, { villa })).date || null; } catch { /* default below */ }
    ...
    await sbPatch(db, `housekeeping_tasks?id=eq.${open.id}`, {
      ...
      thread: [...(open.thread || []), { at: nowIso(), who: person.name, text: body }].slice(-50),
    });
    ...
    if (matched?.group_key) {
      ...
      for (const rep of reports.slice(0, 3)) {
        ...
        if (item?.id) {
          ...
          if (certain) await attachPhotoPaths(db, item.id, [certain]).catch(() => {});
        }
      }
      if (raised.length) {
        await sbPatch(db, `housekeeping_inspections?id=eq.${insp.id}`, {
