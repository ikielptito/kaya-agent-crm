 ...
 "unmatched":["<line>"]}` }],
  ...
  try {
    ...
    for (const a of (out.actions || [])) {
      ...
      actions.push({ id: Number(a.id), action: a.action, note: String(a.note || '').slice(0, 300), estimated_cost: a.estimated_cost != null ? Number(a.estimated_cost) || null : null, until: /^\d{4}-\d{2}-\d{2}$/.test(String(a.until || '')) ? a.until : null });
    }
