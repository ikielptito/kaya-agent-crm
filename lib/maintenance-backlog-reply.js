...
import { looksLikeCompletion } from './ticket-guard.js';
...
export async function handleBacklogReply({ db, wa, fromNum, text, who = 'Era', apiKey }) {
  ...
  // Finished work that matched no ticket is the guard's case, not this
  // lane's: it answers with "File as new / Log as done / Ignore" buttons
  // rather than the "could not match" line, and it never files.
  if (!parsed.actions.length && !HAS_IDS.test(body) && looksLikeCompletion(body)) return false;
