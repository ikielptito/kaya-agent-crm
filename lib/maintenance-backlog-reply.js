...
import { sendButtons, parseTap } from './wa-interactive.js';
import { looksLikeCompletion, similarOpen } from './ticket-guard.js';
...
const PROGRESS = /\b(done|finished|selesai|sudah|replaced|repaired|fixed|bought|installed|waiting|menunggu|estimate|estimasi|price|harga|pending|in progress|scheduled|tomorrow|besok|next week|not yet|belum)\b/i;

// A message that says work is already finished is a status reply whenever
// it arrives, nudge or no nudge (10 Sep 2026: "Tropicana-a4 sofa zipper has
// been repaired" came 22 hours after the last nudge's window and was filed
// as a new ticket). looksLikeCompletion is the same test the ticket guard
// uses, so the two lanes agree.
export function looksLikeStatusReply(text, { nudgedRecently = false, open = null } = {}) {
  const t = String(text || '').trim();
  if (!t || t.length < 4) return false;
  if (HAS_IDS.test(t)) return true;
  if (looksLikeCompletion(t)) return true;
  // The words are an open ticket's own words ("A4 sofa zipper: tukang comes
  // Friday, 150k"): news about it, whatever the verb. The parser below
  // still decides which ticket, by villa and subject.
  if (open?.length && similarOpen(open, { text: t })?.score >= 1) return true;
...
Today is ${new Date(Date.now() + 8 * 3600e3).toISOString().slice(0, 10)} (Bali). Dates she gives without a year are in the coming weeks, never in the past.
...
A line that names a "#number" which is NOT in the list goes to "unmatched" — never re-map it to another ticket, even if the words seem to fit.

{"actions":[{"id":<ticket id>,"line":"<the line of her reply this comes from>","action":"done|estimate|waiting|note","note":"<her words, short, English or Indonesian as written>","estimated_cost":<number or null>,"until":"<YYYY-MM-DD or null>"}],
 "unmatched":["<line>"]}` }],
    }),
  });
  if (!r.ok) return null;
  ...
  try {
    ...
    const unmatched = (out.unmatched || []).map(String);
    const actions = [];
    for (const a of (out.actions || [])) {
      if (!ids.has(Number(a.id)) || !['done', 'estimate', 'waiting', 'note'].includes(a.action)) continue;
      // The hard rule the model is also told: an explicit "#n" in her line
      // is the ticket, full stop. "#4 done" must never close #6 because
      // #6's title mentions unit 4.
      const said = String(a.line || '').match(/#\s?(\d{1,5})\b/);
      if (said && Number(said[1]) !== Number(a.id)) { unmatched.push(String(a.line)); continue; }
      // A date in the past is a mis-read year ("12 Sept" → last year); drop
      // it rather than snooze a ticket to a day that has already gone.
      const today = new Date(Date.now() + 8 * 3600e3).toISOString().slice(0, 10);
      let until = /^\d{4}-\d{2}-\d{2}$/.test(String(a.until || '')) ? a.until : null;
      if (until && until < today) { const bumped = String(Number(until.slice(0, 4)) + 1) + until.slice(4); until = bumped >= today && bumped <= new Date(Date.now() + 200 * 86400e3).toISOString().slice(0, 10) ? bumped : null; }
      actions.push({ id: Number(a.id), action: a.action, note: String(a.note || '').slice(0, 300), estimated_cost: a.estimated_cost != null ? Number(a.estimated_cost) || null : null, until });
    }
    return { actions, unmatched: [...new Set(unmatched)].slice(0, 10) };
  } catch { return null; }
}
  ...
  for (const a of actions) {
    ...
    try {
      if (a.action === 'done') {
        ...
        if (it.status === 'new') {
          await publishItem(db, it.id, { requires_approval: false, actor: who });
          // The "finished" notice IS the owner's first word of it; the done
          // queue only speaks to owners who were told, so stamp it.
          await fetch(`${db.SUPABASE_URL}/rest/v1/maintenance_items?id=eq.${it.id}`, { method: 'PATCH', headers: db.sbHeaders, body: JSON.stringify({ notified_at: nowIso() }) }).catch(() => {});
        }
        if (it.status === 'pending_approval') { lines.push(`⏳ ${label} → the owner has not answered yet; withdraw or wait on the page`); continue; }
        ...
        // A price inside a remark ("tukang comes Friday, 150k") is still
        // the estimate; a note that carries one records it.
        if (a.estimated_cost && !it.estimated_cost) await patchItem(db, it.id, { estimated_cost: a.estimated_cost });
        await appendThread(db, it.id, { who, text: a.note });
        lines.push(`📝 ${label} → noted: ${a.note}${a.estimated_cost && !it.estimated_cost ? ` (estimate IDR ${a.estimated_cost.toLocaleString('en-US')})` : ''}`);
      }
...
export async function handleBacklogReply({ db, wa, fromNum, text, who = 'Era', apiKey }) {
  ...
  if (!looksLikeStatusReply(body, { nudgedRecently: nudged, open })) return false;
  ...
  // Nothing matched and no ticket number was named: not this lane's to
  // answer. A completion goes to the ticket guard (File as new / Log as
  // done / Ignore, nothing filed); a report about another villa ("A5 sofa
  // zipper broken" while A4's is open) goes on to be filed there. Only an
  // explicit "#n" that matched nothing is answered here.
  if (!parsed.actions.length && !HAS_IDS.test(body)) return false;
  ...
  // Up to three Undo buttons, one per applied ticket; more than that and
  // the typed form is the only way, so say so.
  const applied = parsed.actions.filter(a => lines.some(l => l.includes(`#${a.id} `) && !l.startsWith('⚠️')));
  const undoIds = [...new Set(applied.map(a => a.id))];
  const hint = undoIds.length > 3 ? `\n\nIf I read any of that wrong, reply "undo #<number>".` : (undoIds.length ? '' : '');
  const reply = lines.length
    ? `Got it, ${who}. Applied:\n${lines.join('\n')}${unmatched}${hint}`
    : `Thanks ${who}, I could not match that to an open ticket.${unmatched}`;
  if (undoIds.length && undoIds.length <= 3) await sendButtons(wa, fromNum, reply, undoIds.map(id => ({ id: `mt:undo:${id}`, title: `Undo #${id}` })));
  else await sendText(wa, fromNum, reply);
...
export async function handleBacklogUndo({ db, wa, fromNum, text, buttonPayload = null }) {
  const tap = parseTap(buttonPayload);
  const m = tap?.domain === 'mt' && tap.verb === 'undo' ? [null, tap.id] : String(text || '').trim().match(/^undo\s*#?\s?(\d{1,5})\b/i);
  if (!m) return false;
  const { reopenItem } = await import('./maintenance.js');
  try { const r = await reopenItem(db, Number(m[1]), { by: 'Era (undo)' }); await sendText(wa, fromNum, r.already ? `#${m[1]} was not closed (it is ${r.status}).` : `Reopened #${m[1]} — back to ${r.status}.`); }
