...
export async function handleBacklogReply({ db, wa, fromNum, text, who = 'Era', apiKey }) {
  ...
  // Read back first, apply on Yes (Ikiel's rule, 10 Sep 2026). The
  // proposal and its buttons live in the ticket guard, so a completion
  // caught there and a "#4 done" caught here look the same to Era.
  if (parsed.actions.length) {
    const { proposeActions } = await import('./ticket-guard.js');
    if (await proposeActions({ db, wa, num: fromNum, who, text: body, actions: parsed.actions, items: open, extra: unmatched })) return true;
  }
  const reply = `Thanks ${who}, I could not match that to an open ticket.${unmatched}`;
