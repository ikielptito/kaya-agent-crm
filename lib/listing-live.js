...
export function missingForListing(l, lang) {
  ...
  // The map pin. Agents need to find the villa for a viewing, and a listing
  // with no location is one they can't act on — Vila Lestari went live without
  // one because nothing ever asked. Ikiel, 24 Aug 2026.
  if (!String(l.location || '').trim()) out.push(id ? 'link lokasi Google Maps villa (share pin-nya di sini)' : 'a Google Maps pin for the villa (just share the location link here)');
  if (!l.icalUrl && !l.manualDates) out.push(id ? 'link kalender (iCal dari Airbnb/Booking) — atau kabari saya tanggal yang sudah terisi' : 'a calendar link (Airbnb/Booking iCal) — or just tell me which dates are taken');
