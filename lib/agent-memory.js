...
const MIN_TOTAL = 40;              // shorter threads fit in the window anyway
...
export function memoryDue(agent, { force = false } = {}) {
  ...
  return total - Number(m.at_total || 0) >= REFRESH_EVERY;
}
...
async function olderMessages(db, agentId) {
  ...
  const older = rows.slice(WINDOW).reverse();
  return older.map(m => {
...
export async function refreshAgentMemory(db, apiKey, agent, { force = false } = {}) {
  ...
  if (!lines.length && !prev) return null;
