    ...
    if (status !== 'parked' && status !== 'rejected') {
      try {
        ...
        if (row?.wa_message_id) {
          const { turnPlaced } = await import('./staff-turns.js');
          const tt = itemId != null ? ['maintenance_item', itemId] : readinessId != null ? ['housekeeping_readiness', readinessId] : inspectionId != null ? ['housekeeping_inspection', inspectionId] : taskId != null ? ['housekeeping_task', taskId] : [null, null];
          await turnPlaced(db, row.wa_message_id, { targetType: tt[0], targetId: tt[1] });
        }
      } catch { /* best effort */ }
    }
    return true;
