...
async function sb(db, path, init = {}) {
  ...
  if (!r.ok) {
    ...
    if (r.status === 404 || /staff_photos|does not exist|PGRST205|42P01/i.test(t)) { _missing = true; return null; }
    throw new Error(`photos → ${r.status}: ${t.slice(0, 160)}`);
  }
  return r.status === 204 ? true : r.json().catch(() => null);
}
export const photosAvailable = () => !_missing;
