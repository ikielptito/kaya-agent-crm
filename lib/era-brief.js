...
const KIND = { turnover: 'turnover clean', regular: 'regular clean', pre_arrival: 'pre-arrival prep', deep_clean: 'deep clean', inspection: 'inspection round' };
...
export async function buildBrief(db, { date = null, now = new Date() } = {}) {
  ...
  const loose = {
    ...
    readiness: (readiness || []).filter(r => r.status === 'flagged' || r.status === 'unchecked').map(r => ({ villa: villa(r.slug), status: r.status, flags: r.flags || [] })),
  };
...
export function briefHeadline(b) {
  ...
  const per = b.cleaning.filter(p => p.who !== 'Unassigned').map(p => `${first(p.who)} ${p.visits.length}`).join(', ');
  ...
  const le = [];
  if (b.loose.not_done.length) le.push(`${b.loose.not_done.map(x => `${x.villa} ${x.kind}${x.who ? ` (${first(x.who)})` : ''}`).join(', ')} not marked done`);
  if (b.loose.readiness.length) le.push(`${b.loose.readiness.map(x => `${x.villa} photo check ${x.status === 'unchecked' ? 'missing' : 'flagged'}`).join(', ')}`);
...
export function snapshotOf(b, tasks) {
  ...
  for (const t of tasks) if (t.task_date === b.date || t.task_date === plus(b.date, 1)) snap.tasks[t.id] = { date: t.task_date, slug: t.slug, kind: t.kind, who: t.staff?.name || null };
...
// ── The link ────────────────────────────────────────────────────────
// The portal signs the page; the CRM only knows the shape of the URL. The
// signature is minted by the portal's hk_today_url action once and kept
// in settings, so this module never holds the sync secret's derivations.
async function todayUrl(db, date) {
  const cfg = (await getSettingValue(db, STATE_KEY).catch(() => null)) || {};
  if (cfg.url_base) return `${cfg.url_base}?d=${date}`;
  try {
    const r = await fetch(`${PORTAL}/api/housekeeping`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.LISTING_SYNC_SECRET}` }, body: JSON.stringify({ action: 'hk_today_url', payload: {} }) });
    const d = await r.json().catch(() => ({}));
    if (d?.url) { await saveSettingValue(db, STATE_KEY, { ...cfg, url_base: d.url }); return `${d.url}?d=${date}`; }
  } catch { /* fall through */ }
  return `${PORTAL}/payouts#/cleaning`;
}
...
export async function runEraBrief({ db, wa, now = new Date(), preview = false, force = false } = {}) {
  ...
  if (preview || force || (hour >= briefHour && cfg.day !== today)) {
    ...
    const url = await todayUrl(db, today);
  ...
  // After the brief: changes to today and tomorrow, once each.
