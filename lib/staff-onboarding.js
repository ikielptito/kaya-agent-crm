...
*Kapan foto diminta*
Setiap kunjungan diakhiri dengan foto. Bersih-bersih rutin: 2 foto saja (dapur dan kamar mandi) setelah tekan "Sudah selesai". Kalau tugasnya menyiapkan villa untuk tamu — turnover yang ada tamu berikutnya, persiapan sebelum tamu datang, pembersihan menyeluruh — saya minta 7 foto.

*7 foto*
1. Ruang tamu, sofa dengan sarungnya
2. Dapur: meja, kompor, dan bagian dalam oven kalau ada (buka pintunya)
...
// "Halo Maya, saya Putu" from a number already on the roster — the wa.me
// link Era forwards, or a person introducing herself after a long silence.
// Not chatter for Era: the welcome and the summary, as if she had tapped
// "Saya mengerti", and the state records that she has now been onboarded.
export const SELF_INTRO_RE = /^\s*(?:halo|hai|hi|hello|hey)?[\s,]*(?:maya)?[\s,]*(?:ini|saya|aku|nama saya|this is|i am|i'm|it'?s)\s+([A-Za-z][A-Za-z .'-]{1,30}?)(?:\s+(?:dari|from)\s+[A-Za-z0-9 .'-]{1,40})?\s*[.!🙏]*$/i;
export async function handleSelfIntro({ db, wa, fromNum, text, person }) {
  const m = String(text || '').match(SELF_INTRO_RE);
  if (!m || !person) return false;
  const said = m[1].trim().toLowerCase().split(' ')[0];
  const first = String(person.name || '').split(' ')[0];
  if (said !== first.toLowerCase()) return false;
  const to = digits(fromNum);
  const hello = `Halo ${first}, ini Maya dari Samba 🙏 Senang bisa terhubung. Saya yang mengirim jadwal bersih-bersih setiap pagi dan yang menerima foto-fotonya. Ini ringkasannya:`;
  const mid = await sendText(wa, to, hello);
  await logOut(db, { waNum: to, mid, content: hello });
  const dm1 = await sendText(wa, to, DETAIL(first));
  await logOut(db, { waNum: to, mid: dm1, content: DETAIL(first).slice(0, 300) + '…' });
  const dm = await sendDoc(wa, to, GUIDE_URL, 'Panduan-Housekeeper-Samba.pdf', 'Panduan Housekeeper Samba (PDF). Simpan di HP ya.');
  if (dm) await logOut(db, { waNum: to, mid: dm, content: '[Document — Panduan Housekeeper Samba]' });
  await patchState(db, to, { name: person.name, understood_at: nowIso(), intro_at: nowIso() });
  return true;
}
