...
// PostgREST caps any single read at 1,000 rows regardless of `limit=` — a
// 70-day trend asked for in one request came back as the newest ~1,000
// rows and made every earlier week look nearly silent. Page through.
async function sbGetAll(db, path, page = 1000) {
  const all = [];
  for (let offset = 0; ; offset += page) {
    const rows = await sbGet(db, `${path}&limit=${page}&offset=${offset}`);
...
export async function buildTrend(db, { days = 70, now = new Date() } = {}) {
  ...
  const [outRows, inRows] = await Promise.all([
    sbGetAll(db, `wa_messages?direction=eq.outbound&timestamp=gte.${encodeURIComponent(since)}`
      + `&select=status,error,template_name,category,wa_num,timestamp&order=timestamp.asc`),
    sbGetAll(db, `wa_messages?direction=eq.inbound&timestamp=gte.${encodeURIComponent(since)}`
