...
export async function buildSnapshot(db, { waToken, wabaId, phoneId, now = new Date() } = {}) {
  ...
  const rows = (await sbGet(db,
    ...
    + `&select=status,error,template_name,category,wa_num&limit=10000`)) || [];
...
Known context: 131049 is Meta's per-user marketing frequency cap; 131047 is a closed 24h customer window; 131026 is an undeliverable number. A silence-based digest cadence deployed 31 Aug should reduce marketing volume from 7 Sep.
...
export async function buildTrend(db, { days = 70, now = new Date() } = {}) {
  ...
  const [outRows, inRows] = await Promise.all([
    sbGet(db, `wa_messages?direction=eq.outbound&timestamp=gte.${encodeURIComponent(since)}`
      + `&select=status,error,template_name,category,wa_num,timestamp&limit=20000`),
    sbGet(db, `wa_messages?direction=eq.inbound&timestamp=gte.${encodeURIComponent(since)}`
      + `&select=wa_num,timestamp&limit=20000`),
  ]);
  const WITA = 8 * 3600e3;
  const weekOf = (ts) => {
  ...
  const wk = (ts) => {
    ...
    return weeks[k] || (weeks[k] = {
      ...
      by_error: {}, by_category: {}, failed_numbers: new Set(),
  ...
  for (const m of (outRows || [])) {
    ...
    const c = w.by_category[cat] || (w.by_category[cat] = { sent: 0, failed: 0, read: 0 });
    c.sent++;
    if (st === 'read') c.read++;
    if (st !== 'failed') continue;
    w.failed++; c.failed++;
