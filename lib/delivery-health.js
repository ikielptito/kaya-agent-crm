...
// ── Numbers nothing reaches ─────────────────────────────────────────
// "sent" means Meta took the message; "delivered" means a phone did. A
// number whose last three tracked sends all stopped at "sent" for three
// days or more, with nothing inbound in three weeks, is a phone that is
// off, a SIM that changed, or a person who blocked us. Every further send
// is waste and a quality-rating risk, so it rests in marketing_caps (30
// days, then 90) — the same list every broadcast loop already honours. An
// inbound message from the number clears nothing here; the cap simply
// expires, and a reply is its own proof of life for the tiering.
export async function restUnreachable(db, { now = new Date(), dryRun = false } = {}) {
  const since = new Date(now.getTime() - 21 * 86400e3).toISOString();
  const out = await sbRows(db.SUPABASE_URL, db.sbHeaders,
    `wa_messages?direction=eq.outbound&status=not.is.null&timestamp=gte.${encodeURIComponent(since)}&select=wa_num,status,timestamp&order=timestamp.desc`);
  const inb = await sbRows(db.SUPABASE_URL, db.sbHeaders,
    `wa_messages?direction=eq.inbound&timestamp=gte.${encodeURIComponent(since)}&select=wa_num&order=id.asc`);
  const heard = new Set(inb.map(m => m.wa_num).filter(Boolean));
  const byNum = {};
  for (const m of out) { if (!m.wa_num || heard.has(m.wa_num)) continue; (byNum[m.wa_num] ||= []).push(m); }
  const stuckCut = now.getTime() - 72 * 3600e3;
  const rested = [];
  for (const [num, rows] of Object.entries(byNum)) {
    const last3 = rows.slice(0, 3);
    if (last3.length < 3) continue;
    if (!last3.every(m => m.status === 'sent')) continue;
    if (Date.parse(last3[0].timestamp) > stuckCut) continue;      // give the newest one three days
    rested.push(num);
  }
  if (!rested.length || dryRun) return { candidates: rested.length, rested: dryRun ? rested : [] };
  const caps = (await getSettingValue(db, 'marketing_caps')) || {};
  const applied = [];
  for (const num of rested) {
    const prev = caps[num] || {};
    if (prev.until && Date.parse(prev.until) > now.getTime()) continue;   // already resting
    const strike = (prev.unreachable || 0) + 1;
    const days = strike >= 2 ? 90 : 30;
    caps[num] = { ...prev, until: new Date(now.getTime() + days * 86400e3).toISOString(), last: now.toISOString(), reason: 'unreachable', unreachable: strike, count: prev.count || 0 };
    applied.push(num);
  }
  if (applied.length) await saveSettingValue(db, 'marketing_caps', caps);
  return { candidates: rested.length, rested: applied };
}
...
export async function runDeliveryHealth(db, { waToken, wabaId, phoneId, apiKey, preview = false, now = new Date() } = {}) {
  ...
  // Rest the numbers nothing reaches, and say so in the snapshot.
  try { snap.unreachable = await restUnreachable(db, { now, dryRun: preview }); } catch { /* best-effort */ }
