...
// Atomic requested→confirmed transition: the status filter makes the row a
// mutex, so when a contact taps Confirm AND types "Yes" in the same second
// (Casa Suhana, 27 Aug 2026 — duplicate invites and agent pings), exactly one
// path wins and runs the side effects; the loser gets null and stays silent.
export async function confirmViewingOnce(db, id, { scheduledAt = null } = {}) {
  try {
    const r = await fetch(`${db.SUPABASE_URL}/rest/v1/viewings?id=eq.${id}&status=eq.requested`, {
      method: 'PATCH',
      headers: { ...db.sbHeaders, Prefer: 'return=representation' },
      body: JSON.stringify({
        status: 'confirmed', confirmed_at: iso(), updated_at: iso(),
