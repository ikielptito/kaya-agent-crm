...
export async function ensureVisit(db, { slug, date, person, kind = 'regular', actor = 'Maya', reason = '' } = {}) {
  ...
  if (near[0]) {
    const t = near[0];
    // A visit moved to a LATER day is still announced that morning (it is
    // 'planned' until the sweep speaks); one moved to today is confirmed
    // because she just told us. Before 8 Sep 2026 a moved-by-statement
    // visit kept its notified_at and was never sent again, yet the 17:00
    // chase still nagged about it.
    const future = date > today;
    await sbPatch(db, `housekeeping_tasks?id=eq.${t.id}`, {
      task_date: date, status: future ? 'planned' : 'confirmed', confirmed_at: nowIso(), notified_at: future ? null : (t.notified_at || nowIso()),
      moved_by: person?.name || actor, moved_at: nowIso(), updated_at: nowIso(),
  ...
  const r = await fetch(`${db.SUPABASE_URL}/rest/v1/housekeeping_tasks?on_conflict=slug,origin_date,kind`, {
    ...
    headers: { ...db.sbHeaders, Prefer: 'resolution=merge-duplicates,return=representation' },
    body: JSON.stringify({
      slug, task_date: date, origin_date: date, kind, status: date > today ? 'planned' : 'confirmed', confirmed_at: nowIso(),
...
export async function handleScheduleStatement({ db, wa, fromNum, text, mediaId = null, waMessageId = null, person: given = null }) {
  if (mediaId) return false;
  const body = String(text || '').trim();
  if (!looksLikeSchedule(body)) return false;
  const person = given || await staffByWa(db, fromNum);
  ...
  //
  // Days with NO villa named ("jadwal cleaning hari Senin dan Kamis") used to
  // be applied to every villa she covers. Gede's first line on 7 Sep 2026
  // did exactly that and put B2 and B6 on the wrong days until his next
  // line corrected it. Now a person who covers more than one villa is asked
  // which, with a list; a person with one villa is taken at her word.
  const partial = [];
  const askedDays = [];
  if (weeklyNow.length) {
    const grouped = new Map();
    for (const [slug, v] of weeklyNow) {
      if (v.all && slugs.length > 1) { const k = v.days.join(','); if (!askedDays.some(a => a.key === k)) askedDays.push({ key: k, days: v.days }); continue; }
  ...
  // Days without a villa, from someone with several: ask, with a list.
  if (askedDays.length) {
    const { sendList } = await import('./wa-interactive.js');
    const { recordAsk } = await import('./asks.js');
    for (const a of askedDays) {
      const code = a.days.join(',');
      const rows = [...slugs.slice(0, 8).map(s => ({ id: `sched:pick:${s}.${code}`, title: shortName(names, s).slice(0, 24), description: dayList(a.days).slice(0, 72) })), { id: `sched:pick:all.${code}`, title: 'Semua villa saya', description: `${dayList(a.days)} untuk semuanya`.slice(0, 72) }];
      const bodyAsk = `${dayList(a.days)} — untuk villa yang mana?`;
      const mid = await sendList(wa, fromNum, { body: bodyAsk, buttonLabel: 'Pilih villa', rows });
      await logOut(db, { waNum: fromNum, content: bodyAsk, mid, category: 'housekeeping_schedule' });
      await recordAsk(db, { waNum: fromNum, staffId: person.id, kind: 'schedule_pick', wamid: mid, payload: { days: a.days, said: body.slice(0, 160) }, expiresInHours: 24 });
    }
    try { const { coach } = await import('./coaching.js'); await coach(db, wa, { person, fromNum, key: 'schedule_no_villa' }); } catch { /* optional */ }
...
// Her pick from the "which villa?" list: sched:pick:<slug|all>.<d,d>.
export async function handleSchedulePick({ db, wa, fromNum, person: given = null, tap }) {
  if (!tap || tap.domain !== 'sched' || tap.verb !== 'pick') return false;
  const person = given || await staffByWa(db, fromNum);
  if (!person || !person.active) return false;
  const m = String(tap.id || '').match(/^([a-z0-9-]+)\.([0-6](?:,[0-6])*)$/);
  if (!m) return false;
  const days = m[2].split(',').map(Number);
  const slugs = m[1] === 'all' ? (person.slugs || []) : (person.slugs || []).filter(s => s === m[1]);
  if (!slugs.length) { await say(db, wa, fromNum, 'Villa itu bukan villa Anda; pilih dari daftar ya 🙏', 'housekeeping_schedule'); return true; }
  const today = witaToday();
  const names = await catalogNames(db).catch(() => ({}));
  for (const slug of slugs) await applyCleanDays(db, { slug, days, actor: person.name, note: `${person.name} picked on WhatsApp, ${today}`, today, regenerate: false });
  await generateTasks(db).catch(() => null);
  await say(db, wa, fromNum, `Siap, sudah saya catat 🙏\n${listNames(names, slugs)}: ${dayList(days)}`, 'housekeeping_schedule');
  await notifyEra(db, wa, `${person.name} set her schedule: ${listNames(names, slugs)} → ${days.map(d => DAY_ID[d]).join('/')}.`);
  return true;
}
