...
export async function attachInspectionPhotos(db, { inspectionId, itemIds = null, onlyEmpty = true, wa = null } = {}) {
  const insp = (await sbGet(db, `housekeeping_inspections?id=eq.${inspectionId}&select=id,slug,photos,item_ids,by_staff_id,staff:by_staff_id(wa_num)&limit=1`))?.[0];
  ...
  const { suggest, askReporter } = await import('./photo-assign.js');
  const all = new Set();
  ...
  const to = String(insp.staff?.wa_num || '').replace(/\D/g, '');
  if (all.size && wa && to) {
    const names = await (await import('./housekeeping.js')).catalogNames(db).catch(() => ({}));
    await askReporter(db, wa, { to, lang: 'id', items: targets.map(i => ({ id: i.id, title: i.title })), photos: [...all], villa: names[insp.slug] || insp.slug }).catch(() => {});
  }
