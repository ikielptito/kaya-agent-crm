...
import { signPhotoUrl, attachPhotoPaths } from './maintenance.js';
...
export async function attachInspectionPhotos(db, { inspectionId, itemIds = null, onlyEmpty = true } = {}) {
  ...
  for (const [id, paths] of Object.entries(attached)) await attachPhotoPaths(db, Number(id), paths).catch(() => {});
  return { inspection: insp.id, looked_at: looked, attached: Object.fromEntries(Object.entries(attached).map(([k, v]) => [k, v.length])) };
}
