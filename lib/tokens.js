...
//   portfolioSig(slugs)            weekly report, several villas: /r/<slug1+slug2+…>~<sig16>
//
// One Monday message per owner instead of one per villa (Ikiel's number took
// 12 identical pings on 7 Sep 2026). Slugs are sorted so the same set always
// signs the same way; the 'portfolio:' prefix keeps a single-villa token from
// verifying as a portfolio and vice versa. Mirror of the portal's copy.
...
// Era's day page on the portal: /today/<sig>?d=YYYY-MM-DD. Byte-compatible
// with the portal's lib/tokens.js todaySig (same secret, same message).
