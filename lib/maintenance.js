...
const GENERIC_NAME_WORDS = new Set(['villa', 'villas', 'haus', 'house', 'unit', 'units', 'the', 'and', 'valley', 'canggu', 'bali', 'rumah']);
...
export function pickProperty(groups, text) {
  ...
  if (!cands.length) {
    // Group-name fallback. Generic words are not evidence: "villa" alone
    // used to match "Villa Saturno" (half of a two-word name), which is how
    // Era's status replies were filed there on 5 Sep 2026. A name has to be
    // matched on a word that is actually its own.
    for (const g of groups) {
      const nameToks = normWords(g.name).filter(w => w.length > 2 && !isUnitTok(w) && !GENERIC_NAME_WORDS.has(w));
...
export async function createItem(db, fields) {
  ...
  const created = await sbInsert(db, 'maintenance_items', row);
  if (created?.id) await maintEvent(db, created.id, 'created', { actor: fields.reported_by_name || null, payload: { group_key: row.group_key, slug: row.slug, title: row.title, source: fields.source || null }, wamid: fields.wamid || null });
  return created;
}
...
export async function publishItem(db, id, { requires_approval, estimated_cost, actor = 'admin' } = {}) {
  ...
  await sbPatch(db, `maintenance_items?id=eq.${id}`, patch);
  await maintEvent(db, id, 'published', { actor, payload: { status: patch.status, requires_approval: needsApproval, estimated_cost: patch.estimated_cost ?? item.estimated_cost ?? null } });
...
export async function headsUpItem(db, id, { actor = 'admin' } = {}) {
  ...
  await maintEvent(db, id, 'heads_up', { actor });
  return { ok: true };
}
...
export async function approveItem(db, id, { by = 'owner' } = {}) {
  ...
  await maintEvent(db, id, 'approved', { actor: by });
  return { ok: true };
}
...
export async function declineItem(db, id, { note, by = 'owner' } = {}) {
  ...
  await maintEvent(db, id, 'declined', { actor: by, payload: { note: String(note || '').slice(0, 200) || null } });
  return { ok: true };
}

// An owner deciding their own ticket from the cockpit: Oli on Tropicana B,
// or Ikiel and Era on a unit Samba owns itself. There is nobody to ask, so
// the ticket goes straight from review to approved (or declined) and no
// "please approve" message is queued — notified_at is stamped as if it had
// been sent. Era still hears the outcome the way she does for any owner's
// answer (staff_notified_at cleared).
export async function ownerDecideItem(db, id, { decision, by = 'owner', estimated_cost, note } = {}) {
  const item = (await sbGet(db, `maintenance_items?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!item) throw new Error('item not found');
  if (!['new', 'pending_approval', 'scheduled'].includes(item.status)) throw new Error(`item is ${item.status}, not open for a decision`);
  const approve = decision !== 'decline';
  const now = nowIso();
  const patch = {
    requires_approval: true,
    status: approve ? 'approved' : 'declined',
    published_at: item.published_at || now,
    published_by: item.published_by || by,
    notified_at: item.notified_at || now,
    approved_by: by,
    approved_at: approve ? now : null,
    declined_at: approve ? null : now,
    decline_note: approve ? null : (String(note || '').slice(0, 500) || null),
    staff_notified_at: null,
    next_followup_at: approve ? inDays(FOLLOWUP_DAYS) : null,
    updated_at: now,
  };
  if (estimated_cost !== undefined && estimated_cost !== null && estimated_cost !== '') patch.estimated_cost = Number(estimated_cost) || null;
  await sbPatch(db, `maintenance_items?id=eq.${id}`, patch);
  await maintEvent(db, id, approve ? 'approved' : 'declined', { actor: by, payload: { direct: true, from: item.status, ...(approve ? { estimated_cost: patch.estimated_cost ?? item.estimated_cost ?? null } : { note: patch.decline_note }) } });
  return { ok: true, status: patch.status };
}

// A ticket the owner is still being asked about cannot be closed as if it
// were authorised; the caller has to say so (force) — the cockpit does,
// after a human decided the owner's answer no longer matters.
export async function completeItem(db, id, { note, actual_cost, by = 'admin', force = false } = {}) {
  const item = (await sbGet(db, `maintenance_items?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!item) throw new Error('item not found');
  if (item.status === 'done') return { ok: true, already: true };
  if (!['approved', 'scheduled', ...(force ? ['pending_approval'] : [])].includes(item.status)) {
    throw new Error(item.status === 'pending_approval' ? 'the owner has not answered yet — wait for the approval, or withdraw the request on the page' : `cannot complete an item that is ${item.status}`);
  }
  const patch = {
    status: 'done',
    completed_at: nowIso(),
  ...
  await sbPatch(db, `maintenance_items?id=eq.${id}`, patch);
  await maintEvent(db, id, 'completed', { actor: by, payload: { note: patch.completion_note || null, actual_cost: patch.actual_cost ?? null, from: item.status } });
  return { ok: true };
}

// Back to the state it was in before "done": approved work stays approved,
// routine work stays scheduled. Forcing every reopen to 'approved' used to
// make queue 2 tell Era "the owner approved" a job the owner never saw.
export async function reopenItem(db, id, { by = 'admin' } = {}) {
  const item = (await sbGet(db, `maintenance_items?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!item) throw new Error('item not found');
  if (item.status !== 'done') return { ok: true, already: true, status: item.status };
  const back = item.approved_at && item.requires_approval ? 'approved' : item.published_at ? 'scheduled' : 'new';
  await sbPatch(db, `maintenance_items?id=eq.${id}`, {
    status: back, completed_at: null, done_notified_at: null, completion_note: null,
    next_followup_at: back === 'new' ? null : inDays(FOLLOWUP_DAYS), updated_at: nowIso(),
  });
  await maintEvent(db, id, 'reopened', { actor: by, payload: { to: back } });
  return { ok: true, status: back };
}

// Filed under the wrong villa: the ticket follows the villa to its owner
// group, and the messaging starts over — the new owner is asked (or told)
// by the next sweep, and the old owner's link keeps opening, on a page that
// says the item was re-filed. Nothing is un-sent; the old group is kept on
// the thread so publicItem can honour the old link.
export async function moveItem(db, id, { slug, group_key, unit_label = null, by = 'admin' } = {}) {
  const item = (await sbGet(db, `maintenance_items?id=eq.${id}&select=*&limit=1`))?.[0];
  if (!item) throw new Error('item not found');
  const patch = {
    group_key, slug, unit_label: unit_label ? String(unit_label).slice(0, 40) : null,
    // The new owner has heard nothing: every owner-facing latch resets. What
    // Era was told (approved / dispatch) stands.
    notified_at: null, heads_up_sent_at: null, done_notified_at: item.status === 'done' ? null : item.done_notified_at,
    // An approval given by the old owner is not the new owner's approval.
    ...(item.status === 'approved' && item.group_key !== group_key ? { status: 'pending_approval', approved_at: null, approved_by: null } : {}),
    thread: [...(item.thread || []), { at: nowIso(), who: by, text: `Moved from ${item.slug || item.group_key} to ${slug}`, moved_from: item.group_key, moved_from_slug: item.slug || null }].slice(-50),
    updated_at: nowIso(),
  };
  await sbPatch(db, `maintenance_items?id=eq.${id}`, patch);
  await maintEvent(db, id, 'moved', { actor: by, payload: { from: item.group_key, from_slug: item.slug, to: group_key, to_slug: slug, owner_was_notified: !!item.notified_at } });
  return { ok: true, group: group_key, owner_was_notified: !!item.notified_at, was: item.slug || item.group_key, re_ask: patch.status === 'pending_approval' };
}
...
export async function snoozeItem(db, id, { untilDate, note, who = 'era' } = {}) {
  ...
  await sbPatch(db, `maintenance_items?id=eq.${id}`, patch);
  await maintEvent(db, id, 'snoozed', { actor: who, payload: { until: untilDate || null, note: String(note || '').slice(0, 200) } });
...
export async function publicItem(db, groupKey, id) {
  let item = (await sbGet(db, `maintenance_items?id=eq.${id}&group_key=eq.${encodeURIComponent(groupKey)}&select=*,statement_groups(key,name,owner_names)&limit=1`))?.[0];
  if (!item) {
    // The old owner's link after a move: the page says so instead of
    // "no maintenance item for that link" (Cielo, 6 Sep 2026).
    const moved = (await sbGet(db, `maintenance_items?id=eq.${id}&select=id,title,thread&limit=1`))?.[0];
    const from = (moved?.thread || []).find(e => e && e.moved_from === groupKey);
    if (moved && from) return { id: moved.id, moved: true, title: moved.title, moved_at: from.at, group: { key: groupKey } };
    return null;
  }
  // Items Era hasn't published yet are internal — the owner sees nothing.
  // Except a heads-up: Era chose to tell the owner before the cost was
  // known, so the notice's link has to open (Cielo, 6 Sep 2026: two
  // "View details" buttons that said "no maintenance item for that link").
  if (item.status === 'new' && !item.heads_up_at) return null;
  ...
  return {
    ...
    status: item.status,
    heads_up: item.status === 'new' && !!item.heads_up_at,
...
export async function ownerItems(db, groupKeys) {
  ...
  const rows = (await sbGet(db, `maintenance_items?group_key=in.(${encodeURIComponent(list)})&or=(status.neq.new,heads_up_at.not.is.null)&select=*,statement_groups(key,name)&order=created_at.desc&limit=200`)) || [];
