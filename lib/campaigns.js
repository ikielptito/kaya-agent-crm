...
export const CAMPAIGN_REGISTRY = {
  ...
  intro_question: {
    name: 'Intro question', goal: 'reply', seedStatus: 'live',
    categories: ['availability_intro_question'],
    context: '"Are you a rental agent?" with three buttons, to introduced agents who never replied — rung two of the ladder',
    schedule: { cron: 'daily 09:00 WITA (not Mondays)', cap_setting: 'samba_availability.intro_question_daily_cap' },
    control: { cap: { key: 'samba_availability', path: 'intro_question_daily_cap' } },
  },
  intro_follow: {
    name: 'Intro follow-through', goal: 'reply', seedStatus: 'live',
    categories: ['availability_intro_follow'],
    context: 'Second and third hello: the Monday digest to introduced agents who never replied, a fortnight apart, then stalled',
    schedule: { cron: 'Mondays 09:00 WITA, inside the digest waves', cap_setting: 'samba_availability.intro_follow_weekly_cap' },
    control: { cap: { key: 'samba_availability', path: 'intro_follow_weekly_cap' } },
  },
  new_arrivals: {
  ...
  maintenance: {
    ...
    categories: ['maintenance', 'maintenance_staff', 'maintenance_tukang'],
    context: 'Owner approvals, completion notices, nudges to Era, and tukang dispatch',
    schedule: { cron: 'daily 09:00 WITA (wave 0), event-driven on publish/approve/complete/assign', cap_setting: 'maintenance.notify_daily_cap', env_gate: 'OWNERS_ENABLED=1' },
    control: { cap: { key: 'maintenance', path: 'notify_daily_cap' }, env: 'OWNERS_ENABLED' },
  },
  housekeeping: {
    name: 'Housekeeping', goal: 'reply', seedStatus: 'paused',
    categories: ['housekeeping'],
    context: 'Daily cleaning tasks to housekeepers, the week ahead on Mondays, and bi-weekly inspection rounds',
    schedule: { cron: 'generator hourly (relay sweep), notifications daily 09:00 WITA (wave 0)', cap_setting: 'housekeeping.notify_daily_cap' },
    control: { cap: { key: 'housekeeping', path: 'notify_daily_cap' } },
  },
};
...
export async function buildAudience(db, filter = {}) {
  ...
  if (filter.not_received_category) {
    ...
    const rows = await sbRows(db.SUPABASE_URL, db.sbHeaders,
      `wa_messages?select=agent_id&direction=eq.outbound&category=eq.${encodeURIComponent(filter.not_received_category)}&timestamp=gte.${since}&agent_id=not.is.null&order=id.asc`);
