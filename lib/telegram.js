...
// A photo with a caption to the private chat (product feedback screenshots).
// `url` must be fetchable by Telegram's servers — a signed storage URL is.
export async function postTelegramPhoto(url, caption, opts = {}) {
  if (!telegramEnabled()) return null;
  const token = process.env.TELEGRAM_BOT_TOKEN;
  const chatId = process.env.TELEGRAM_CHAT_ID;
  try {
    const r = await fetch(`${TG_API}/bot${token}/sendPhoto`, {
      ...
      body: JSON.stringify({ chat_id: chatId, photo: url, caption: String(caption || '').slice(0, 1000), parse_mode: 'HTML', ...opts }),
    });
    if (!r.ok) { console.warn('Telegram photo failed:', r.status, (await r.text()).slice(0, 200)); return null; }
    return (await r.json())?.result?.message_id || null;
  } catch (e) { console.warn('Telegram photo error:', e.message); return null; }
}
