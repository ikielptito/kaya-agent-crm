...
export function modeOf({ undeliveredSince = null, lastReadAt = null, lastInboundAt = null, ignoredAsks7d = 0, asks7d = 0, graceUntil = null, now = new Date() } = {}) {
  const t = now.getTime();
  // Someone who wrote to Maya in the last two days is reachable, whatever
  // the delivery statuses say; so is someone Era has vouched for (a reset
  // from the console, good for a day).
  if (lastInboundAt && t - Date.parse(lastInboundAt) < 2 * 86400e3) return 'ok';
  if (graceUntil && t < Date.parse(graceUntil)) return 'ok';
...
export async function refreshChannels(db, { now = new Date() } = {}) {
  ...
  const grace = (await (await import('./campaigns.js')).getSettingValue(db, 'staff_channel_grace').catch(() => null)) || {};
  const modes = {};
  for (const s of staff) {
    if (!s.wa_num) continue;
    const f = await channelFacts(db, s.wa_num, { now });
    const mode = modeOf({ undeliveredSince: f.undelivered_since, lastReadAt: f.last_read_at, lastInboundAt: f.last_inbound_at, ignoredAsks7d: f.ignored_asks_7d, asks7d: f.asks_7d, graceUntil: grace[s.id] || null, now });
...
// Era spoke to her: treat the channel as open for a day, so today's visit
// and the onboarding go out instead of "uncovered".
export async function resetChannel(db, staffId, { hours = 24 } = {}) {
  const { getSettingValue, saveSettingValue } = await import('./campaigns.js');
  const grace = (await getSettingValue(db, 'staff_channel_grace').catch(() => null)) || {};
  grace[staffId] = new Date(Date.now() + hours * 3600e3).toISOString();
  await saveSettingValue(db, 'staff_channel_grace', grace);
  if (!isMissing()) await fetch(`${db.SUPABASE_URL}/rest/v1/staff_channel?staff_id=eq.${staffId}`, { method: 'PATCH', headers: db.sbHeaders, body: JSON.stringify({ mode: 'ok', note: `reset until ${grace[staffId]}`, updated_at: nowIso() }) }).catch(() => {});
  return grace[staffId];
}
