...
const OWNER_VISIT = { done: 'done', skipped: 'skipped', unconfirmed: 'scheduled, not confirmed by the housekeeper', not_sent: 'not covered', uncovered: 'not covered (no housekeeper reachable)', open: 'today, in progress' };
export const STATUS_LABEL = {
  pass: 'Checked, nothing to fix', flagged: 'Checked, issues fixed before arrival', unchecked: 'Not photographed', unverified: 'Photos received, not yet checked',
...
export async function ownerRecords(db, { slugs = [], from, to, limit = 1500 } = {}) {
  ...
  const [built, upcoming] = await Promise.all([
    buildRecords(db, { from, to, slugs: list, limit, audience: 'owner' }),
    upcomingRounds(db, list).catch(() => []),
  ]);
  return { from, to, names: built.names, records: built.records, upcoming };
}
  ...
  for (const [slug, name] of Object.entries(data.names)) {
    ...
    for (const r of recs.slice(0, 12)) {
      ...
      if (r.type === 'handover') {
        ...
        s += ` — ${OWNER_VISIT[r.status] || r.status}${r.photo_count ? ` (${r.photo_count} photos)` : ''}`;
      }
      lines.push(s);
    }
