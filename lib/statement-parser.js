...
export function parseMonthTab(rows, { tabTitle = '' } = {}) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let r = i + 1; r < rows.length; r++) {
      ...
      // Era leaves up to ~5 pre-formatted empty rows between the last booking
      // and the merged Total — tolerate a wide gap before giving up.
      if (!windowText) { if (++blank >= 7) break; continue; }
      blank = 0;
  ...
  // ── Expense blocks (a tab can hold several — e.g. a catch-up block of
  // last month's costs after the current month's, observed on B4) ──────
  const expenses = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let r = i + 1; r < rows.length; r++) {
      ...
      if (!windowText) { if (++blank >= 7) { end = r; break; } continue; }
      blank = 0;
      if (/^total\s*$/i.test(cellStr(row[0])) || /^total\s*$/i.test(windowText.replace(/[\d.,\s]+$/, '').trim())) {
        // Several blocks per tab: era totals accumulate.
        eraExpenseTotal = (eraExpenseTotal || 0) + (amtCol(row) || 0);
        consumed.add(r);
        end = r;
        break;
      }
      if (/t[o0]tal\s*payout/i.test(windowText)) { end = r - 1; break; }
    ...
    i = end;   // continue scanning: another expense block may follow
  }

  // ── TOTAL PAYOUT ──────────────────────────────────────────────────
  // /t[o0]tal/ also matches Era's real-world "T0TAL PAYOUT" (zero for O).
  let eraPayoutTotal = null;
  for (let i = rows.length - 1; i >= 0; i--) {
    const row = rows[i] || [];
    if (!row.some(c => /t[o0]tal\s*payout/i.test(cellStr(c)))) continue;
  ...
  // "Minus from February | 2,053,995" — a prior month's deficit (or credit)
  // Era rolls into this month between the expense TOTAL and TOTAL PAYOUT.
  // Parsed as a signed adjustment line so the payout math reconciles.
  const adjustments = [];
  rows.forEach((row, r) => {
    if (consumed.has(r)) return;
    const label = row.map(cellStr).find(Boolean) || '';
    const m = label.match(/^(minus|less|deficit|plus|add|carry)\b.{0,40}$/i);
    if (!m) return;
    let value = null;
    for (const c of row) { const n = cellNum(c); if (n !== null && Math.abs(n) >= 1000) { value = n; break; } }
    if (value === null) return;
    const negative = /^(minus|less|deficit)/i.test(label);
    adjustments.push({
      description: label,
      amount: negative ? -Math.abs(value) : Math.abs(value),
      flags: ['carry_forward'], source_row: r,
    });
    consumed.add(r);
  });
  ...
  const adjSum = adjustments.reduce((a, x) => a + x.amount, 0);
  if (eraExpenseTotal !== null) check('expenses_vs_era_total', eraExpenseTotal, expSum);
  if (eraPayoutTotal !== null) check('payout_vs_nett_minus_expenses', eraPayoutTotal, nettSum - expSum + adjSum);
