...
export function introQuestionDue(agent, cfg, now = new Date()) {
  ...
  // Gap from the LATEST touch, not the intro: while Meta reviews the template
  // the digest may have gone first, and two hellos in one week is the pattern
  // that costs the number its quality rating.
  const last = [samba.last_intro_digest_at, samba.intro_at, agent.last_availability_alert_at]
    .map(x => (x ? Date.parse(x) : NaN)).filter(n => !Number.isNaN(n));
  return daysSince(last.length ? new Date(Math.max(...last)).toISOString() : null, now) >= cfg.intro_question_gap_days;
}
