// Shared mapping: a Samba portal listing (from sambarentals.com/api/listings)
...
function extractAmenities(features) {
  ...
  if (text.includes('oven')) tags.push('Oven');
...
export function mapListingToRental(l, idx, coverMap = {}) {
  ...
  // KEY FACTS — the questions agents ask most, as the portal's admin (or the
  // listed contact, via Maya's chase) filled them in. Missing ones are named
  // explicitly so Maya knows to ask the listed contact rather than guess.
  const KEY_FACTS = [['Deposit', l.deposit], ['Electricity', l.electricity], ['Wifi', l.wifi], ['Pool', l.pool], ['Minimum stay', l.minStay],
    ['Pets', l.petFriendly === true ? 'allowed' : (l.petFriendly === false ? 'not allowed' : '')]];
  const known = KEY_FACTS.filter(([, v]) => String(v || '').trim());
  // A fact the inclusions list already answers is not missing: Tropicana's
  // list says "400mbps fibre internet" and "electricity excluded for
  // yearly", and listing wifi and electricity as "not on file" beside it
  // taught Maya to say the inclusions were not on file either (7 Sep 2026).
  const inclText = inclusions.join(' ').toLowerCase();
  const coveredByInclusions = (k) =>
    (k === 'Wifi' && /wifi|internet|fibre|fiber|mbps/.test(inclText))
    || (k === 'Electricity' && /electric|listrik|pln/.test(inclText))
    || (k === 'Pool' && /pool/.test(inclText));
  const missing = KEY_FACTS.filter(([k, v]) => !String(v || '').trim() && !coveredByInclusions(k)).map(([k]) => k.toLowerCase());
  if (known.length || missing.length) {
    extendedSections.push('KEY FACTS:\n' + known.map(([k, v]) => `- ${k}: ${String(v).trim()}`).join('\n')
      + (missing.length ? `${known.length ? '\n' : ''}- Not on file (ask the listed contact via ask_owner, do not guess): ${missing.join(', ')}` : ''));
  }
  if (l.overview) extendedSections.push(`OVERVIEW:\n${l.overview}`);
  if (features.length) extendedSections.push('FEATURES:\n' + features.map(f => `- ${f}`).join('\n'));
  if (inclusions.length) extendedSections.push('INCLUDED IN THE RENT (on file — answer "what\'s included" from this list):\n' + inclusions.map(i => `- ${i}`).join('\n'));
  ...
  // The portal's third price is the TWO-YEAR lease figure ("/ 2 years" on the
  // listing page) — Maya once read "ALTERNATE YEARLY: 550jt" as a second yearly price.
  if (l.yearly2) extendedSections.push(`2-YEAR LEASE RATE: ${l.yearly2} (price for a two-year lease, not another yearly figure)`);
  if (l.yearly && !l.monthly) extendedSections.push('RATE BASIS: yearly only — no monthly rate exists for this villa; the yearly figure is the rate.');
  ...
  if (l.yearly2) mayaParts.push(`2-year lease: ${l.yearly2} IDR`);
  if (l.yearly && !l.monthly) mayaParts.push('Yearly only, no monthly rate');
  ...
  const row = {
    ...
    // Pet-friendly is a first-class portal toggle (l.petFriendly). Surface it in
    // amenities only when explicitly true so Maya can confirm it; when off/unset
    // she simply won't list it (and won't claim pets are allowed).
    amenities: [extractAmenities(features), l.petFriendly ? 'Pet-friendly' : null].filter(Boolean).join(', '),
    features: features.join('\n'),
    nightly_rate_usd: null,
    nightly_rate_idr: null,
    min_stay_nights: (() => { const m = String(l.minStay || '').match(/(\d+)\s*(night|malam|day|hari)/i); return m ? parseInt(m[1], 10) : 30; })(),
    occupancy_pct: null,
