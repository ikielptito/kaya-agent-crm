...
// Every number the staff console may see or message: the active roster
// plus the villa manager's own line, so Era's app also shows her own thread
// with Maya (the readiness exceptions, the dispatch updates). Era is not a
// roster row on purpose — the webhook routes roster numbers to the staff
// branch, and her line must keep its team-question handling.
export const MANAGER_NUMS = () => [String(process.env.ERA_WA_NUM || '6281246357778').replace(/\D/g, '')];
export async function staffLineNumbers(db) {
  const rows = await listStaff(db, { active_only: true });
  const set = new Set(MANAGER_NUMS());
  rows.forEach(p => { const n = digits(p.wa_num); if (n) set.add(n); });
  return set;
}
