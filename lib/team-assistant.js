...
async function applyWrite(db, tool, input, { actor, dryRun, villa }) {
  ...
  if (tool === 'hk_set_status') {
    ...
    if (t.status !== input.status) await recordCorrection(db, { targetType: 'housekeeping_task', targetId: t.id, change: { status: { from: t.status, to: input.status } }, actor, source: 'team' }).catch(() => {});
  ...
  if (tool === 'hk_move') {
    ...
    if (t.task_date !== input.date) await recordCorrection(db, { targetType: 'housekeeping_task', targetId: t.id, change: { task_date: { from: t.task_date, to: input.date } }, actor, source: 'team' }).catch(() => {});
  ...
  if (tool === 'hk_reassign') {
    ...
    if (t.assigned_staff_id !== p.id) await recordCorrection(db, { targetType: 'housekeeping_task', targetId: t.id, change: { assigned_staff_id: { from: t.assigned_staff_id, to: p.id } }, actor, source: 'team' }).catch(() => {});
  ...
  if (tool === 'maint_move') {
    ...
    const summary = `#${it.id} ${it.title.slice(0, 40)} moved to ${villa(input.slug)}${it.notified_at ? ' — the previous owner had been told; their link now says it was re-filed, and the new owner is asked on the next pass' : ''}`;
    if (dryRun) return { summary, would: true };
    const { moveItem } = await import('./maintenance.js');
    await moveItem(db, it.id, { slug: input.slug, group_key: group.key, by: `${actor} (via Maya)` });
    if (it.slug !== input.slug) await recordCorrection(db, { targetType: 'maintenance_item', targetId: it.id, change: { slug: { from: it.slug, to: input.slug }, group_key: { from: it.group_key, to: group.key } }, actor, source: 'team' }).catch(() => {});
    return { summary, inverse: { table: 'maintenance_items', id: it.id, fields: { group_key: it.group_key, slug: it.slug, unit_label: it.unit_label, notified_at: it.notified_at, status: it.status, approved_at: it.approved_at } } };
  }
...
async function executePending(db, wa, pending, { actor, num }) {
  ...
  if (tool === 'reask_readiness') {
    ...
    const to = String(t.staff.wa_num).replace(/\D/g, '');
    const mid = await sendText(wa, to, ask.text || String(ask));
    if (!mid) throw new Error('WhatsApp refused (her window may be shut)');
    try { const { recordAsk } = await import('./asks.js'); await recordAsk(db, { waNum: to, staffId: t.staff.id, kind: 'readiness', targetType: 'housekeeping_readiness', targetId: ask.id || null, wamid: mid, payload: { slug: t.slug, task_id: t.id, by: 'Era' }, expiresInHours: 20 }); } catch { /* optional */ }
    return { ok: true };
  }
