...
const WRITE_TOOLS_T2 = [
  { name: 'maint_publish', description: 'Publish a ticket to the owner: either ask them to approve the cost (requires_approval true) or tell them it is routine and scheduled (false). Optionally set the estimate at the same time. Waits for Yes.',
...
async function undoWrite(db, inverse) {
  ...
  if (inverse.delete) { await fetch(`${db.SUPABASE_URL}/rest/v1/${inverse.table}?id=eq.${inverse.delete}`, { method: 'DELETE', headers: db.sbHeaders }); return; }
...
export async function handleTeamMessage({ db, wa, fromNum, text, apiKey = process.env.ANTHROPIC_API_KEY, dryRun = false, role: roleOverride = null, buttonPayload = null }) {
  ...
  if (pendingOut) {
    ...
    const text = reply || `Just to confirm: ${pendingOut.summary}`;
