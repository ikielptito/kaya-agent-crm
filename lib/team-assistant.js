...
- If the message is not addressed to you — a note for ${role === 'admin' ? 'Era' : 'Ikiel'}, an aside, a forwarded chat, a plain status update with nothing to look up — call not_for_me and stop. It is passed on to a person. When in doubt about whether a question is for you, answer it.
...
export async function handleTeamMessage({ db, wa, fromNum, text, apiKey = process.env.ANTHROPIC_API_KEY, dryRun = false, role: roleOverride = null }) {
  ...
  const system = systemPrompt({ role, name, ctx, paused: false });
  const tools = role === 'admin' ? [...READ_TOOLS, ...ADMIN_TOOLS] : READ_TOOLS;
  const thread = await recentThread(db, num).catch(() => '');
  const msgs = [{ role: 'user', content: `${thread ? `Recent thread:\n${thread}\n\n` : ''}${name} just sent: "${body}"` }];
