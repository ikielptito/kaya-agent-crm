...
export const PER_MESSAGE_CAP = 6;   // villas per contact per round — Era manages 16
const ERA_WA = String(process.env.ERA_WA_NUM || '6281246357778').replace(/\D/g, '');

const KEY_FACTS = [
  ['deposit', 'deposit (amount and whether it is refundable)'],
  ['electricity', 'electricity (included or paid by the tenant, monthly and yearly)'],
  ['wifi', 'wifi speed'],
...
export function missingFacts(listing) {
  ...
  if (listing.petFriendly !== true && listing.petFriendly !== false) out.push('whether pets are allowed');
  return out;
}
...
export async function chaseMissingListingInfo(db, wa, { now = new Date() } = {}) {
  ...
  for (const [contactWa, { name, items: all }] of byContact) {
    // One message per contact per round, capped so it stays readable; the
    // rest wait for the next round.
    const items = all.slice(0, PER_MESSAGE_CAP);
    const lines = items.map(i => `• ${i.name}: ${i.missing.join(', ')}`).join('\n');
    const first = items[0];
    const res = await openRelay(db, wa, {
      agent: null,
      question: LISTING_INFO_PREFIX + lines,
      slug: first.slug,
      propertyName: items.length === 1 ? first.name : `${items.length} of your listings`,
