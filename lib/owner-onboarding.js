...
right here in chat. Never pressure; you are following up on THEIR yes.`;
  // The ad's fact sheet, when the prospect came in via listing screenshots.
  const intelMatch = String(owner.notes || '').match(/AD INTEL \(unverified\):\n([\s\S]{0,900})/);
  const intelBlock = intelMatch ? `
WHAT THEIR OWN AD SAYS (extracted from their listing screenshots — UNVERIFIED; these are THEIR claims, not our facts):
${intelMatch[1].trim()}
Use this well: mention one or two specifics naturally so they know you actually looked at THEIR villa ("the rooftop terrace looks great") — specificity is what earns a reply to a cold message. Never repeat these details to agents or anyone else as confirmed facts. When they agree to list, prefill the intake from this fact sheet and CONFIRM the key facts with them (price, bedrooms, area) instead of re-interviewing from scratch. Photos always come fresh from the owner in chat — ad screenshots are never listing photos.
` : '';
  return `
${openingFrame}
${langLine}${intelBlock}
