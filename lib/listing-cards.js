...
export async function fetchPortalCards() {
  ...
  const cards = listings
    ...
    .map(l => {
      // Yearly-only villas carry their yearly figure; the card used to say
      // "Monthly rental" for a villa that is not let monthly at all.
      const rate = l.monthly ? `${l.monthly}/mo` : (l.yearly ? `${l.yearly}/yr · yearly only` : 'Monthly rental');
