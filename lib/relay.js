...
// Villa contacts are never messaged by the sweep at night. The sweep runs
// every hour, and on 7 Sep 2026 its first Monday pass (00:05 WITA) re-asked
// 22 questions across 8 owners at midnight. Everything contact-facing in the
// sweep — repairs, nudges, the Monday re-ask, answer delivery — waits for
// this window; bookkeeping that sends nothing still runs.
export const SWEEP_HOURS_WITA = { from: 8, until: 21 };
export const REASK_HOUR_WITA = 9;
export const witaDate = (now = new Date()) => new Date(now.getTime() + 8 * 3600e3);
export const witaHour = (now = new Date()) => witaDate(now).getUTCHours();
export const inSweepHours = (now = new Date()) => {
  const h = witaHour(now);
  return h >= SWEEP_HOURS_WITA.from && h < SWEEP_HOURS_WITA.until;
};
// Viewing yes/no from the villa goes to the agent up to midnight — requests
// come in late and confirmations are wanted first thing (Ikiel, 10 Sep 2026).
// Only the small hours wait for the 08:05 sweep.
export const VIEWING_HOURS_WITA = { from: 8, until: 24 };
export const inViewingHours = (now = new Date()) => {
  const h = witaHour(now);
  return h >= VIEWING_HOURS_WITA.from && h < VIEWING_HOURS_WITA.until;
};
...
async function waText(wa, to, body, { quote = null } = {}) {
  if (!wa?.phoneId || !wa?.token || !to) return null;
  try {
    const r = await fetch(`${GRAPH}/${wa.phoneId}/messages`, {
      method: 'POST',
      headers: { Authorization: 'Bearer ' + wa.token, 'Content-Type': 'application/json' },
      body: JSON.stringify({ messaging_product: 'whatsapp', to, type: 'text', text: { body }, ...(quote ? { context: { message_id: quote } } : {}) }),
    });
...
// An agent-facing line composed elsewhere (a viewing confirmed / declined by
// the villa contact) that the sweep must send word for word in daytime hours.
// Era tapped "Can't this time" at 05:17 WITA and the agent got the decline at
// 05:17 (BAM, 10 Sep 2026); the viewing path now parks it here instead.
export const VERBATIM_PREFIX = '[verbatim]';
export function handbackText(prop, contactName, contactWa, { haveCard = false } = {}) {
  const who = (contactName && !isPlaceholderName(contactName)) ? contactName : 'the villa contact';
  const num = String(contactWa || '').replace(/\D/g, '');
  return `${HANDBACK_PREFIX}Still no answer from the villa side on ${prop}, sorry — I'll keep trying, but the quickest route now is ${who} directly${haveCard ? ' (you have their card from earlier)' : ` on +${num} — sending the card so it's to hand`}.`;
}
// Whether this agent already holds this contact's card (sent within `days`).
async function agentHasCard(db, agentWa, contactWa, days = 14) {
  const num = String(contactWa || '').replace(/\D/g, '');
  const to = String(agentWa || '').replace(/\D/g, '');
  if (!num || !to) return false;
  const since = new Date(Date.now() - days * 86400e3).toISOString();
  const rows = await sbGet(db, `wa_messages?wa_num=eq.${to}&direction=eq.outbound&timestamp=gte.${since}&content=like.${encodeURIComponent(`[Contact card:%+${num}]`)}&select=wa_message_id&order=timestamp.desc&limit=1`);
...
export async function deliverAnswers(db, wa, agentWa) {
  ...
  // Meta ACCEPTS free text into a shut window and fails it later (131047),
  // so a returned id proved nothing: Vira's Villa Rice answer was marked
  // delivered to an agent who never got it (7 Sep 2026). Same rule as the
  // contact leg — decide from our own inbound log first.
  const open = await windowOpen(db, to);
  let delivered = 0;
  for (const r of rows) {
    const prop = r.property_name || r.rental_slug || 'that property';
    const isHandback = String(r.answer || '').startsWith(HANDBACK_PREFIX);
    const isVerbatim = String(r.answer || '').startsWith(VERBATIM_PREFIX);
    const who = (r.contact_name && !isPlaceholderName(r.contact_name)) ? ` ${firstName(r.contact_name)} confirmed:` : ' They confirmed:';
    const body = isHandback ? String(r.answer).slice(HANDBACK_PREFIX.length)
      : isVerbatim ? String(r.answer).slice(VERBATIM_PREFIX.length)
      : `Good news on ${prop} —${who} ${r.answer}`;
    const refersToCard = isHandback && /you have their card from earlier/.test(body);
    const priorCard = refersToCard ? await agentHasCard(db, to, r.contact_wa) : null;
    const mid = open ? await waText(wa, to, body, { quote: typeof priorCard === 'string' ? priorCard : null }) : null;
    if (mid) {
      if (isHandback && !/you have their card from earlier/.test(body)) {
...
export async function sweepRelays(db, wa, { now: nowDate = new Date() } = {}) {
  const now = nowDate.getTime();
  const out = { nudged: 0, expired: 0, retried: 0, repaired: 0 };

  // Night: close the no-agent answered rows (no send) and stop there. Every
  // other step here messages a villa contact or an agent.
  if (!inSweepHours(nowDate)) {
    await closeOrphanAnswers(db);
    return { ...out, quiet: `${String(witaHour(nowDate)).padStart(2, '0')}:${String(witaDate(nowDate).getUTCMinutes()).padStart(2, '0')} WITA — contact-facing sweep runs ${SWEEP_HOURS_WITA.from}:00–${SWEEP_HOURS_WITA.until}:00` };
  }
  ...
  for (const r of stale) {
    ...
    if (ageH >= RELAY_TTL_HOURS) {
      ...
      const haveCard = hasAgent ? await agentHasCard(db, r.agent_wa, r.contact_wa) : false;
      await sbPatch(db, `relays?id=eq.${r.id}`, hasAgent
        ? { status: 'answered', answer: handbackText(prop, r.contact_name, r.contact_wa, { haveCard }), answered_at: new Date().toISOString() }
        : { status: 'expired' });
      out.expired++;
      continue;
    }
  ...
  try { out.reask = await reaskExpired(db, wa, { now: nowDate }); } catch (e) { out.reask = { error: e.message }; }
...
async function closeOrphanAnswers(db) {
...
// The re-ask carries the same rule as openRelay: one plain information
// question per listing with a contact at a time, newest wording wins, and one
// listing-info round per contact (its text already lists every listing). The
// rest stay expired with their nudge count untouched, so a later Monday picks
// them up once the first is answered. Exact-text dedup alone let Villa Rice's
// contact get four wordings of the oven question in one flush (7 Sep 2026).
export function pickReaskRelays(list) {
  const keep = []; let info = null; const perSlug = new Map();
  for (const r of [...list].sort((a, b) => Date.parse(b.created_at || 0) - Date.parse(a.created_at || 0))) {
    if (isListingInfo(r.question)) { if (!info) { info = r; keep.push(r); } continue; }
    const k = r.rental_slug || `#${r.id}`;
    if (perSlug.has(k)) continue;
    perSlug.set(k, r); keep.push(r);
  }
  return keep;
}
...
export async function reaskExpired(db, wa, { now = new Date() } = {}) {
  ...
  const wita = witaDate(now);
  if (wita.getUTCDay() !== 1) return { ...out, skipped: 'not Monday' };
  // The sweep is hourly; without this the re-ask fired on Monday's FIRST
  // pass, 00:05 WITA (7 Sep 2026). The morning pass owns it.
  if (wita.getUTCHours() < REASK_HOUR_WITA) return { ...out, skipped: `before ${REASK_HOUR_WITA}:00 WITA` };
  ...
  for (const [to, list] of Object.entries(byContact)) {
    const keep = pickReaskRelays(list);
...
// kbFact: Ikiel's correction of the line before it is written (a typo in
// the owner's answer, a unit); it replaces the staged text on the relay too.
export async function resolveFact(db, relayId, approve, { kbFact = null } = {}) {
  ...
  const fact = kbFact && String(kbFact).trim() ? String(kbFact).trim().slice(0, 600) : r.kb_fact;
  if (fact !== r.kb_fact) await sbPatch(db, `relays?id=eq.${relayId}`, { kb_fact: fact });
  const line = `${fact} (confirmed by ${r.contact_name || 'the villa contact'}, ${stamp})`;
