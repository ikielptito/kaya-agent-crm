...
function relayBody(contactName, prop, q, { followUp = false } = {}) {
  if (isListingInfo(q)) {
    return `Hi ${firstName(contactName)}, Maya here from Samba Realty.\n\n`
      + `To answer agents faster about ${prop}, could you confirm a few details I don't have on file?\n`
      + `${q.slice(LISTING_INFO_PREFIX.length)}\n\n`
      + `Reply here in any format and I'll update the listing.`;
  }
  return (followUp ? `Thanks ${firstName(contactName)} — here's the question.\n\n` : `Hi ${firstName(contactName)}, Maya here from Samba Realty.\n\n`)
...
export async function openRelay(db, wa, {
  ...
  const open = await sbGet(db,
    `relays?contact_wa=eq.${to}&status=in.(queued,asked,answered)&asked_at=gte.${since}&select=id,question`);
  ...
  try {
    ...
    row = (await r.json().catch(() => []))?.[0];
  } catch (e) {
    console.warn('openRelay insert failed:', e.message);
  }
  if (!row?.id) return { ok: false, reason: 'insert failed' };
  ...
  if (mid) {
    await logMessage(db, { ownerId, waNum: to, content: `[Relay → ${prop}] ${q}`, waMessageId: mid });
    return { ok: true, status: 'asked', relayId: row.id };
  }

  // Window shut — open it with the template. The question itself goes out the
  // moment they reply (flushRelayQuestions).
  const tid = await waTemplate(wa, to, OWNER_QUESTION_TEMPLATE, prop);
...
export async function flushRelayQuestions(db, wa, fromNum) {
  ...
  for (const r of rows) {
    ...
    const mid = await waText(wa, to, body);
    if (!mid) continue;
    await sbPatch(db, `relays?id=eq.${r.id}`, { status: 'asked', asked_at: new Date().toISOString() });
    await logMessage(db, {
      ownerId: r.owner_id, waNum: to, waMessageId: mid,
      content: `[Relay → ${prop}] ${r.question}`,
    });
    sent++;
  }
  return sent;
...
export async function openRelaysForContact(db, contactWa) {
  ...
  if (!to) return [];
  return sbGet(db,
    `relays?contact_wa=eq.${to}&status=eq.asked&select=id,question,property_name,rental_slug,agent_wa,agent_id,contact_name,asked_at&order=asked_at.asc`);
}
...
export async function captureRelayAnswer(apiKey, relays, message) {
...
Return ONLY JSON:
{ "relay_id": <id or null>, "answer": "<their answer, rewritten as one clear sentence an agent can act on>", "durable_fact": "<a permanent fact about the property worth storing, or null>", "confident": true|false }
...
export async function deliverAnswers(db, wa, agentWa) {
  ...
  for (const r of rows) {
    ...
    const who = r.contact_name ? ` ${firstName(r.contact_name)} confirmed:` : '';
    const body = `Good news on ${prop} —${who} ${r.answer}`;
    const mid = await waText(wa, to, body);
    if (mid) {
      await sbPatch(db, `relays?id=eq.${r.id}`, { status: 'delivered', delivered_at: new Date().toISOString() });
...
export async function sweepRelays(db, wa) {
  const now = Date.now();
  const out = { nudged: 0, expired: 0, retried: 0 };
  ...
  for (const r of stale) {
    ...
    if (ageH >= RELAY_TTL_HOURS) {
      await sbPatch(db, `relays?id=eq.${r.id}`, { status: 'expired' });
      const prop = r.property_name || r.rental_slug || 'that villa';
      const body = `Following up on your question about ${prop} — I haven't heard back from the villa side yet. `
        + `I'm still chasing it; in the meantime I can put you straight in touch with them if that's quicker.`;
      const mid = await waText(wa, r.agent_wa, body);
      if (mid) await logMessage(db, { agentId: r.agent_id, waNum: r.agent_wa, content: body, waMessageId: mid });
      out.expired++;
      continue;
    }
