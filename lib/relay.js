...
async function agentHasCard(db, agentWa, contactWa, days = 14) {
  ...
  const rows = await sbGet(db, `wa_messages?wa_num=eq.${to}&direction=eq.outbound&timestamp=gte.${since}&content=like.${encodeURIComponent(`[Contact card:%+${num}]`)}&select=id&limit=1`);
