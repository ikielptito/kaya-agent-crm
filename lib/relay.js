...
function relayBody(contactName, prop, q, { followUp = false } = {}) {
  if (isListingInfo(q)) {
    ...
    return `Hi ${firstName(contactName)}, ${q.slice(LISTING_INFO_PREFIX.length)}\n\n`
...
export async function openRelay(db, wa, {
  ...
  const mid = (await windowOpen(db, to)) ? await waText(wa, to, body) : null;
  if (mid) {
...
export async function sweepRelays(db, wa) {
  ...
  for (const r of asked) {
    ...
    const prop = r.property_name || r.rental_slug || 'your listing';
    const tid = await waTemplate(wa, r.contact_wa, OWNER_QUESTION_TEMPLATE, prop);
