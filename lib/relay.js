...
export async function openRelay(db, wa, {
  ...
  const body = `Hi ${firstName(contactName)}, Maya here from Samba Realty.\n\n`
    + `An agent asked about ${prop}:\n"${q}"\n\n`
    + `Reply to this message with the answer and I'll pass it straight back to them.`;
...
export async function flushRelayQuestions(db, wa, fromNum) {
  ...
  for (const r of rows) {
    ...
    const body = `Thanks ${firstName(r.contact_name)} — here's the question.\n\n`
      + `An agent asked about ${prop}:\n"${r.question}"\n\n`
      + `Reply with the answer and I'll pass it straight back to them.`;
    const mid = await waText(wa, to, body);
    if (!mid) continue;
...
export async function sweepRelays(db, wa) {
  ...
  const pending = await sbGet(db, `relays?status=eq.answered&select=agent_wa`);
