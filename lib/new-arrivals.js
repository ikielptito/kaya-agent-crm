...
const CAROUSEL_TEMPLATE = 'samba_weekly_carousel_v1';
...
function cardFor(l, badge) {
  ...
  const area = l.location && !/^https?:|maps\./i.test(l.location) ? l.location : (l.tag || '');
...
export async function sendNewArrivals(db, wa, { eligible, digestProperties, previewMode = false }) {
  ...
  for (const agent of eligible) {
    ...
    try {
      const r = await fetch(`${GRAPH}/${wa.phoneId}/messages`, {
        ...
        body: JSON.stringify({ messaging_product: 'whatsapp', to: agent.wa_num, type: 'template', template: { name: CAROUSEL_TEMPLATE, language: { code: 'en' }, components } }),
      });
      ...
      await fetch(`${db.SUPABASE_URL}/rest/v1/wa_messages`, {
        ...
        body: JSON.stringify({ agent_id: agent.id, wa_num: agent.wa_num, direction: 'outbound', content: `[[carousel]]${JSON.stringify({ title: 'New arrivals on Samba Rentals', cards: cards.map(c => ({ title: c.name, subtitle: c.detail, image: c.imageUrl, url: `https://sambarentals.com/?property=${c.slug}`, badge: c.badge })) })}`, wa_message_id: mid, timestamp: new Date().toISOString(), source: 'cron', category: 'new_arrivals', template_name: CAROUSEL_TEMPLATE, status: 'sent' }),
      }).catch(() => {});
