...
import { coverPhotoUrl, buildCarouselComponents, uploadCardMedia, CAROUSEL_CARD_COUNT } from './wa-carousel.js';
...
function cardFor(l, badge) {
  ...
  const rate = l.monthly ? `${l.monthly}/mo` : (l.yearly ? `${l.yearly}/yr` : '');
...
export async function sendNewArrivals(db, wa, { eligible, digestProperties, previewMode = false, templatesMap = {}, campaign = null }) {
  ...
  const up = await uploadCardMedia(cards, { token: wa.token, phoneId: wa.phoneId });
  out.carousel_media = `${up.uploaded} uploaded, ${up.failed} kept as link`;
