...
import { sendButtons, sendList, sendText, parseTap } from './wa-interactive.js';
...
async function say(db, wa, to, body, buttons = null) {
  const mid = buttons?.list ? await sendList(wa, to, buttons.list) : buttons ? await sendButtons(wa, to, body, buttons) : await sendText(wa, to, body);
...
// ── Read back, then act on Yes ──────────────────────────────────────
// Ikiel's rule (10 Sep 2026): Maya says what she understood BEFORE she
// changes a ticket, so Era can correct her. The proposal is parked for
// thirty minutes; Yes applies it (Undo still offered), "Other ticket"
// re-targets it from a list, Cancel drops it. A typed yes / no works too.
const idr = (n) => 'IDR ' + Math.round(n).toLocaleString('en-US');
export function describeActions(actions, items) {
  const by = Object.fromEntries((items || []).map(i => [i.id, i]));
  return actions.map(a => {
    const it = by[a.id]; if (!it) return null;
    const label = `#${it.id} ${it.title.slice(0, 45)} — ${placeOf(it)}`;
    if (a.action === 'done') return it.status === 'pending_approval'
      ? `⏳ ${label} → still waiting on the owner; I cannot close it from here`
      : `✅ ${label} → *done*${it.status === 'new' ? ' (the owner hears it is finished; add a cost on the page if there is one)' : ' (the owner will be told)'}`;
    if (a.action === 'estimate' && a.estimated_cost) return `💰 ${label} → estimate ${idr(a.estimated_cost)}${it.status === 'new' ? ' (still needs Publish on the page)' : ''}`;
    if (a.action === 'waiting') return `⏸ ${label} → waiting${a.until ? ` until ${a.until}` : ''}: ${a.note}`;
    return `📝 ${label} → note: ${a.note}${a.estimated_cost && !it.estimated_cost ? ` (estimate ${idr(a.estimated_cost)})` : ''}`;
  }).filter(Boolean);
}
export async function proposeActions({ db, wa, num, who = 'Era', text, actions, items, extra = '' }) {
  const lines = describeActions(actions, items);
  if (!lines.length) return false;
  const s = await park(db, num, { kind: 'confirm', text, actions, itemIds: items.map(i => i.id) });
  await say(db, wa, num, `I read that as:\n${lines.join('\n')}${extra}\n\nShall I apply it? Nothing has changed yet.`,
    [{ id: `mt:yes:${s}`, title: 'Yes, apply' }, { id: `mt:pick:${s}`, title: 'Other ticket' }, { id: `mt:drop:${s}`, title: 'Cancel' }]);
  return true;
}
async function applyParked(db, wa, num, parked, { who }) {
  const { applyStatusActions } = await import('./maintenance-backlog-reply.js');
  const ids = [...new Set(parked.actions.map(a => a.id))];
  const items = (await sbGet(db, `maintenance_items?id=in.(${ids.join(',')})&select=*,statement_groups(key,name)`)) || [];
  const lines = await applyStatusActions(db, parked.actions, items, { who });
  const undo = [...new Set(parked.actions.filter(a => a.action === 'done' && lines.some(l => l.startsWith('✅') && l.includes(`#${a.id} `))).map(a => a.id))].slice(0, 3);
  await say(db, wa, num, `Done:\n${lines.join('\n')}`, undo.length ? undo.map(id => ({ id: `mt:undo:${id}`, title: `Undo #${id}` })) : null);
}
// Maya's reading of a message about one ticket, as actions (not applied).
async function readActionsFor(item, { text, apiKey, done = false }) {
  if (done) return [{ id: item.id, action: 'done', note: text.slice(0, 300), estimated_cost: null, until: null }];
  const { parseStatusReply } = await import('./maintenance-backlog-reply.js');
  const parsed = await parseStatusReply(text, [item], { apiKey }).catch(() => null);
  const acts = (parsed?.actions || []).filter(a => a.id === item.id);
  return acts.length ? acts : [{ id: item.id, action: 'note', note: text.slice(0, 300), estimated_cost: null, until: null }];
}
// The "Log as done" tap: file, then close — she asked for both explicitly.
...
const YES_RE = /^\s*(yes|yes[, ]+apply|apply|ya|iya|ok|oke|okay|correct|betul|benar|go ahead|do it|👍)\s*[.!]*\s*$/i;
const NO_RE = /^\s*(no|nope|cancel|batal|tidak|wrong|stop|don'?t)\s*[.!]*\s*$/i;
async function peekPark(db, num) {
  const all = (await getSettingValue(db, PARK_KEY)) || {};
  const mine = all[num];
  return mine && Date.now() - (mine.at || 0) <= PARK_MS ? mine : null;
}
...
export async function guardTicketCreate({ db, wa, fromNum, who, lang = 'en', body, matched, parsedItems = [], apiKey = process.env.ANTHROPIC_API_KEY, pre = null }) {
  ...
  if (team) {
    if (relation === 'done' || relation === 'update') {
      const actions = await readActionsFor(target, { text: body, apiKey, done: relation === 'done' });
      await proposeActions({ db, wa, num, who, text: body, actions, items: [target] });
      return { proceed: false, read };
    }
...
export async function handleTicketTap({ db, wa, fromNum, buttonPayload, text = '', who = 'Era', apiKey = process.env.ANTHROPIC_API_KEY }) {
  let tap = parseTap(buttonPayload);
  const num = String(fromNum || '').replace(/\D/g, '');
  // A typed answer to an open proposal: "yes" applies, "no" cancels.
  if (!tap) {
    const pending = await peekPark(db, num);
    if (!pending || pending.kind !== 'confirm') return false;
    if (YES_RE.test(text)) tap = { domain: 'mt', verb: 'yes', id: pending.sid };
    else if (NO_RE.test(text)) tap = { domain: 'mt', verb: 'drop', id: pending.sid };
    else return false;
  }
  ...
  // "Other ticket": a list to re-target the proposal; the proposal stays
  // parked until she picks or it expires. The row id carries sid.itemId.
  if (tap.verb === 'pick') {
    const parked = await peekPark(db, num);
    if (!parked || parked.sid !== tap.id || parked.kind !== 'confirm') { await say(db, wa, num, 'That question has expired — send the message again and I will ask afresh.'); return true; }
    if (parked.actions.length !== 1) { await say(db, wa, num, 'That reading covers several tickets, so tell me in words instead — for example "#15 done, #16 waiting for the part".'); return true; }
    const open = await openTicketsFor(db, {});
    const first = open.filter(i => parked.itemIds.includes(i.id)).map(i => i.slug || i.group_key);
    const sorted = [...open].sort((a, b) => (first.includes(b.slug || b.group_key) ? 1 : 0) - (first.includes(a.slug || a.group_key) ? 1 : 0));
    await say(db, wa, num, `Which ticket is "${parked.text.slice(0, 80)}" about?`, { list: { body: `Which ticket is "${parked.text.slice(0, 80)}" about?`, buttonLabel: 'Choose', rows: sorted.slice(0, 10).map(i => ({ id: `mt:to:${parked.sid}.${i.id}`, title: `#${i.id} ${i.title}`.slice(0, 24), description: `${placeOf(i)} · ${i.status.replace('_', ' ')}` })) } });
    return true;
  }
  if (tap.verb === 'to') {
    const [s, idStr] = String(tap.id).split('.');
    const parked = await takePark(db, num, s);
    if (!parked) { await say(db, wa, num, 'That question has expired — send the message again and I will ask afresh.'); return true; }
    const item = (await sbGet(db, `maintenance_items?id=eq.${Number(idStr)}&select=*,statement_groups(key,name)&limit=1`))?.[0];
    if (!item) { await say(db, wa, num, `Ticket #${idStr} is gone; nothing applied.`); return true; }
    const actions = parked.actions.map(a => ({ ...a, id: item.id }));
    await applyParked(db, wa, num, { ...parked, actions }, { who });
    return true;
  }
  const parked = await takePark(db, num, tap.id);
  if (!parked) { await say(db, wa, num, 'That question has expired — send the message again and I will ask afresh.'); return true; }
  if (tap.verb === 'drop') { await say(db, wa, num, parked.kind === 'confirm' ? 'Cancelled — nothing changed. Tell me again in words if I misread it (for example "#28 done").' : 'Ok, ignored. Nothing logged.'); return true; }
  if (tap.verb === 'yes' && parked.kind === 'confirm') { await applyParked(db, wa, num, parked, { who }); return true; }
  if (tap.verb === 'upd' && parked.itemId) {
    ...
    const actions = await readActionsFor(item, { text: parked.text, apiKey, done: looksLikeCompletion(parked.text) });
    await proposeActions({ db, wa, num, who, text: parked.text, actions, items: [item] });
    return true;
  }
