...
let _missing = { maintenance_events: false, housekeeping_events: false };
const nowIso = () => new Date().toISOString();

async function write(db, table, row) {
  if (_missing[table]) return null;
  try {
    ...
    if (!r.ok) {
      ...
      if (r.status === 404 || /does not exist|PGRST205|42P01/i.test(t)) _missing[table] = true;
    }
    return r.ok;
  } catch { return null; }
}
...
export async function eventsFor(db, what, id, { limit = 100 } = {}) {
  ...
  try {
    ...
    if (!r.ok) { if (r.status === 404) _missing[table] = true; return []; }
    return r.json();
  } catch { return []; }
}
