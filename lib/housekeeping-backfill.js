...
export async function runEvidenceNudge({ db, wa, now = new Date(), catalogNames = {}, hour: hourOverride = null } = {}) {
  ...
  const today = wita.toISOString().slice(0, 10);
  const hour = hourOverride != null ? hourOverride : wita.getUTCHours();
  if (hour < 15 || hour > 21) return { skipped: `hour ${hour}` };
  // The review can move one person's reminder later when she writes late.
  const profiles = (await getSettingValue(db, 'staff_profiles').catch(() => null)) || {};
  const hourFor = (staff) => { const h = +profiles[digits(staff?.wa_num)]?.nudge_hour; return Number.isInteger(h) ? h : 16; };
  const name = (slug) => catalogNames[slug] || slug;
  ...
  for (const t of done) {
    if ((t.photos || []).length || !t.staff?.wa_num || !t.staff.active || hourFor(t.staff) !== hour) continue;
  ...
  for (const r of rounds) {
    ...
    if (!task?.staff?.wa_num || hourFor(task.staff) !== hour) continue;
