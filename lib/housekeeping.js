...
export const DEFAULTS = {
  ...
  // A stay this long earns a deep clean when it ends, quarter or not.
  deep_clean_after_nights: 21,
  horizon_days: 21,
};
...
export async function fetchStays({ from, to } = {}) {
  ...
  try {
    ...
    // Bounded: this runs on the hourly beat ahead of the chase and Era's
    // brief, and a hung portal call used to starve both for the hour.
    const r = await fetch(`${PORTAL_BASE}/api/statements?${qs}`, {
      headers: { Authorization: `Bearer ${secret}` }, signal: AbortSignal.timeout(20000),
    });
    if (!r.ok) return null;
    return await r.json();
  } catch { return null; }
...
export function planTasks({ units, today, cfg = {}, lastInspection = {}, lastDeepClean = {}, careDays = {} } = {}) {
  ...
  for (const u of units || []) {
    ...
    for (let i = 0; i < stays.length; i++) {
      ...
      // 3. A long stay ends with a deep clean, quarter or not (Oli's rule,
      //    5 Sep 2026): six weeks of someone living there is what the next
      //    guest notices on day one, and the quarterly clock does not know
      //    it happened. The day after the turnover, so the same person is
      //    not asked for two visits' worth of work in one message; on the
      //    turnover day itself when the next guest is already arriving, and
      //    flagged same-day so Era can decide. Skipped when a deep clean
      //    happened in the last month.
      const nights = s.nights ?? diff(s.check_in, s.check_out);
      if (c.deep_clean_after_nights > 0 && nights >= c.deep_clean_after_nights) {
        const recent = lastDeepClean[u.slug] && diff(lastDeepClean[u.slug], s.check_out) < 30;
        if (!recent) {
          const dayAfter = plus(s.check_out, 1);
          const free = !(next && next.check_in <= dayAfter);
          add({
            slug: u.slug, kind: 'deep_clean', after_stay: true,
            task_date: free ? dayAfter : s.check_out,
            same_day: !free,
            guest_in_date: next ? next.check_in : null,
          });
        }
      }
    }
    ...
    if (c.deep_clean_every_days > 0) {
      // A long-stay deep clean already in this plan counts as the last one:
      // the quarterly clock restarts from it rather than adding a second
      // visit a fortnight later.
      const planned = out.filter(t => t.slug === u.slug && t.kind === 'deep_clean').map(t => t.task_date).sort().pop();
      const lastDeep = [lastDeepClean[u.slug], planned].filter(Boolean).sort().pop() || null;
...
// Drop planned tasks that collide with a task already on the calendar for
// the same villa, kind and day (whatever its origin). Pure, so it is pinned.
export function dropCollisions(planned, existing) {
  const taken = new Set((existing || []).map(t => `${t.slug}|${t.kind}|${t.task_date}`));
  return (planned || []).filter(t => !taken.has(`${t.slug}|${t.kind}|${t.task_date}`));
}
...
export async function generateTasks(db, { now = new Date() } = {}) {
  ...
  const raw = planTasks({ units: feed.units, today, cfg, lastInspection, lastDeepClean, careDays });
  // A task someone MOVED onto a day the rule also produces would otherwise
  // come out twice: uniqueness is on origin_date, and the moved task's
  // origin is another day. On 7 Sep 2026 that put a second B3 clean, a
  // second B5 clean and a second Unit 3 clean on the same morning as the
  // ones a housekeeper had just been told about. Anything already on the
  // calendar for that villa, kind and day wins.
  const existing = (await sbGet(db,
    `housekeeping_tasks?task_date=gte.${today}&status=neq.skipped&select=slug,kind,task_date&limit=2000`)) || [];
  const planned = dropCollisions(raw, existing);
