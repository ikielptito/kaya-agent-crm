...
export async function sweepUnanswered(db, wa, { now = new Date() } = {}) {
  ...
  const agents = await sbGet(db,
    `agents?is_test=eq.false&wa_num=not.is.null&dead_number=not.is.true&samba_alerts_opt_out=not.is.true&last_inbound_at=gte.${windowOpenSince}&last_inbound_at=lte.${holdCut}&select=id,name,agency,wa_num,last_inbound_at,automation_override,suggested_reply&limit=200`);
