...
import { bump as bumpCampaign } from './campaigns.js';

const GRAPH_DAY = () => new Date().toISOString().slice(0, 10);

// Send an agent their onboarding welcome as an approved WhatsApp TEMPLATE and log
// it to wa_messages. A template is required because a freshly-added agent has
// never messaged our business number, so no 24h session window exists yet.
// Shared by quick_add_agent and the onboard_existing_agents backfill so the send
// logic can never drift between them. Returns { sent, template } or { sent:false, reason }.
// The caller owns enrollment + quiet-hours deferral — this just sends.
// category/campaignId tag the send for the command center: cold screenshot
// imports report under 'agent_cold', everything else under 'onboarding'.
export async function sendWelcomeTemplate({ SUPABASE_URL, headers, agentId, name, wa, source = 'api', timeoutMs = 8000, category = 'onboarding', campaignId = null }) {
  const TOKEN = process.env.META_WA_TOKEN;
  const PHONE_ID = process.env.META_WA_PHONE_ID;
  const WABA_ID = process.env.META_WABA_ID;
  if (!TOKEN || !PHONE_ID || !WABA_ID) return { sent: false, reason: 'missing_meta_env' };
  // The two Graph API calls below (template list + send) are the slow part of
  // quick_add_agent and occasionally hang, which used to stall the whole add
  // request behind them. Bound them: on timeout we return { sent:false,
  // reason:'timeout' } so the caller can respond immediately and let the 9am
  // cron drain the welcome instead of the browser hanging.
  const ctl = new AbortController();
  const timer = setTimeout(() => ctl.abort(), timeoutMs);
  try {
    const tr = await fetch(`https://graph.facebook.com/v24.0/${WABA_ID}/message_templates?fields=name,status,language,components&limit=100`, {
      headers: { 'Authorization': 'Bearer ' + TOKEN }, signal: ctl.signal
    });
    const td = await tr.json();
    const welcomeTpl = pickWelcomeTemplate(td.data);
    if (!welcomeTpl) { clearTimeout(timer); return { sent: false, reason: 'no_template' }; }
    ...
    clearTimeout(timer);   // Meta calls are done — stop the abort clock before the DB log write.
    if (!sr.ok) return { sent: false, reason: 'send_failed', detail: sd };
    const rendered = (bodyComp?.text || '').replace(/\{\{1\}\}/g, fName);
    await fetch(`${SUPABASE_URL}/rest/v1/wa_messages`, {
      method: 'POST', headers,
      body: JSON.stringify({
        agent_id: agentId, wa_num: wa, direction: 'outbound',
        content: rendered, wa_message_id: sd.messages?.[0]?.id,
        timestamp: new Date().toISOString(), source,
        category, status: 'sent', template_name: welcomeTpl.name,
        campaign_id: campaignId,
      })
    }).catch(() => {});
    if (campaignId) await bumpCampaign({ SUPABASE_URL, sbHeaders: headers }, campaignId, { sent: 1 });
    return { sent: true, template: welcomeTpl.name };
  } catch (e) {
    return { sent: false, reason: e.name === 'AbortError' ? 'timeout' : 'exception', detail: e.message };
