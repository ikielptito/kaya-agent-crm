...
export async function handleStaffMaintenance({ db, wa, fromNum, text, mediaType, mediaId, waToken, force = false, buttonPayload = null, waMessageId = null, replyTo = null, staffSlugs = null, skipGuard = false }) {
  ...
  // ── The guard: never a second ticket for a fault that has one ─────
  // The villa is known; before the words are even read as a report they
  // are resolved against the open tickets at that villa. "Has been
  // repaired" closes (team) or notes (staff) the ticket it is about; the
  // same fault reported twice is a question, not a second ticket; only a
  // genuinely different problem goes on to become one. skipGuard is the
  // team's own "File as new" tap, after the guard has already asked.
  if (!skipGuard && body) {
    try {
      const { guardTicketCreate } = await import('./ticket-guard.js');
      const g = await guardTicketCreate({ db, wa, fromNum: me, who: reporterName, lang: inId ? 'id' : 'en', body, matched });
      if (!g.proceed) return true;
    } catch (e) { console.warn('ticket guard failed, filing as new:', e.message); }
  }
