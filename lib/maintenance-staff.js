...
import { matchProperty, createItem, savePhoto, savePendingPhoto, attachPhotoPaths, completeItem, snoozeItem, appendThread, isReporter } from './maintenance.js';
import { looksLikeMaintenance, extractReports, parseStaffReply } from './maintenance-intake.js';
...
export async function handleStaffMaintenance({ db, wa, fromNum, text, mediaType, mediaId, waToken }) {
  ...
  if (!matched) {
    // A photo we can't place yet: park it rather than lose it, and ask once
    // — people routinely send two or three pictures and explain afterwards,
    // and being asked after every single photo would be maddening.
    if (hasImage) {
      const media = await fetchMediaBase64(mediaId, waToken);
  ...
  for (const parsed of parsedItems) {
    ...
    // Photos belong to the whole report, so every ticket from it gets them —
    // Era can delete any that don't apply.
    if (parked.length) await attachPhotoPaths(db, item.id, parked).catch(() => {});
    if (freshPhoto) await savePhoto(db, item.id, { base64: freshPhoto.base64, contentType: freshPhoto.mime }).catch(() => {});
  }
  ...
  const nPhotos = parked.length + (freshPhoto ? 1 : 0);
  ...
  await sendText(wa, fromNum,
    `📋 Logged for *${where}*${nPhotos ? ` (${nPhotos} photo${nPhotos > 1 ? 's' : ''})` : ''}:\n${lines.join('\n')}\n\n` +
    (anyCost
