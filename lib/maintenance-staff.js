...
import { rememberPhoto, photoForWamid, suggest, askEra } from './photo-assign.js';
...
export async function handleStaffMaintenance({ db, wa, fromNum, text, mediaType, mediaId, waToken, force = false, buttonPayload = null, waMessageId = null, replyTo = null }) {
  ...
  if (matched.ambiguous) {
    if (hasImage) {
      const media = await fetchMediaBase64(mediaId, waToken);
      if (media) {
        try { await parkPhoto(db, me, await rememberPhoto(db, waMessageId, await savePendingPhoto(db, me, { base64: media.base64, contentType: media.mime }))); } catch {}
      }
    }
    ...
    await sendList(wa, fromNum, {
      body: `Which villa is this for? I don't want to log it against the wrong one.`,
      buttonLabel: 'Pick the villa',
  ...
  for (const path of allPaths) {
    const certain = created.length === 1 || path === freshPath || path === quotedPath;
    if (certain) { for (const t of created) await attachPhotoPaths(db, t.id, [path]).catch(() => {}); continue; }
    unsure.push(path);
    for (const t of created) await suggest(db, t.id, [path], { why: 'sent before the report, ticket unclear', from: me }).catch(() => {});
  }
  if (unsure.length) {
    await askEra(db, wa, { items: created.map(c => ({ id: c.id, title: c._parsed.title })), photos: unsure, villa: matched.group?.name || null }).catch(() => {});
  }
  ...
  const ask = unsure.length
    ? `\n\n❓ ${unsure.length > 1 ? `${unsure.length} photos` : 'One photo'} could belong to more than one job, so ${unsure.length > 1 ? 'they are' : 'it is'} not on any ticket yet — I have sent ${unsure.length > 1 ? 'each one' : 'it'} to you with buttons to pick the ticket.`
    : '';
