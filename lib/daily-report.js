...
async function gather(env, hours = 24) {
  ...
  // Tier split for context — buckets that add up to the enrolled total
  // (aliases folded, untagged counted, introduced-silent shown as a stage).
...
- STATS should lead with portal activity and compare to the 7-day average when it's meaningfully above or below it. The "Samba audience" line already adds up to the total: quote it whole (every bucket, including untagged and introduced-no-reply) or not at all — never a subset presented as the whole. Merge small buckets as "other" if you must save characters. If there are NEW SIGNUPS, always name them — a new agent account is headline material and usually worth a welcome touch in YOUR MOVE.
...
Samba audience: ${s.audience_line}
