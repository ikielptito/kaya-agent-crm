...
// ── Asking Era, one photo at a time ─────────────────────────────────
// The photo itself, then buttons: up to two tickets and "not a fault". More
// than two candidates become a list. Each pending photo has a short id.
export async function askEra(db, wa, { items, photos, villa = null }) {
  const era = ERA();
  ...
  for (const path of photos.slice(0, 8)) {
    ...
    pending[sid] = { path, items: items.map(i => ({ id: i.id, title: String(i.title).slice(0, 60) })), at: nowIso() };
    await sendImage(wa, era, url, `${villa ? `${villa} — ` : ''}which ticket is this photo for?`);
    const label = (i) => `#${i.id} ${String(i.title).replace(/^(maintenance|inspection)\s*[-–:]?\s*/i, '')}`.slice(0, 20);
    if (items.length <= 2) {
      await sendButtons(wa, era, `Photo ${asked + 1}: tap the ticket it shows, or "Not a fault".`, [...items.map(i => ({ id: `pa:pick:${sid}.${i.id}`, title: label(i) })), { id: `pa:none:${sid}`, title: 'Not a fault' }]);
    } else {
      await sendList(wa, era, { body: `Photo ${asked + 1}: which ticket does it show?`, buttonLabel: 'Pick the ticket', rows: [...items.slice(0, 9).map(i => ({ id: `pa:pick:${sid}.${i.id}`, title: `#${i.id}`, description: String(i.title).slice(0, 72) })), { id: `pa:none:${sid}`, title: 'Not a fault', description: 'keep it on the round only' }] });
    }
    asked++;
    // On her thread in the console too, so the question is visible there.
    await fetch(`${db.SUPABASE_URL}/rest/v1/wa_messages`, { method: 'POST', headers: { ...db.sbHeaders, Prefer: 'return=minimal' }, body: JSON.stringify({ wa_num: era, direction: 'outbound', content: `[Photo ${asked} of ${Math.min(photos.length, 8)} — which ticket? ${items.map(i => `#${i.id}`).join(' / ')} / not a fault]`, timestamp: nowIso(), source: 'cron', category: 'photo_assign', status: 'sent' }) }).catch(() => {});
...
export async function handlePhotoTap({ db, wa, fromNum, buttonPayload }) {
  ...
  const p = pending[sid];
  if (!p) { await sendText(wa, fromNum, 'That photo question has expired; the photos are still on the round in Records.'); return true; }
  delete pending[sid];
  await saveSettingValue(db, PENDING_KEY, pending);
  if (tap.verb === 'none') {
    await rejectPhoto(db, p.path);
    await sendText(wa, fromNum, 'Okay — not attached to any ticket. It stays on the round.');
    return true;
  }
  const it = p.items.find(i => String(i.id) === String(itemId));
  if (!it) { await sendText(wa, fromNum, 'I could not match that tap to a ticket.'); return true; }
  await confirmPhoto(db, it.id, p.path);
  await sendText(wa, fromNum, `Attached to #${it.id} ${it.title}. The owner sees it on the ticket now.`);
  return true;
}
