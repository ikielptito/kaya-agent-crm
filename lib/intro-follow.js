...
//
// The ladder now: intro → (gap) Monday digest → (gap) Monday digest → stalled.
...
export const INTRO_FOLLOW_DEFAULTS = {
  intro_follow_max: 2,          // digests after the intro before we stop
...
function lastRung(samba, agent) {
  return samba.last_intro_digest_at || samba.intro_at || agent.last_availability_alert_at || null;
}

/** Pure: is this introduced-but-silent contact due for the next digest? */
export function introFollowDue(agent, cfg, now = new Date()) {
...
export function introStallDue(agent, cfg, now = new Date()) {
  if (sambaStatus(agent) !== INTRO_SENT) return false;
  const samba = agent.campaign_engagement.samba;
  if ((samba.intro_digests || 0) < cfg.intro_follow_max) return false;
  return daysSince(lastRung(samba, agent), now) >= cfg.intro_follow_gap_days;
 ...
 */
export function pickIntroFollowUps(agents, config, now, gate = () => true) {
  const cfg = introFollowConfig(config);
  if (cfg.intro_follow_weekly_cap <= 0 || cfg.intro_follow_max <= 0) return [];
  return agents
    .filter(a => introFollowDue(a, cfg, now) && gate(a))
...
export async function runIntroStall({ SUPABASE_URL, sbHeaders }, agents, config, now = new Date(), { dryRun = false } = {}) {
  const cfg = introFollowConfig(config);
  const due = agents.filter(a => introStallDue(a, cfg, now));
