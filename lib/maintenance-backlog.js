...
// 09:00 folded into the morning brief (lib/era-brief.js) on 6 Sep 2026.
const DEFAULTS = { era_nudge_hours: [12, 15, 18], era_nudge_min_age_hours: 2, overdue_days: 3, ikiel_digest_hour: 17 };
...
export async function eraBacklog(db, { now = new Date(), minAgeHours = 0 } = {}) {
  ...
  for (const i of items) {
    ...
    // Snoozed ("waiting until the 20th") means not waiting on her today.
    if (i.status !== 'new' && i.next_followup_at && Date.parse(i.next_followup_at) > now.getTime()) continue;
    // A tukang is booked: the dispatch updates cover it.
    if (['offered', 'confirmed', 'arrived'].includes(i.visit_status)) continue;
    let action = null, since = i.created_at;
    if (i.status === 'new') action = i.estimated_cost ? 'publish it (estimate is in)' : (i.heads_up_sent_at ? 'add the estimate and publish (owner already has the heads-up)' : 'add an estimate and publish');
...
export async function runEraBacklogNudge({ db, wa, now = new Date(), force = false, preview = false } = {}) {
  ...
  // The same list is not sent three times a day. A nudge goes at the first
  // slot of the day, and at later slots only when the list has changed
  // (audit, 8 Sep 2026: "6 waiting on Era" at 12, 15 and 18, verbatim).
  const sig = backlog.map(b => `${b.id}:${b.action}`).join('|');
  const unchangedToday = state.last_sig === sig && String(state.last_slot || '').startsWith(day);
  const dueEra = force || (hours.includes(hour) && state.last_slot !== slot && !unchangedToday);
  ...
  if (preview) { out.era = { would_send: dueEra, unchanged_today: unchangedToday, message: msg }; }
  else if (dueEra) {
    ...
    if (mid) {
      ...
      await saveSettingValue(db, STATE_KEY, { ...state, last_slot: slot, last_sent_at: nowIso(), last_count: backlog.length, last_sig: sig, streak });
