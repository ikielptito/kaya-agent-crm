...
const FACTS = {
  ...
  era: [
    ...
    'Expenses: tell Maya what you paid — "laundry HAUS 5 250rb", "beli galon 3x untuk Saturno 60rb" — and send the receipt photo right after. It lands on that month\'s draft statement (flagged "via Maya"), or waits and is added when the draft is built from your sheet, skipping anything the sheet already has; the receipt is filed under Invoices & receipts. No need to write it in the sheet as well. A month already published becomes a change request for Ikiel.',
